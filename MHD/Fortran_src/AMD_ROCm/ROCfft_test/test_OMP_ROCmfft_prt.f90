...
!
      implicit none
!
      character(len=kchara) :: file_name = 'prt_OMP_ROCmfft_test.dat'
...
!
!   Forward transform
        call multi_pin_fwd_OMP_ROCmFFT(fwd, WK_fft, ft1%s_k(1,1),       &
     &                                 elapsed(2), elapsed(3))
!
        start = OMP_GET_WTIME()
!$omp parallel workshare
...
!
!   Backword transform
        call multi_pin_bwd_OMP_ROCmFFT(bwd, WK_fft, ft1%f_x,            &
     &                                 elapsed(2), elapsed(3))
      end do
!
!   Finalize
...
!
      stop 'finish'
      end program test_OMP_ROCmfft_prt
