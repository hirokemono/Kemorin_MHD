...
!
        start = OMP_GET_WTIME()
        call norm_swap_from_prt_fwd_FFTW((nd-1), ione, n_comp,          &
     &      WK_fwd%NFFT_c, WK_fwd%C_ROCmFFT(1),                         &
     &      int(fwd%Nfft), WK_fwd%aNfft, X(1,1))
        elapsed_cpy = elapsed_cpy + OMP_GET_WTIME() - start
      end do
!
...
!   Backword transform
      do nd = 1, n_comp
        start = OMP_GET_WTIME()
        call norm_swap_to_prt_bwd_FFTW((nd-1), ione, n_comp,            &
     &      int(bwd%Nfft), X(1,1), WK_bwd%NFFT_c, WK_bwd%C_ROCmFFT(1))
        elapsed_cpy = elapsed_cpy + OMP_GET_WTIME() - start
!
        start = OMP_GET_WTIME()
