!>@file   LIC_visualize_each_repart.f90
!!@brief  module LIC_visualize_each_repart
!!
...
!!     &          rep_ref, m_SR)
!!      subroutine LIC_fixview_viz_each_repart                          &
!!     &         (istep_lic, time, geofem, ele_comm, next_tbl, nod_fld, &
!!     &          repart_p, rep_ref_m, repart_data, pvr, lic_param,     &
!!     &          rep_ref, m_SR)
...
!!@endverbatim
!
      module LIC_visualize_each_repart
!
      use m_precision
      use calypso_mpi
!
...
!
!
      call LIC_fixview_viz_each_repart(istep_lic, time,                 &
     &    geofem, ele_comm, next_tbl, nod_fld, repart_p, rep_ref_m,     &
     &    repart_data, pvr, lic_param, rep_ref, m_SR)
!
      call LIC_movie_visualize_each_repart(istep_lic, time,             &
     &    geofem, ele_comm, next_tbl, nod_fld, repart_p, rep_ref_m,     &
     &    repart_data, pvr, lic_param, rep_ref, m_SR)
!
      call LIC_movie_quilt_each_repart(istep_lic, time,                 &
     &    geofem, ele_comm, next_tbl, nod_fld, repart_p, rep_ref_m,     &
     &    repart_data, pvr, lic_param, rep_ref, m_SR)
!
      call s_LIC_anaglyph_w_each_repart(istep_lic, time,                &
     &    geofem, ele_comm, next_tbl, nod_fld, repart_p, rep_ref_m,     &
     &    repart_data, pvr, lic_param, rep_ref, m_SR)
!
...
!
      end subroutine LIC_fixview_viz_each_repart
!
!  ---------------------------------------------------------------------
!
      end module LIC_visualize_each_repart
