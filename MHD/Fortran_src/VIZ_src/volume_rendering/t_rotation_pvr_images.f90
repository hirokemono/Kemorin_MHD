...
!
!
      call alloc_rot_pvr_image_arrays(view_param, rot_imgs)
!
      call set_rank_to_write_rot_images(pvr_rgb,                        &
     &    view_param%num_rot, rot_imgs%rot_pvr_rgb(1))
      do i_rot = 1, view_param%num_rot
...
!  ---------------------------------------------------------------------
!
      subroutine alloc_rot_pvr_image_arrays(view_param, rot_imgs)
!
      type(pvr_view_parameter), intent(in) :: view_param
      type(rotation_pvr_images), intent(inout) :: rot_imgs
!
!
      allocate(rot_imgs%rot_pvr_rgb(view_param%num_rot))
!
...
!
     ...
     &         (org_pvr_rgb, num_rot, rot_pvr_rgb)
!
      integer(kind = kint), intent(in) :: num_rot
      type(pvr_image_type), intent(in) :: org_pvr_rgb
      type(pvr_image_type), intent(inout) :: rot_pvr_rgb(num_rot)
!
      integer(kind = kint) :: i_rot
!
!
...
!
      i_rot = num_rot
      rot_pvr_rgb(i_rot)%irank_end_composit = nprocs - 1
      rot_pvr_rgb(i_rot)%npe_img_composit                               &
     &        = rot_pvr_rgb(i_rot+1)%irank_end_composit                 &
     &         - rot_pvr_rgb(i_rot)%irank_image_file + 1
!
