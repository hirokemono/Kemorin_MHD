...
!
     ...
     &          mat_istack, mat_item, fld_params, pvr_bound, projected)
!
...
!
     ...
     &          projected, view_params(i_pvr), pvr_bound(i_pvr))
        end if
      end do
!
...
!
     ...
     &          ntot_int_3d, dnx, xjac, num_nod_phys, num_tot_nod_phys, &
     &          istack_nod_component, d_nod, fld_params, projected)
!
      do i_pvr = 1, num_pvr
...
!
     ...
     &       cbar_params(i_pvr), view_params(i_pvr), pvr_bound(i_pvr),  &
     &      pixel_xy(i_pvr))
!
