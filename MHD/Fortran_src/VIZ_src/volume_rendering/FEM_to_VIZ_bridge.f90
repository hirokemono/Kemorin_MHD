...
!
     ...
     &      geofem%mesh%edge, VIZ_DAT%edge_comm, VIZ_DAT%edge_gl)
      end if
!
...
!     &     (geofem%mesh%ele%nnod_4_ele, VIZ_DAT%jacobians%g_FEM)
        call set_max_integration_points(ione, VIZ_DAT%jacobians%g_FEM)
        call jacobian_and_element_volume(my_rank, nprocs,               &
     &      geofem%mesh, geofem%group, spfs, VIZ_DAT%jacobians)
...
! ----------------------------------------------------------------------
!
      subroutine normals_and_jacobians_all_VIZs                         &
     &         (viz_step, geofem, VIZ_DAT)
!
      use t_fem_gauss_int_coefs
      use int_volume_of_domain
...
!
!
      call normals_and_jacobians_4_VIZ(viz_step, geofem, VIZ_DAT)
!
...
!
     ...
     &   (VIZ_DAT%ele_4_nod_v, VIZ_DAT%jacobians_v, VIZ_DAT)
      call normals_and_jacobians_all_VIZs(viz_step, geofem, VIZ_DAT)
!
