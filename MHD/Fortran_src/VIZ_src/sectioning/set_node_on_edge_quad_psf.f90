...
!
!
      if(    const_psf(1).eq.zero .and. const_psf(2).eq.zero            &
     &  .or. const_psf(1).eq.zero .and. const_psf(2).eq.zero            &
     &  .or. const_psf(1).eq.zero .and. const_psf(2).eq.zero) return
!
!$omp parallel do private(ist,ied,inum,iedge,inod1,inod2,               &
!$omp&                    x_mid,y_mid,z_mid,x_dif,y_dif,z_dif,diag,     &
!$omp&                    c_xi2,c_xi1,c_xi0,Xi,X1,X2,ierr)
      do ip = 1, np_smp
