...
!
!!      subroutine node_on_edge_4_quad_psf_smp(numnod, numedge,         &
...
!!     &          nnod_4_edge, ie_edge, xx, const_psf, nnod_on_edge,    &
!!     &          ied_prev, ied_edge, iedge_4_nod, coef_on_edge)
!
!
      module set_node_on_edge_quad_psf
!
...
!
      integer(kind = kint) :: ip, ied_prev, ied_edge
!
!
!$omp parallel do private(ied_prev,ied_edge)
      do ip = 1, np_smp
        ied_prev = istack_n_on_e_smp(ip-1)
        ied_edge = istack_n_on_e_smp(ip)
...
!$omp end parallel do
!
      end subroutine node_on_edge_4_quad_psf_smp
!
!  ---------------------------------------------------------------------
!
      subroutine set_node_on_edge_4_quad_psf(numnod, numedge,           &
...
!
      integer(kind = kint), intent(in) :: nnod_on_edge
      integer(kind = kint), intent(in) :: ied_prev, ied_edge
      integer(kind = kint), intent(in) :: iedge_4_nod(nnod_on_edge)
!
      real(kind = kreal), intent(in) :: const_psf(10)
!
