...
!
      subroutine set_all_block_points_4_itp                             &
     &         (nnod, nele, xx, nprocs_org, para_mesh)
!
      use t_mesh_data
      use t_geometry_data
...
!
      integer(kind = kint), intent(in) :: nnod, nele, nprocs_org
...
!
      call set_block_points_4_itp(nnod, xx, dest_block)
      do i = 1, nprocs_org
        call set_block_points_4_itp(para_mesh(i)%mesh%node%numnod,      &
     &      para_mesh(i)%mesh%node%xx, org_blocks(i))
      end do
!
      do i = 1, nprocs_org
        call allocate_block_list(nnod, org_blocks(i))
        call set_block_list_4_target(nnod, xx(1,1),                     &
     &      org_blocks(i)%xdomain_min, org_blocks(i)%xdomain_max,       &
     &      num_itp_block(1), org_blocks(i)%iblock_tgt_node)
!
        org_blocks(i)%ele_list_by_ctr%num_grp = ntot_itp_block
...
!
     ...
     &     org_blocks(i)%xdomain_min, org_blocks(i)%xdomain_max,        &
     &     num_itp_block, org_blocks(i)%ele_list_by_ctr%num_grp,        &
...
!
     ...
     &    (para_mesh(i)%mesh%node%numnod, para_mesh(i)%mesh%ele%x_ele,  &
     &     org_blocks(i)%xdomain_min, org_blocks(i)%xdomain_max,        &
     &     num_itp_block, org_blocks(i)%ele_list_by_ctr%num_grp,        &
     &     org_blocks(i)%ele_list_by_ctr%num_item,                      &
     &     org_blocks(i)%ele_list_by_ctr%nitem_grp,                     &
...
!
!
        org_blocks(i)%ele_list_by_rng%num_grp = ntot_itp_block
        call allocate_grp_type_num(org_blocks(i)%ele_list_by_rng)
!
        call count_ele_list_with_range                                  &
     ...
     &     num_itp_block, org_blocks(i)%ele_list_by_rng%num_grp,        &
...
!
      ...
      write(id_file,*) '#'
!
      write(id_file,'(3i16)') num_itp_block(1:3)
      close(id_file)
!
...
!
      ntot_itp_block                                                    &
     &      = num_itp_block(1)*num_itp_block(2)*num_itp_block(3)
      allocate(org_blocks(nprocs_org))
!
      do i = 1, nprocs_org
        call allocate_block_points(org_blocks(i))
      end do
      call allocate_block_points(dest_block)
!
      end subroutine alloc_interpolate_blocks
!
!  ---------------------------------------------------------------------
!
      subroutine allocate_block_points(block)
!
!
      type(block_4_interpolate), intent(inout) :: block
!
!
      allocate( block%x_block(0:num_itp_block(1)) )
      allocate( block%y_block(0:num_itp_block(2)) )
      allocate( block%z_block(0:num_itp_block(3)) )
!
      block%x_block = 0.0d0
      block%y_block = 0.0d0
      block%z_block = 0.0d0
...
!  ---------------------------------------------------------------------
!
      subroutine allocate_block_list(nnod, block)
!
      integer(kind = kint), intent(in) :: nnod
      type(block_4_interpolate), intent(inout) :: block
!
!
      allocate( block%iblock_tgt_node(nnod,3) )
!
      block%iblock_tgt_node = 0
!
...
!  ---------------------------------------------------------------------
!
      subroutine set_block_points_4_itp(nnod, xx, block)
!
      use set_ele_list_4_itp_table
!
      integer(kind = kint), intent(in) :: nnod
...
!
!
      call allocate_block_points(block)
!
      call set_block_boundary(nnod, xx(1,1), num_itp_block(1),          &
     &    block%xdomain_min(1), block%xdomain_max(1), block%x_block)
      call set_block_boundary(nnod, xx(1,2), num_itp_block(2),          &
     &    block%xdomain_min(2), block%xdomain_max(2), block%y_block)
      call set_block_boundary(nnod, xx(1,3), num_itp_block(3),          &
