...
!
!
            write(l_out,'(10i10)')   nodgrptot
            write(l_out,'(10i10)')  (index(i),i=1,nodgrptot)
!                                                 ..zmin
            write(l_out,'(a)'  )  'zmin'
            if (kpe == 1) then 
              write(l_out,'(6i10)') ((node_id_lc(i,j,1),i=1,nx),j=1,ny)
            else
              write(l_out,'(6i10)')
            endif
!                                                 .. zmax
            write(l_out,'(a)'  )  'zmax'
            if (kpe == ndz) then 
              write(l_out,'(6i10)') ((node_id_lc(i,j,nz),i=1,nx),j=1,ny)
            else
              write(l_out,'(6i10)')
            endif
!                                                           . mid.
