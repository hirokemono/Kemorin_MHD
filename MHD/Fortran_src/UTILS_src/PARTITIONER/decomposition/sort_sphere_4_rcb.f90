...
!!      &        (nnod, ncou, num_domain, nlevel, nproc, part_tbl,      &
!!      &         sort_item, ig_item, tbl_size, order,                  &
!!      &         group_id, domain_id)
!!      subroutine set_domain_list_with_part_volume                     &
!!     &         (nnod, ncou, num_domain, nlevel, nproc, part_volume,   &
!!     &          n_volume, sort_item, ig_item, tbl_size, order,        &
!!     &          group_id, domain_id)
!
      module sort_sphere_4_rcb
!
...
!     which is the smallest num in this partition group
     ...
     &   'istart,iend,tbl_size,nproc,ncnt',                             &
     &    istart,iend,tbl_size,nproc,ncnt
      do ii = 1, tbl_size
...
!write(*,*) 'sum ratio', sum_ratio
      ...
      num(1:nproc) = int(ratio(1:nproc)) * ncou
      num(nproc) = ncou
      do ii = 1, nproc-1
        num(nproc) = num(nproc) - num(ii)
      end do
      if(iflag_debug .gt. 0) write(*,*) 'num of node', ncou,            &
     &                                  'node num ', num(:)
      icou = 0
      do ii = 1, nproc
        do ic= 1, num(ii)
...
!     which is the smallest num in this partition group
      do istart = 1, num_domain
        if(domain_id(istart) .eq. order) exit
      end do
      iend = istart + tbl_size - 1
