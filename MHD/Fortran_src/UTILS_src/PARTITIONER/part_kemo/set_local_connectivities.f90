...
!
     ...
     &         (ele_new, surf_org, surf_d_grp, surf_new)
!
...
!
     ...
     &                      = surf_d_grp%id_local_part(isurf)           &
...
!
     ...
     &         (ele_new, edge_org, edge_d_grp, edge_new)
!
      use m_geometry_constants
!
      use t_geometry_data
...
!
     ...
     &                      = edge_d_grp%id_local_part(iedge)           &
...
!
      do inum = 1, surf_new%numsurf
        ...
        do k1 = 1, surf_org%nnod_4_surf
          ...
          surf_new%ie_surf(inum,k1) = nod_d_grp%id_local_part(inod_g)
        end do
      end do
!
...
!
     ...
     &         (surf_new, edge_org, edge_d_grp, edge_new)
!
      use m_geometry_constants
      use t_surface_data
      use t_edge_data
...
!   --------------------------------------------------------------------
!
      subroutine set_local_connect_4_edge                               &
     &         (edge_org, nod_d_grp, edge_new)
!
      use t_edge_data
!
      type(edge_data), intent(in) :: edge_org
...
!
      do inum = 1, edge_new%numedge
        ...
        do k1 = 1, edge_org%nnod_4_edge
          ...
          edge_new%ie_edge(inum,k1) = nod_d_grp%id_local_part(inod_g)
        end do
      end do
!
