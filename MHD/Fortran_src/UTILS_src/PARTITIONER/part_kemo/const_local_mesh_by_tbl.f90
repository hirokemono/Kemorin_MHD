...
!
     ...
     &   (part_p, numnod, ele%numele, ele_grp, domain_grp%ele_d_grp,    &
     &    internals_part%itl_ele_part, included_ele)
      call const_local_node_by_near_tbl(ele, part_p%num_domain,         &
...
!
!
      call s_const_local_mesh_by_tbl(part_p, numnod, ele, ele_grp,      &
     &    internals_part, domain_grp, included_ele)
!
      call const_local_surf_by_near_tbl(ele, surf, part_p%num_domain,   &
     &    internals_part%itl_ele_part, internals_part%itl_surf_part,    &
     &    domain_grp%surf_d_grp)
      call const_local_edge_by_near_tbl(ele, edge, part_p%num_domain,   &
...
!
     ...
     &         (part_p, numnod, numele, ele_grp, ele_d_grp,             &
...
!
!
      call alloc_numbers_4_part(part_p%num_domain, itl_ele_part)
!
      itl_ele_part%num_4_subdomain(1:part_p%num_domain)                 &
     &     = included_ele%num_nod(1:part_p%num_domain)
      itl_ele_part%istack_4_subdomain(0:part_p%num_domain)              &
     &     =  included_ele%istack_nod(0:part_p%num_domain)
...
!
     ...
     &    numnod, numele, ele_grp, part_p%num_domain,                   &
     &    included_ele%ntot, included_ele%istack_nod,                   &
     &    included_ele%id_near_nod, itl_ele_part%ntot_sub,              &
     &    itl_ele_part%id_4_subdomain)
!
      call dealloc_near_node(included_ele)
      call dealloc_num_4_near_node(included_ele)
!
      call count_internal_id_by_tbl                                     &
     &   (part_p%num_domain, ele_d_grp, itl_ele_part)
!
      call s_cal_minmax_and_stacks                                      &
     &   (part_p%num_domain, itl_ele_part%num_inter_sub, izero,         &
