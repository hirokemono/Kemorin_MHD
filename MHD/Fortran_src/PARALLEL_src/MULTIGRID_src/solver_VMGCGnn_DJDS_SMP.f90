...
!
     ...
     &     matNN(0)%aiccg(1), matNN(0)%aiccg(matNN%istart_l),           &
     &     matNN(0)%aiccg(matNN(0)%istart_u), W(1,R), B, X, W(1,iWK))
!
!C
!C +---------------+
!C | BNORM2 = B^2  |
...
!
     ...
     &      ntotWK_CG, W(1,1), SR_sig, SR_r, COMPtime_CG, COMMtime_MG)
...
!
     ...
     &     djds_tbl(0)%indexDJDS_L, djds_tbl(0)%indexDJDS_U,            &
     &     djds_tbl(0)%itemDJDS_L, djds_tbl(0)%itemDJDS_U,              &
     &     matNN(0)%aiccg(1), matNN(0)%aiccg(matNN(0)%istart_l),        &
     &     matNN(0)%aiccg(matNN(0)%istart_u), W(1,Q), W(1,P), W(1,iWK))
!
!C===
!C +---------------------+
!C | ALPHA= RHO / {p}{q} |
