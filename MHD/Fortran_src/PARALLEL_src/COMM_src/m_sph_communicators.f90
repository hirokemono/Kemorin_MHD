...
!>      maximum number of components for date transform for  f(r,t,p) 
...
!>      maximum number of components for date transform for  f(r,j) 
...
!
      integer(kind = kint) :: icolor_rj_rlm, icolor_rtp_rtm
      integer(kind = kint) :: isum_rtp, isum_rj
!
!
      if(nneib_domain_rtp .gt. 1) then
        if(abs(id_domain_rtp(2) - id_domain_rtp(1)) .eq. 1) then
          icolor_rj_rlm =  mod(my_rank,nneib_domain_rtp)
          icolor_rtp_rtm = (my_rank - mod(my_rank,nneib_domain_rtp))    &
     &                   / nneib_domain_rtp
        else
          icolor_rtp_rtm =  mod(my_rank,nneib_domain_rj)
          icolor_rj_rlm =  (my_rank - mod(my_rank,nneib_domain_rj))     &
     &                   / nneib_domain_rj
        end if
      else
        icolor_rtp_rtm = 0
        icolor_rj_rlm =  my_rank
      end if
!
      call MPI_COMM_SPLIT(CALYPSO_COMM, icolor_rtp_rtm, my_rank,        &
     &    CALYPSO_RTP_COMM, ierr_MPI)
      call MPI_COMM_SIZE(CALYPSO_RTP_COMM, nprocs_rtp, ierr_MPI)
      call MPI_COMM_RANK(CALYPSO_RTP_COMM, my_rank_rtp, ierr_MPI)
!
      call MPI_COMM_SPLIT(CALYPSO_COMM, icolor_rj_rlm, my_rank,         &
     &    CALYPSO_RJ_COMM, ierr_MPI)
      call MPI_COMM_SIZE(CALYPSO_RJ_COMM, nprocs_rj, ierr_MPI)
      call MPI_COMM_RANK(CALYPSO_RJ_COMM, my_rank_rj, ierr_MPI)
!
      if(i_debug .eq. 0) return
!
      write(*,'(a,6i4)') 'my_rank, RTP_COMM, np_rtp, rank_rtp',         &
     &           my_rank, CALYPSO_RTP_COMM, nprocs_rtp, my_rank_rtp
      write(*,'(a,6i4)') 'my_rank, RJ_COMM,  np_rj,  rank_rj ',         &
...
!
     ...
     &     CALYPSO_RJ_COMM,ierr_MPI)
      write(*,'(a,6i4)') 'SUM_test', my_rank, isum_rtp, isum_rj
!
...
!
     ...
     &                                            ' ', message
!
      call  MPI_ABORT(CALYPSO_RTP_COMM, ierr_MPI)
!
...
!
     ...
     &                                            ' ', message
!
      call  MPI_ABORT(CALYPSO_RJ_COMM, ierr_MPI)
!
