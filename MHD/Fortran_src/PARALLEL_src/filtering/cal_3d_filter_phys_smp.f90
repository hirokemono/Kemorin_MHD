...
!!     &          i_field, i_filter, x_flt, nod_fld, vect)
...
!
     ...
     &    filter_smp%ied_nsum, nnod_flt, x_flt,                         &
     &    node%numnod, vect%x_vec(1))
!
      call scalar_send_recv_3d_filter(nod_comm, node%numnod,            &
     &    vect%x_vec(1), nod_fld%ntot_phys, i_filter, nod_fld%d_fld)
!
...
!
     ...
     &    filter_smp%istack_near_nod, filter_smp%ntot_near_nod,         &
     &    filter_smp%inod_near, filter_smp%weight,                      &
     &    nnod_flt, x_flt, node%numnod, vect%x_vec(1))
