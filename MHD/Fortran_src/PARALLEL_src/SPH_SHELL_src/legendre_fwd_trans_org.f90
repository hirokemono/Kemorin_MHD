...
!$omp             private(ip,kst,ked,lp,lst,led,l_rtm,j_rlm,k_rlm,nd,   &
...
!
      integer(kind = kint) :: l_rtm, ip_rtm, i_rlm, k_rlm, j_rlm
      integer(kind = kint) :: nd, ip, kst, ked, lp, lst, led
      real(kind = kreal) :: sp1
...
!
!$omp parallel do schedule(static)                                      &
!$omp&            private(ip,kst,ked,lp,lst,led,j_rlm,k_rlm,nd,         &
