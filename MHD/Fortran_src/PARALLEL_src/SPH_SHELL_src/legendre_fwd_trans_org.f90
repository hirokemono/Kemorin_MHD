...
!$omp             private(ip,kst,ked,l_rtm,j_rlm,k_rlm,nd,i_rlm,        &
!$omp&                    ip_rtm,in_rtm,r2_1d_rlm_r,sp1,sp2,sp3,        &
...
!
      integer(kind = kint) :: i_rlm, k_rlm, j_rlm
      integer(kind = kint) :: l_rtm, ip_rtm, nd, ip, kst, ked
      real(kind = kreal) :: sp1
      real(kind = kreal) :: Pws_l(nidx_rtm(2))
!
!
!$omp parallel do schedule(static)                                      &
!$omp&  private(ip,kst,ked,j_rlm,k_rlm,nd,i_rlm,ip_rtm,l_rtm,sp1,Pws_l)
      do ip = 1, np_smp
        kst = idx_rlm_smp_stack(ip-1,1) + 1
        ked = idx_rlm_smp_stack(ip,  1)
        do k_rlm = kst, ked
