...
!!     &          sph_ISPACK3, sph_domain_ispack3,                      &
!!     &          sph_comp_ispack3, sph_sgl_ispack3, flag_FFT)
!!      subroutine sel_finalize_sph_ISPACK3                             &
!!     &         (iflag_size, sph_ISPACK3, sph_domain_ispack3,          &
!!     &          sph_comp_ispack3, sph_sgl_ispack3, flag_FFT)
!!      subroutine sel_verify_sph_ISPACK3(iflag_size, sph_rtp, comm_rtp,&
!!     &          ncomp_bwd, ncomp_fwd, sph_ISPACK3, sph_domain_ispack3,&
!!     &          sph_comp_ispack3, sph_sgl_ispack3, flag_FFT)
...
!
      if     (iflag_size .eq. iflag_once_fft) then
        ...
        call finalize_sph_ISPACK3(sph_ISPACK3, flag_fft)
      else if(iflag_size .eq. iflag_domain_once) then
        if(iflag_debug .gt. 0) write(*,*)                               &
     &                       'Finalize ISPACK V3.0.1 for domain'
        call finalize_sph_domain_ISPACK3(sph_domain_ispack3, flag_fft)
      else if(iflag_size .eq. iflag_component_once) then
        if(iflag_debug .gt. 0) write(*,*)                               &
     &                       'Finalize ISPACK V3.0.1 for component'
        call finalize_sph_comp_ISPACK3(sph_comp_ispack3, flag_fft)
      else if(iflag_size .eq. iflag_single_fft) then
        if(iflag_debug .gt. 0) write(*,*)                               &
     &                       'Finalize single ISPACK V3.0.1'
        call finalize_sph_single_ISPACK3(sph_sgl_ispack3, flag_fft)
      end if
!
...
!
     ...
     &      ncomp_bwd, ncomp_fwd, sph_ISPACK3, flag_fft)
      else if(iflag_size .eq. iflag_domain_once) then
        if(iflag_debug .gt. 0) write(*,*)                               &
     &                       'Use ISPACK V3.0.1 for domain'
        call verify_sph_domain_ISPACK3                                  &
     &     (sph_rtp, comm_rtp, sph_domain_ispack3, flag_fft)
      else if(iflag_size .eq. iflag_component_once) then
        if(iflag_debug .gt. 0) write(*,*)                               &
     ...
     &     (sph_rtp, sph_sgl_ispack3, flag_fft)
      end if
!
