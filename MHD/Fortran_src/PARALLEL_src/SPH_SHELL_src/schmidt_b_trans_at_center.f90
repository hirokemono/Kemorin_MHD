...
!!
!!@verbatim
!!      subroutine schmidt_b_trans_center_scalar(nvector, nscalar)
...
!
      do nd = 3*nvector+1, 3*nvector+nscalar
        v_ct_local(nd) =  two * v_ct_local(nd)
      end do
!
...
!
      if(ist_rtm_order_zero.gt.0 .and. idx_gl_1d_rlm_r(1).eq.1) then
        do i_fld = 1, nvector
          ...
          v_ct_local(3*i_fld-2) =  two * WR(i_recv-2)                   &
...
!
      if(ist_rtm_order_1s.gt.0 .and. idx_gl_1d_rlm_r(1).eq.1) then
        do i_fld = 1, nvector
          ...
          v_ct_local(3*i_fld  ) = -two * WR(i_recv-2)                   &
     &                  * a_r_1d_rlm_r(1)*a_r_1d_rlm_r(1)
        end do
      end if
!
      if(ist_rtm_order_1c.gt.0 .and. idx_gl_1d_rlm_r(1).eq.1) then
        do i_fld = 1, nvector
          ...
          v_ct_local(3*i_fld-1) = -two * WR(i_recv-2)                   &
     &                  * a_r_1d_rlm_r(1)*a_r_1d_rlm_r(1)
        end do
      end if
!
