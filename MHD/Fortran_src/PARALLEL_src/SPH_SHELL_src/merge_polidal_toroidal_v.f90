!merge_polidal_toroidal_v.f90
!       module merge_polidal_toroidal_v
!
!      subroutine const_vect_sph_b_trans(nb, vr_rtp)
!      subroutine prod_r_vect_sph_f_trans(nb, vr_rtp)
!
!      subroutine const_grad_v_sph_b_trans(nb, vr_rtp)
!      subroutine prod_r_grad_v_sph_f_trans(nb, vr_rtp)
!
       module merge_polidal_toroidal_v
!
      use m_precision
...
!$omp end parallel do
!
      end subroutine const_grad_v_sph_b_trans
!
! -----------------------------------------------------------------------
!
      subroutine prod_r_grad_v_sph_f_trans(nb, vr_rtp)
...
!cdir nodep
        do i_rtp = ist, ied
          ...
          k_rtp = 1 + mod((inod-1),nidx_rtp(1))
!
          vr_rtp(3*i_rtp-1) = radius_1d_rtp_r(k_rtp)                    &
     &                       * vr_rtp(3*i_rtp-1)
          vr_rtp(3*i_rtp  ) = radius_1d_rtp_r(k_rtp)                    &
     &                       * vr_rtp(3*i_rtp  )
        end do
      end do
!$omp end parallel do
!
      end subroutine prod_r_grad_v_sph_f_trans
!
! -----------------------------------------------------------------------
!
