...
!
      type(legendre_trns_works), save :: WK1_leg
!WK1_leg%WK_spin
!
...
!
     ...
     &      idx_trns, nvector, nscalar, WK1_leg%WK_l_sml)
     ...
     &      idx_trns, nvector, nscalar, WK1_leg%WK_l_bsm)
     ...
     &      nvector, idx_trns, WK1_leg%WK_l_mtl)
        call alloc_leg_scl_matmul                                       &
     &     (sph_rtm%nidx_rtm(2), sph_rtm%maxidx_rtm_smp(1),             &
     &      nscalar, idx_trns, WK1_leg%WK_l_mtl)
     ...
     &     (sph_rtm, sph_rlm, leg, idx_trns, WK1_leg%WK_l_sml)
     ...
     &     (sph_rtm%nidx_rtm(2), idx_trns, WK1_leg%WK_l_mtl)
     ...
     &      idx_trns, nvector, nscalar, WK1_leg%WK_l_tst)
      else
        call alloc_work_sph_trans                                       &
     &     (ncomp, sph_rtm%nnod_rtm, sph_rlm%nnod_rlm, WK1_leg%WK_spin)
      end if
!
...
!
     ...
     &   .or. id_legendre_transfer .eq. iflag_leg_sym_spin_loop) then
        call finalize_legendre_sym_matmul(WK1_leg%WK_l_sml)
      else if(id_legendre_transfer .eq. iflag_leg_sym_matmul_big        &
     &   .or. id_legendre_transfer .eq. iflag_leg_sym_dgemm_big         &
     &   .or. id_legendre_transfer .eq. iflag_leg_sym_matprod_big) then
        call dealloc_leg_sym_matmul_big(WK1_leg%WK_l_bsm)
      else if(id_legendre_transfer .eq. iflag_leg_matmul                &
     &   .or. id_legendre_transfer .eq. iflag_leg_dgemm                 &
     &   .or. id_legendre_transfer .eq. iflag_leg_matprod) then
        call dealloc_leg_vec_matmul(WK1_leg%WK_l_mtl)
      else if(id_legendre_transfer .eq. iflag_leg_blocked               &
     &   .or. id_legendre_transfer .eq. iflag_leg_krloop_outer) then
        call dealloc_leg_vec_matmul(WK1_leg%WK_l_mtl)
      else if(id_legendre_transfer .eq. iflag_leg_test_loop) then
        call dealloc_leg_vec_test(WK1_leg%WK_l_tst)
      else
        call dealloc_work_sph_trans(WK1_leg%WK_spin)
      end if
!
...
!
     ...
     &      sph_rlm, sph_rtm, comm_rlm, comm_rtm, leg, idx_trns,        &
     &      n_WR, n_WS, WR, WS, WK1_leg%WK_l_bsm)
