...
!!      subroutine cal_sph_nod_gradient_2_w_src                         &
!!     &         (kr_in, kr_out, is_grad, nidx_rj, radius_1d_rj_r,      &
!!     &          g_sph_rj, d1nod_mat_fdm_2, n_point, ntot_phys_rj,     &
!!     &          d_rj_src, d_rj)
...
!
     ...
     &          d_rj_src, d_rj)
!
      integer(kind = kint), intent(in) :: kr_in, kr_out, is_grad
      integer(kind = kint), intent(in) :: nidx_rj(2)
      integer(kind = kint), intent(in) :: n_point, ntot_phys_rj
      real(kind = kreal), intent(in) :: radius_1d_rj_r(nidx_rj(1))
      real(kind = kreal), intent(in) :: g_sph_rj(nidx_rj(2),13)
      real(kind = kreal), intent(in)                                    &
     &                   :: d1nod_mat_fdm_2(nidx_rj(1),-1:1)
      real(kind = kreal), intent(in) :: d_rj_src(n_point)
      real(kind = kreal), intent(inout) :: d_rj(n_point,ntot_phys_rj)
!
      integer(kind = kint) :: inod, i_p1, i_n1, j, k
      integer(kind = kint) :: ist, ied
...
!$omp do private(inod)
      do inod = ist, ied
        d_rj(inod,is_grad+1) = d_rj_src(inod)
        d_rj(inod,is_grad+2) = zero
      end do
!$omp end do
!
!$omp do private(inod,i_p1,i_n1,j,k,d1sdr)
      do inod = ist, ied
        ...
        k = 1 + (inod-j) / nidx_rj(2)
!
        d1sdr =  d1nod_mat_fdm_2(k,-1) * d_rj_src(i_n1)                &
     &         + d1nod_mat_fdm_2(k, 0) * d_rj_src(inod)                &
     &         + d1nod_mat_fdm_2(k, 1) * d_rj_src(i_p1)
!
        d_rj(inod,is_grad) = d1sdr * g_sph_rj(j,13)                    &
     &                       * radius_1d_rj_r(k)**2
      end do
...
!$omp end parallel
!
      end subroutine cal_sph_nod_gradient_2_w_src
!
! -----------------------------------------------------------------------
!
      subroutine normalize_sph_average_grad                             &
