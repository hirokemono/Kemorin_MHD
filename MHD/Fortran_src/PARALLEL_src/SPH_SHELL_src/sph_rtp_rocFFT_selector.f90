!>@file   sph_rtp_rocFFT_selector.F90
!!@brief  module sph_rtp_rocFFT_selector
!!
!!@author H. Matsui
...
!! ------------------------------------------------------------------
!!
!!      subroutine sel_rtp_bwd_rocFFT_from_recv                         &
!!     &         (iflag_sph_FFT, iflag_size, sph_rtp, comm_rtp,         &
...
!
      module sph_rtp_rocFFT_selector
!
      use m_precision
...
!
      end subroutine sel_init_rtp_rocFFT
!
! ------------------------------------------------------------------
! ------------------------------------------------------------------
!
...
!
      end subroutine sel_verify_rtp_rocFFT
!
! ------------------------------------------------------------------
! ------------------------------------------------------------------
!
...
!
      if     (iflag_size .eq. iflag_once_fft) then
        if     (iflag_sph_FFT .eq. iflag_real_rocFFT) then
          call rtp_fwd_real_rocFFT_to_send(sph_rtp,                     &
     ...
     &        sph_rocFFT%WK_rocFFT, flag_FFT)
          ...
          call rtp_fwd_OMP_rocFFT_to_send(sph_rtp,                      &
     &        sph_rocFFT%comm_sph_rocFFT, sph_rocFFT%rocFFT_fwd,        &
     &        ncomp_fwd, n_WS, v_rtp(1,1), WS(1),                       &
     &        sph_rocFFT%WK_rocFFT, flag_FFT)
        else if(iflag_sph_FFT .eq. iflag_rocFFT) then
          call rtp_fwd_cplx_rocFFT_to_send(sph_rtp,                     &
     &        sph_rocFFT%comm_sph_rocFFT, sph_rocFFT%rocFFT_fwd,        &
     &        ncomp_fwd, n_WS, v_rtp(1,1), WS(1),                       &
     &        sph_rocFFT%WK_rocFFT, flag_FFT)
        end if
!
        ...
        if     (iflag_sph_FFT .eq. iflag_real_rocFFT) then
          call rtp_dmn_fwd_real_rocFFT_to_send(sph_rtp,                 &
     &        sph_rocFFT%comm_sph_rocFFT, sph_rocFFT%rocFFT_fwd,        &
     &        ncomp_fwd, n_WS, v_rtp(1,1), WS(1),                       &
     &        sph_rocFFT%WK_rocFFT, flag_FFT)
        else if(iflag_sph_FFT .eq. iflag_OMP_rocFFT) then
          call rtp_dmn_fwd_OMP_rocFFT_to_send(sph_rtp,                  &
     &        sph_rocFFT%comm_sph_rocFFT, sph_rocFFT%rocFFT_fwd,        &
     &        ncomp_fwd, n_WS, v_rtp(1,1), WS(1),                       &
     &        sph_rocFFT%WK_rocFFT, flag_FFT)
        else if(iflag_sph_FFT .eq. iflag_rocFFT) then
          call rtp_dmn_fwd_cplx_rocFFT_to_send(sph_rtp,                 &
     &        sph_rocFFT%comm_sph_rocFFT, sph_rocFFT%rocFFT_fwd,        &
     &        ncomp_fwd, n_WS, v_rtp(1,1), WS(1),                       &
     &        sph_rocFFT%WK_rocFFT, flag_FFT)
        end if
...
!
      if     (iflag_size .eq. iflag_once_fft) then
        if     (iflag_sph_FFT .eq. iflag_real_rocFFT) then
          call rtp_bwd_real_rocFFT_from_recv                            &
     &       (sph_rtp, comm_rtp, sph_rocFFT%rocFFT_bwd, ncomp_bwd,      &
     &        n_WR, WR(1), v_rtp(1,1), sph_rocFFT%WK_rocFFT, flag_FFT)
        else if(iflag_sph_FFT .eq. iflag_OMP_rocFFT) then
          call rtp_bwd_OMP_rocFFT_from_recv                             &
     &       (sph_rtp, comm_rtp, sph_rocFFT%rocFFT_bwd, ncomp_bwd,      &
     &        n_WR, WR(1), v_rtp(1,1), sph_rocFFT%WK_rocFFT, flag_FFT)
        else if(iflag_sph_FFT .eq. iflag_rocFFT) then
          call rtp_bwd_cplx_rocFFT_from_recv                            &
     &       (sph_rtp, comm_rtp, sph_rocFFT%rocFFT_bwd, ncomp_bwd,      &
     &        n_WR, WR(1), v_rtp(1,1), sph_rocFFT%WK_rocFFT, flag_FFT)
        end if
!
        ...
        if     (iflag_sph_FFT .eq. iflag_real_rocFFT) then
          call rtp_dmn_bwd_real_rocFFT_fm_recv                          &
     &       (sph_rtp, comm_rtp, sph_rocFFT%rocFFT_bwd, ncomp_bwd,      &
     &        n_WR, WR(1), v_rtp(1,1), sph_rocFFT%WK_rocFFT, flag_FFT)
        else if(iflag_sph_FFT .eq. iflag_OMP_rocFFT) then
          call rtp_dmn_bwd_OMP_rocFFT_fm_recv                           &
     &       (sph_rtp, comm_rtp, sph_rocFFT%rocFFT_bwd, ncomp_bwd,      &
     &        n_WR, WR(1), v_rtp(1,1), sph_rocFFT%WK_rocFFT, flag_FFT)
        else if(iflag_sph_FFT .eq. iflag_rocFFT) then
          call rtp_dmn_bwd_c_rocFFT_from_recv                           &
     &       (sph_rtp, comm_rtp, sph_rocFFT%rocFFT_bwd, ncomp_bwd,      &
     &        n_WR, WR(1), v_rtp(1,1), sph_rocFFT%WK_rocFFT, flag_FFT)
        end if
      end if
!
      end subroutine sel_rtp_bwd_rocFFT_from_recv
!
! ------------------------------------------------------------------
!
      end module sph_rtp_rocFFT_selector
