...
!$omp&            private(ip,kst,ked,lp,lst,led,j_rlm,kr_nd,l_rtm,k_rlm,&
!$omp&                    mp_rlm,mn_rlm,r2_1d_rlm_r,sp1,sp2,sp3,        &
...
!
     ...
     &                     * radius_1d_rlm_r(k_rlm)
              ...
              sp3 = 0.0d0
              do l_rtm = lst, led
                sp1 = sp1 + vr_rtm_spin(l_rtm,kr_nd,         mp_rlm)    &
     &                     * Pvw_l(l_rtm)
!
                sp2 = sp2 + (vr_rtm_spin(l_rtm,kr_nd+nb_nri,  mp_rlm)   &
     &                      * dPvw_l(l_rtm)                             &
     &                     - vr_rtm_spin(l_rtm,kr_nd+2*nb_nri,mn_rlm)   &
     &                      * Pgvw_l(l_rtm))
!
                sp3 = sp3 - (vr_rtm_spin(l_rtm,kr_nd+nb_nri,  mn_rlm)   &
     &                      * Pgvw_l(l_rtm)                             &
...
!
      ...
      integer(kind = kint) :: kr_nd, kst, ked, ip, lp, lst, led
      real(kind = kreal) :: sp1
      real(kind = kreal) :: Pws_l(nidx_rtm(2))
!
!
!$omp parallel do schedule(static)                                      &
!$omp&         private(ip,kst,ked,lp,lst,led,j_rlm,kr_nd,l_rtm,mp_rlm,  &
!$omp&                 sp1,Pws_l)
      do ip = 1, np_smp
        kst = 3*nvector*nidx_rtm(1)                                     &
     &       + nscalar*idx_rlm_smp_stack(ip-1,1) + 1
        ked = 3*nvector*nidx_rtm(1)                                     &
     &       + nscalar*idx_rlm_smp_stack(ip,  1)
        do kr_nd = kst, ked
          do j_rlm = 1, nidx_rlm(2)
            sp_rlm_spin(kr_nd,j_rlm) = 0.0d0
          end do
...
!
     ...
     &                     * Pws_l(l_rtm)
              ...
              sp_rlm_spin(kr_nd,j_rlm) = sp_rlm_spin(kr_nd,j_rlm) + sp1
            end do
          end do
        end do
      end do
