...
!
      ...
      integer(kind = kint), intent(in) :: ntot_X
      real(kind = kreal), intent(in) :: X_FFT(ntot_X,np_smp)
!
      integer(kind = kint), intent(in) :: n_WS
      integer(kind = kint), intent(in) :: irev_sr_rtp(nnod_rtp)
      real (kind=kreal), intent(inout):: WS(n_WS)
...
!$omp&                    ic_rtp,is_rtp,ic_send,is_send)
        do ip = 1, np_smp
          ...
          do j = 1, num
            ...
            ic_send = nd + (irev_sr_rtp(ic_rtp) - 1) * ncomp_fwd
            is_send = nd + (irev_sr_rtp(is_rtp) - 1) * ncomp_fwd
            WS(ic_send) = X_FFT(j,ip)
            WS(is_send) = X_FFT(j+num,ip)
          end do
          do m = 2, nphi_rtp/2
            do j = 1, num
              ic_rtp = j+ist + (2*m-2) * irt_rtp_smp_stack(np_smp)
              ...
              inod_c = j + (2*m-2) * num
              inod_s = j + (2*m-1) * num
              WS(ic_send) =   two * X_FFT(inod_c,ip)
              WS(is_send) = - two * X_FFT(inod_s,ip)
            end do
          end do
        end do
!$omp end parallel do
...
!
      integer(kind = kint), intent(in) :: ntot_X
      real(kind = kreal), intent(inout) :: X_FFT(ntot_X,np_smp)
!
      integer(kind = kint) :: m, j, ip, ist, num
      integer(kind = kint) :: inod_s, inod_c
      integer(kind = kint) :: ic_rtp, is_rtp, ic_recv, is_recv
...
!
        do j = 1, num
          ...
          X_FFT(j,ip) = WR(ic_recv)
          X_FFT(j+num,ip) = WR(is_recv)
        end do
        do m = 2, nphi_rtp/2
          do j = 1, num
            inod_c = j + (2*m-2) * num
            ...
            ic_recv = nd + (irev_sr_rtp(ic_rtp) - 1) * ncomp_bwd
            is_recv = nd + (irev_sr_rtp(is_rtp) - 1) * ncomp_bwd
            X_FFT(inod_c,ip) =  half * WR(ic_recv)
            X_FFT(inod_s,ip) = -half * WR(is_recv)
          end do
        end do
      end do
!$omp end parallel do
