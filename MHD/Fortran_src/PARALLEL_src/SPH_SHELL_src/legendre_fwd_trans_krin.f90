...
!
      ...
      integer(kind = kint) :: nb_nri, k_rtm, nd, mst, med
      real(kind = kreal) :: pwt_tmp, dpwt_tmp, pgwt_tmp
!
!
      nb_nri = nvector*nidx_rtm(1)
!$omp parallel do private(j_rlm,l_rtm,ip_rtm_1,in_rtm_1,mst,med,        &
...
!
              do k_rtm = 1, nidx_rtm(1)
                sp_rlm_krin(k_rtm,j_rlm,3*nd-2)                         &
     &                  = sp_rlm_krin(k_rtm,j_rlm,3*nd-2)               &
     &                 + vr_rtm_krin(k_rtm,ip_rtm_1,3*nd-2) * pwt_tmp
                sp_rlm_krin(k_rtm,j_rlm,3*nd-1)                         &
     &                  = sp_rlm_krin(k_rtm,j_rlm,3*nd-1)               &
     &               + ( vr_rtm_krin(k_rtm,ip_rtm_1,3*nd-1) * dpwt_tmp  &
     &                 - vr_rtm_krin(k_rtm,in_rtm_1,3*nd  ) * pgwt_tmp)
                sp_rlm_krin(k_rtm,j_rlm,3*nd  )                         &
     &                  = sp_rlm_krin(k_rtm,j_rlm,3*nd  )               &
     &               - ( vr_rtm_krin(k_rtm,in_rtm_1,3*nd-1) * pgwt_tmp  &
     &                 + vr_rtm_krin(k_rtm,ip_rtm_1,3*nd  ) * dpwt_tmp)
              end do
            end do
          end do
!
          do j_rlm = mst, med
            do k_rtm = 1, nidx_rtm(1)
              sp_rlm_krin(k_rtm,j_rlm,3*nd-2)                           &
     &           = sp_rlm_krin(k_rtm,j_rlm,3*nd-2) * g_sph_rlm(j_rlm,7)
              sp_rlm_krin(k_rtm,j_rlm,3*nd-1)                           &
     &           = sp_rlm_krin(k_rtm,j_rlm,3*nd-1) * g_sph_rlm(j_rlm,7)
              sp_rlm_krin(k_rtm,j_rlm,3*nd  )                           &
     &           = sp_rlm_krin(k_rtm,j_rlm,3*nd  ) * g_sph_rlm(j_rlm,7)
            end do
          end do
        end do
      end do
...
!$omp parallel do private(j_rlm,k_rtm,nd,ip_rtm_1,mst,med,l_rtm,pwt_tmp)
      do mp_rlm = 1, nidx_rtm(3)
        mst = lstack_rlm(mp_rlm-1)+1
        med = lstack_rlm(mp_rlm)
        do nd = 1, nscalar
...
!
              do k_rtm = 1, nidx_rtm(1)
                sp_rlm_krin(k_rtm,j_rlm,nd)                             &
     &                   = sp_rlm_krin(k_rtm,j_rlm,nd)                  &
     &                    + vr_rtm_krin(k_rtm,ip_rtm_1,nd) * pwt_tmp
              end do
            end do
          end do
!
