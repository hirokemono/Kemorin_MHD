...
!!
!!@verbatim
!!      subroutine cal_sv_rlm_sym_matmul_big_smp(nnod_rlm, nidx_rlm,    &
...
!
      nkrv = nkr * nvector
!$omp  parallel do collapse(2) private(jj,kk,g7,gm,kr_nd,k_rlm,r1,r2)
      do jj = 1, n_jk_e
        do kk = 1, nkrv
...
!$omp& private(jj,kk,kr_nd,k_rlm,nd,ie_rlm,io_rlm,ie_send,io_send)
      do jj = 1, n_jk_o
        do kk = 1, nkrv
          kr_nd = kk + nvector*kst
          k_rlm = 1 + mod((kr_nd-1),nidx_rlm(1))
...
!
!$omp  parallel do collapse(2)                                          &
!$omp& private(jj,kk,kr_nd,k_rlm,nd,ie_rlm,ie_send)
...
!
!$omp  parallel do collapse(2)                                          &
!$omp& private(kk,jj,kr_nd,k_rlm,nd,ie_rlm,io_rlm,ie_send,io_send)
      do kk = 1, nkr*nscalar
        do jj = 1, n_jk_o
...
!
!$omp  parallel do collapse(2)                                          &
!$omp& private(kk,jj,kr_nd,k_rlm,nd,ie_rlm,ie_send)
      do kk = 1, nkr*nscalar
        do jj = n_jk_o+1, n_jk_e
          kr_nd = kk + kst*nscalar
          k_rlm = 1 + mod((kr_nd-1),nidx_rlm(1))
