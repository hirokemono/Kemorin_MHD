...
!
              symp_r(i_kl,ip) =  vr_rtm(ip_rtm-2)
!
              asmp_t(i_kl,ip) =  vr_rtm(ip_rtm-1)
              asmp_p(i_kl,ip) =  vr_rtm(ip_rtm  )
!
              symn_t(i_kl,ip) =  vr_rtm(in_rtm-1)
...
!$omp&                    ll,l_rtm,i_kl,i_lj,i_kj,i_rlm,st_elapsed)     &
!$omp& reduction(+:elaps)
      do ip = 1, np_smp
