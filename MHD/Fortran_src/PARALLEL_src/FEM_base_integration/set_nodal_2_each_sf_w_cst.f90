!set_nodal_2_each_sf_w_cst.f90
!      module set_nodal_2_each_sf_w_cst
!
!      Written by H. Matsui on Sep., 2005
!
!      subroutine scalar_phys_2_each_sf_w_const(np_smp, numnod, numele, &
!     &          nnod_4_ele, ie, nnod_4_surf, node_on_sf, num_surf_bc,  &
!     &          surf_item, num_surf_smp, isurf_grp_smp_stack, igrp,    &
!     &          k2, i_scalar, ntot_comp, d_nod, coef, scalar_sf)
!        Input:  d_nod(1,i_scalar), Output:  scalar_sf
!      subroutine vector_phys_2_each_sf_w_const(np_smp, numnod, numele, &
!     &          nnod_4_ele, ie, nnod_4_surf, node_on_sf, num_surf_bc,  &
!     &          surf_item, num_surf_smp, isurf_grp_smp_stack, igrp,    &
!     &          k2, i_vector, ntot_comp, d_nod, coef, vector_sf)
!        Input:  d_nod(1,i_vector), Output:  vector_sf
!      subroutine tensor_phys_2_each_sf_w_const(np_smp, numnod, numele, &
!     &          nnod_4_ele, ie, nnod_4_surf, node_on_sf, num_surf_bc,  &
!     &          surf_item, num_surf_smp, isurf_grp_smp_stack, igrp,    &
!     &          k2, i_tensor, ntot_comp, d_nod, coef, tensor_sf)
...
!voption, indep, vec
        do inum = ist, ied
          ...
          inod =   ie(iele,kk2)
!
          scalar_sf(inum) = coef * d_nod(inod,i_scalar  )
        end do
      end do
!$omp end parallel do
!
      end subroutine scalar_phys_2_each_sf_w_const
!
! ----------------------------------------------------------------------
!
      subroutine vector_phys_2_each_sf_w_const(np_smp, numnod, numele,  &
...
!voption, indep, vec
        do inum = ist, ied
          ...
          inod =   ie(iele,kk2)
!
          vector_sf(inum,1) = coef * d_nod(inod,i_vector  )
          vector_sf(inum,2) = coef * d_nod(inod,i_vector+1)
          vector_sf(inum,3) = coef * d_nod(inod,i_vector+2)
        end do
      end do
!$omp end parallel do
!
      end subroutine vector_phys_2_each_sf_w_const
!
! ----------------------------------------------------------------------
!
      subroutine tensor_phys_2_each_sf_w_const(np_smp, numnod, numele,  &
...
!voption, indep, vec
        do inum = ist, ied
          ...
          inod =   ie(iele,kk2)
!
          tensor_sf(inum,1) = coef * d_nod(inod,i_tensor  )
          tensor_sf(inum,2) = coef * d_nod(inod,i_tensor+1)
          tensor_sf(inum,3) = coef * d_nod(inod,i_tensor+2)
          tensor_sf(inum,4) = coef * d_nod(inod,i_tensor+3)
          tensor_sf(inum,5) = coef * d_nod(inod,i_tensor+4)
          tensor_sf(inum,6) = coef * d_nod(inod,i_tensor+5)
        end do
      end do
!$omp end parallel do
!
      end subroutine tensor_phys_2_each_sf_w_const
!
! ----------------------------------------------------------------------
!
      end module set_nodal_2_each_sf_w_cst
