...
!
      type marks_for_sleeve_extend
        ...
        type(mark_for_each_comm), allocatable :: mark_nod_checked(:)
...
!
      ...
      allocate(marks_4_extend%mark_nod_checked(nprocs))
      allocate(marks_4_extend%mark_ele(nprocs))
!
!$omp parallel do
      do ip = 1, nprocs
        marks_4_extend%mark_nod_done(ip)%num_marked =    -1
        marks_4_extend%mark_nod_checked(ip)%num_marked = -1
...
!
      ...
      deallocate(marks_4_extend%mark_nod_checked)
...
!$omp parallel do
     ...
     &       (izero, marks_4_extend%mark_nod_done(ip))
        ...
        if(marks_4_extend%mark_nod_checked(ip)%num_marked .lt. 0) then
          call alloc_mark_for_each_comm                                 &
     &       (izero, marks_4_extend%mark_nod_checked(ip))
        end if
