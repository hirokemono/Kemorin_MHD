...
!!
!!      subroutine append_merged_import_distance                        &
!!     &         (org_comm, add_comm, dist_nod_import, dist_add_import, &
!!     &          nprocs, ineib_org, ineib_add,                         &
!!     &          num_new_neib, id_new_neib, istack_new_import,         &
!!     &          ntot_new_import, dist_new_import)
!!      subroutine append_merged_export_distance                        &
!!     &         (org_comm, add_comm, dist_nod_export, dist_add_export, &
!!     &          nprocs, ineib_org, ineib_add,                         &
!!     &          num_new_neib, id_new_neib, istack_new_export,         &
!!     &          ntot_new_export, dist_new_export)
!!        type(communication_table), intent(in) :: org_comm
!!        type(calypso_comm_table), intent(in) ::  add_comm
!!@endverbatim
!
...
!
     ...
     &                      :: dist_nod_import(org_comm%ntot_import)
      real(kind = kreal), intent(in)                                    &
     &                      :: dist_add_import(add_comm%ntot_import)
!
      integer, intent(in) :: nprocs
      integer(kind = kint), intent(in) :: ineib_org(nprocs)
      integer(kind = kint), intent(in) :: ineib_add(nprocs)
...
!
     ...
     &                      :: dist_new_import(ntot_new_import)
!
      integer :: i, irank, ineib, jneib, jst, ist, num, inum
!
!
!$omp parallel do private(i,irank,ineib,jneib,jst,ist,num,inum)
      do i = 1, num_new_neib
        ...
        if(ineib .gt. 0) then
          ...
          num = org_comm%istack_import(ineib) - ist
          do inum = 1, num
            dist_new_import(inum+jst) = dist_nod_import(inum+ist)
          end do
          jst = jst + num
        end if
        if(jneib .gt. 0) then
          ...
          num = add_comm%istack_import(jneib) - ist
          do inum = 1, num
            dist_new_import(inum+jst) = dist_add_import(inum+ist)
          end do
        end if
      end do
!$omend p parallel do
!
      end subroutine append_merged_import_distance
!
! ----------------------------------------------------------------------
!
      subroutine append_merged_export_distance                          &
...
!
      ...
      type(calypso_comm_table), intent(in) :: add_comm
      real(kind = kreal), intent(in)                                    &
     &                      :: dist_nod_export(org_comm%ntot_export)
      real(kind = kreal), intent(in)                                    &
     &                      :: dist_add_export(add_comm%ntot_export)
!
      integer, intent(in) :: nprocs
      integer(kind = kint), intent(in) :: ineib_org(nprocs)
      integer(kind = kint), intent(in) :: ineib_add(nprocs)
...
!
      real(kind = kreal), intent(inout)                                 &
     &                      :: dist_new_export(ntot_new_export)
!
      integer :: i, irank, ineib, jneib, jst, ist, num, inum
!
!
!$omp parallel do private(i,irank,ineib,jneib,jst,ist,num,inum)
      do i = 1, num_new_neib
        ...
        if(ineib .gt. 0) then
          ...
          num = org_comm%istack_export(ineib) - ist
          do inum = 1, num
            dist_new_export(inum+jst) = dist_nod_export(inum+ist)
          end do
          jst = jst + num
        end if
        if(jneib .gt. 0) then
          ...
          num = add_comm%istack_export(jneib) - ist
          do inum = 1, num
            dist_new_export(inum+jst) = dist_add_export(inum+ist)
          end do
        end if
      end do
!$omend p parallel do
!
      end subroutine append_merged_export_distance
!
! ----------------------------------------------------------------------
!
