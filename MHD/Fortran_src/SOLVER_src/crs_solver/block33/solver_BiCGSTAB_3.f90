...
!
      ...
      integer(kind=kint ) :: I, K, L, IP, J, isL, isU, ieL, ieU, in
      real   (kind=kreal) :: TOL, BETA, OMEGA
      real   (kind=kreal), dimension(3)                    :: PW
      data IFLAG/0/
!
      real(kind=kreal) :: X1, X2, X3, W1, W2, W3
      real(kind=kreal) :: SW1, SW2, SW3
      real(kind=kreal) :: ALPHA, ALO
!
!C
!C-- INIT.
      ERROR= 0
...
!C===
      ...
      do j= 1, N
        ...
        DNRM20(1)= DNRM20(1)+WW(3*j-2,S)**2+WW(3*j-1,S)**2+WW(3*j,S)**2
      enddo
