!>@file   interpolate_rev_SR_3.f90
!!@brief  module interpolate_rev_SR_3
!!
!!@author H. Matsui
...
!!
!!@n @param  X_new(3*nnod_new)   Send data
!!@n @param  X_org(3*nnod_org)   Received data
!
...
!
      real   (kind=kreal), intent(in):: X_new(ithree*nnod_new)
      real   (kind=kreal), intent(inout):: X_org(ithree*nnod_org)
!
...
!
      do neib = 1, npe_recv
        ...
        do k= istart, iend
          j = inod_import(k)
          WR(ithree*k-2) = X_new(ithree*j-2)
          WR(ithree*k-1) = X_new(ithree*j-1)
          WR(ithree*k  ) = X_new(ithree*j  )
        end do
      end do
