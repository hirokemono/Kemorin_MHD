...
!
      real   (kind=kreal), intent(in):: X_org(ithree*numnod)
!       interpolated result vector
      real   (kind=kreal), intent(inout):: X(ithree*nnod_2nd)
!       interpolated result vector
!
...
!
      ...
      do neib = 1, npe_recv
        ...
        do k= istart, iend
          j = inod_import(k)
          X(ithree*j-2) = WR(ithree*k-2)
          X(ithree*j-1) = WR(ithree*k-1)
          X(ithree*j  ) = WR(ithree*k  )
        end do
      end do
