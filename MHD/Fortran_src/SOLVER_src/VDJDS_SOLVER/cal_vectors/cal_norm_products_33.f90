...
!     Written by Kemorin
!
!       subroutine cal_local_norm_3(NP, PEsmpTOT, STACKmcG, B,          &
!     &           BNRM, BNRM_smp)
!           BNRM = B^2
...
!!           BNRM = WN^2
!!           SP = WN \cdot WP
!!       subroutine cal_local_sproduct_and_norm_3(NP, PEsmpTOT, STACKmcG,&
!!     &           WN, WP1, WP2, SP, BNRM, SP_smp, BNRM_smp)
!!           BNRM = WN^2
!!           SP = WP1 \cdot WP2
!
...
!voption indep (B)
        do i= iS, iE
          BNRM_smp(ip)= BNRM_smp(ip)                                    &
     &                  + B(3*i-2)**2 + B(3*i-1)**2 + B(3*i  )**2
        enddo
      enddo
!$omp end parallel do
...
!cdir parallel do private(iS,iE,i)
!$omp parallel do private(iS,iE,i)
!poption indep (W,WT,STACKmcG) tlocal (iS,iE,i)
      do ip= 1, PEsmpTOT
        iS= STACKmcG(ip-1) + 1
        iE= STACKmcG(ip  )
!OCL VECTOR, NOVREC
...
!voption indep (W,WT)
        do i= iS, iE
          SP_smp(ip)= SP_smp(ip) + W(3*i-2)*WT(3*i-2)                   &
     &                           + W(3*i-1)*WT(3*i-1)                   &
     &                           + W(3*i  )*WT(3*i  )
        enddo
      enddo
...
!cdir parallel do private(iS,iE,i)
!$omp parallel do private(iS,iE,i)
!poption indep (WN,WP,STACKmcG) tlocal (iS,iE,i)
      do ip= 1, PEsmpTOT
        iS= STACKmcG(ip-1) + 1
        iE= STACKmcG(ip  )
!OCL VECTOR, NOVREC
...
!voption indep (WN,WP)
        do i= iS, iE
          SP_smp(ip)  = SP_smp(ip)   + WN(3*i-2) * WP(3*i-2)            &
     &                               + WN(3*i-1) * WP(3*i-1)            &
     &                               + WN(3*i  ) * WP(3*i  )
          BNRM_smp(ip)= BNRM_smp(ip) + WN(3*i-2) * WN(3*i-2)            &
     &                               + WN(3*i-1) * WN(3*i-1)            &
     &                               + WN(3*i  ) * WN(3*i  )
        enddo
      enddo
...
!cdir parallel do private(iS,iE,i)
!$omp parallel do private(iS,iE,i)
!poption indep (WN,WP1,WP2,STACKmcG) tlocal (iS,iE,i)
      do ip= 1, PEsmpTOT
        iS= STACKmcG(ip-1) + 1
        iE= STACKmcG(ip  )
!OCL VECTOR, NOVREC
...
!voption indep (WN,WP1,WP2)
        do i= iS, iE
          SP_smp(ip)  = SP_smp(ip)   + WP1(3*i-2) * WP2(3*i-2)          &
     &                               + WP1(3*i-1) * WP2(3*i-1)          &
     &                               + WP1(3*i  ) * WP2(3*i  )
          BNRM_smp(ip)= BNRM_smp(ip) + WN(3*i-2)  * WN(3*i-2)           &
     &                               + WN(3*i-1)  * WN(3*i-1)           &
...
!cdir parallel do private(iS,iE,i)
!$omp parallel do private(iS,iE,i)
!poption indep (WY,WT,WTT,STACKmcG) tlocal (iS,iE,i)
      do ip= 1, PEsmpTOT
        iS= STACKmcG(ip-1) + 1
        iE= STACKmcG(ip  )
!OCL VECTOR, NOVREC
...
!voption indep (WY,WT,WTT)
        do i= iS, iE
          C0_smp(ip,1) = C0_smp(ip,1) + WY(3*i-2) *  WY(3*i-2)          &
     &                                + WY(3*i-1) *  WY(3*i-1)          &
     &                                + WY(3*i  ) *  WY(3*i  )
          C0_smp(ip,2) = C0_smp(ip,2) + WTT(3*i-2) * WT(3*i-2)          &
     &                                + WTT(3*i-1) * WT(3*i-1)          &
     &                                + WTT(3*i  ) * WT(3*i  )
          C0_smp(ip,3) = C0_smp(ip,3) + WY(3*i-2) *  WT(3*i-2)          &
     &                                + WY(3*i-1) *  WT(3*i-1)          &
     &                                + WY(3*i  ) *  WT(3*i  )
          C0_smp(ip,4) = C0_smp(ip,4) + WTT(3*i-2) * WY(3*i-2)          &
     &                                + WTT(3*i-1) * WY(3*i-1)          &
     &                                + WTT(3*i  ) * WY(3*i  )
          C0_smp(ip,5) = C0_smp(ip,5) + WTT(3*i-2) * WTT(3*i-2)         &
     &                                + WTT(3*i-1) * WTT(3*i-1)         &
     &                                + WTT(3*i  ) * WTT(3*i  )
        enddo
      enddo
