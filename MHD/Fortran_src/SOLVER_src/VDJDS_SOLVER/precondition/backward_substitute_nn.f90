!
!      module backward_substitute_nn
!
!     Written by Kemorin
!
!       subroutine backward_substitute_1xnn(N, NP, NB, NU, NPU,         &
!     &           PEsmpTOT, NVECT, npUX1, STACKmc, NUhyp, INU, IAU,     &
!     &           S, WP, AU, ALU_U)
!
!       subroutine backward_substitute_3xnn(N, NP, NB, NU, NPU,         &
!     &           PEsmpTOT, NVECT, npUX1, STACKmc, NUhyp, INU, IAU,     &
...
!
       ...
       integer (kind = kint) :: k, kk, k1, k2, im, ix, ii
!
!
      iv= NVECT
!cdir parallel do private(iS,iE,i,k1,k2,ii,ix,im)
!$omp parallel do private(iS,iE,i,k1,k2,ii,ix,im)
!poption indep (WP,S,ALU_U,STACKmc) tlocal (iS,iE,i,k1,k2,ii,ix,im)
      do ip= 1, PEsmpTOT
...
!poption noparallel      
      do iv= NVECT-1, 1, -1
!cdir parallel do private(iv0,iS,iE,i,j,k,k1,k2,ii,ix,im)
!$omp parallel do private(iv0,iS,iE,i,j,k,k1,k2,ii,ix,im)
...
!$omp end parallel do
      ...
      end subroutine backward_substitute_1xnn
!
!  ---------------------------------------------------------------------
!
       subroutine backward_substitute_3xnn(N, NP, NB, NU, NPU,          &
...
!voption indep (WP1,WP2,WP3,ALU_U)
            do i= iS, iE
              ...
              ix =    NB*(i-1)             + k2
              im = NB*NB*(i-1) + NB*(k1-1) + k2
              WP1(ii) = WP1(ii) - ALU_U(im)*WP1(ix)
              WP2(ii) = WP2(ii) - ALU_U(im)*WP2(ix)
              WP3(ii) = WP3(ii) - ALU_U(im)*WP3(ix)
            end do
          end do
        end do
...
!$omp end parallel do
      ...
      end subroutine backward_substitute_3xnn
!
!  ---------------------------------------------------------------------
!
      end module backward_substitute_nn
