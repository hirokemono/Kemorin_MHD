...
!C
!      subroutine init_VCG33_DJDS_SMP_d(NP, PEsmpTOT, PRECOND)
...
!
     ...
     &    PRECOND(1:3).eq.'ILU' .or. PRECOND(1:4).eq.'SSOR') then
        if (iterPREmax .eq. 1) then
          call verify_work_4_I_Cholesky33d(NP)
        else
          call verify_work_4_IC_asdd33d(NP)
        end if
      end if
...
!      call SOLVER_SEND_RECV_3                                           &
!     &   ( NP, NEIBPETOT, NEIBPE, STACK_IMPORT, NOD_IMPORT,             &
!     &     STACK_EXPORT, NOD_EXPORT, W(1,1) , SOLVER_COMM, my_rank)

!      call SOLVER_SEND_RECV_3                                           &
!     &   ( NP, NEIBPETOT, NEIBPE, STACK_IMPORT, NOD_IMPORT,             &
!     &     STACK_EXPORT, NOD_EXPORT, W(1,2) , SOLVER_COMM, my_rank)

!      call SOLVER_SEND_RECV_3                                           &
!     &   ( NP, NEIBPETOT, NEIBPE, STACK_IMPORT, NOD_IMPORT,             &
!     &     STACK_EXPORT, NOD_EXPORT, W(1,3) , SOLVER_COMM, my_rank)

!C
!C== change B,X

      call back_2_original_order_bx3d(NP, NtoO, B, X)
