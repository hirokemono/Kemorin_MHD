...
!!
!!@author H. Matsui
!!@date    Programmed by H.Matsui in Oct., 2005
!!
!>@brief  FEM integration for fixed boundaries
!!
!!@verbatim
...
!!
!!      subroutine int_sk_4_fixed_vector(iflag_commute, ifilter_final,  &
!!     &          num_int, i_field, node, ele, nod_fld, g_FEM, jac_3d,  &
!!     &          rhs_tbl, FEM_elens, Cdiff, nod_bc, ak_d, coef_imp,    &
!!     &          fem_wk, f_l)
...
!!        type(scaler_fixed_nod_bc_type), intent(in) :: nod_bc
...
!
     ...
     &    node, ele, nod_fld, g_FEM, jac_3d, rhs_tbl, FEM_elens, Cdiff, &
     &    nod_bc_v, ak_d, fl_prop%coef_imp, fem_wk, f_l)
!
      if (iflag_commute_velo .eq. id_SGS_commute_ON) then
     ...
     &      ifilter_final, i_velo, Cdiff%coef(1,1), ak_d,               &
...
!
     ...
     &      i_field, Cdiff%coef(1,1), ak_d, coef_imp, fem_wk, f_l)
      else
