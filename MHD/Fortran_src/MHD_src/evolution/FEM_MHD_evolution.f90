...
!
     ...
     &     SGS_MHD_wk%fem_int, icomp_sgs, ifld_diff, iphys_elediff,     &
...
!     &      my_rank, sf_grp, fem%group%surf_nod_grp)
     ...
     &     SGS_MHD_wk%iphys_ele, SGS_MHD_wk%fem_int, FEM_filters,       &
...
!
     ...
     &   then
          ...
          call s_cal_light_element(iphys%i_per_light, time_d%dt,        &
     &       FEM_prm, SGS_par, fem%mesh, fem%group,                     &
...
!
          call add_2_nod_scalars(nod_fld,                               &
     &        iphys%i_ref_c, iphys%i_per_light, iphys%i_light)
        else
     ...
     &       SGS_MHD_wk%mhd_fem_wk, SGS_MHD_wk%rhs_mat, nod_fld)
!
          if (iphys%i_per_light .gt. 0) then
            call subtract_2_nod_scalars(nod_fld,                        &
     &          iphys%i_light, iphys%i_ref_c, iphys%i_per_light)
          end if
        end if
!
...
!
     ...
     &      SGS_MHD_wk%fem_int, FEM_filters, SGS_MHD_wk%mk_MHD,         &
...
!
     ...
     &      SGS_MHD_wk%iphys_ele, SGS_MHD_wk%ele_fld,                   &
     &      SGS_MHD_wk%fem_int, FEM_filters, SGS_MHD_wk%mk_MHD,         &
     &      SGS_MHD_wk%FEM_SGS_wk, SGS_MHD_wk%rhs_mat,                  &
     &      nod_fld, diff_coefs)
      end if
!
...
!
     ...
     &      FEM_filters, ifld_sgs, icomp_sgs, ifld_diff, iphys_elediff, &
     &      sgs_coefs_nod, diff_coefs, SGS_MHD_wk%mk_MHD,               &
...
!
     ...
     &     (nod_fld, iphys%i_temp, iphys%i_ref_t, iphys%i_per_temp)
      ...
      if(MHD_prop%ref_param_C%iflag_reference .ne. id_no_ref_temp) then
        ...
        call subtract_2_nod_scalars                                     &
     &     (nod_fld, iphys%i_light, iphys%i_ref_c, iphys%i_per_light)
      end if
!
