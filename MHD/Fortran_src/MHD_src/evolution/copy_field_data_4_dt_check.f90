...
!
      if((iphys%i_chk_mom_2*iphys%i_chk_mom) .gt. izero) then
        call copy_vector_component(iphys%i_chk_mom, iphys%i_chk_mom_2)
      end if
!
...
!
      if((iphys%i_chk_uxb_2*iphys%i_chk_uxb) .gt. izero) then
        call copy_vector_component(iphys%i_chk_uxb, iphys%i_chk_uxb_2)
      end if
!
...
!
      if( (iphys%i_chk_mom*iphys%i_velo) .gt. izero) then
        call copy_vector_component(iphys%i_velo, iphys%i_chk_mom)
      end if
!
      if( (iphys%i_chk_press*iphys%i_press) .gt. izero) then
        call copy_scalar_component(iphys%i_press, iphys%i_chk_press)
      end if
!
!
      if(iflag_t_evo_4_vect_p .gt. id_no_evolution) then
        if( (iphys%i_chk_uxb*iphys%i_vecp) .gt. izero) then
          call copy_vector_component(iphys%i_vecp, iphys%i_chk_uxb)
        end if
      else
        if( (iphys%i_chk_uxb*iphys%i_magne) .gt. izero) then
          call copy_vector_component(iphys%i_magne, iphys%i_chk_uxb)
        end if
      end if
!
...
!
      if( (iphys%i_chk_heat*iphys%i_temp) .gt. izero) then
        call copy_scalar_component(iphys%i_temp, iphys%i_chk_heat)
      end if
!
      if( (iphys%i_chk_composit*iphys%i_light) .gt. izero) then
        call copy_scalar_component(iphys%i_light, iphys%i_chk_composit)
      end if
!
