...
!$omp parallel
     ...
     &     (ipol, ipol%div_forces%i_buoyancy,                           &
...
!
     ...
     &        rj_fld%n_point, rj_fld%ntot_phys, rj_fld%d_fld)
        ...
        if(fl_prop%iflag_4_gravity .ne. id_turn_OFF) then
          call add_term_to_div_force(ipol, ipol%div_forces%i_buoyancy,  &
     &        rj_fld%n_point, rj_fld%ntot_phys, rj_fld%d_fld)
...
!
!$omp parallel
      if(ipol%rot_SGS%i_SGS_inertia  .ne. 0                             &
     &  .and.  ipol%rot_SGS%i_SGS_Lorentz .ne. 0) then
