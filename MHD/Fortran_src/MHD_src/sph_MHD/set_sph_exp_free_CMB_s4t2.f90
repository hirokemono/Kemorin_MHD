...
!!
!!@verbatim
!!      subroutine cal_sph_cmb_free_v_and_w_s4t2(is_fld, is_rot)
!!      subroutine cal_sph_cmb_free_rot_s4t2(is_fld, is_rot)
!!      subroutine cal_sph_cmb_free_diffuse_s4t2(coef_d,                &
!!     &          is_fld, is_diffuse)
!!@endverbatim
!!
...
!$omp parallel do private(inod,i_n1,i_n2,i_n3,j,d1s_dr1,d2s_dr2,d1t_dr1)
      do j = 1, nidx_rj(2)
        ...
        i_n3 = i_n2 - nidx_rj(2)
!
        d1s_dr1 =  coef_fdm_free_CMB_vp4(-2,2) * d_rj(i_n2,is_fld  )    &
     &           + coef_fdm_free_CMB_vp4(-1,2) * d_rj(i_n1,is_fld  )
        d2s_dr2 =  coef_fdm_free_CMB_vp4(-2,3) * d_rj(i_n2,is_fld  )    &
     &           + coef_fdm_free_CMB_vp4(-1,3) * d_rj(i_n1,is_fld  )
...
!
!
        d1s_dr1 =  coef_fdm_free_CMB1_vp4(-2,2) * d_rj(i_n3,is_fld  )   &
     &           + coef_fdm_free_CMB1_vp4(-1,2) * d_rj(i_n2,is_fld  )   &
     &           + coef_fdm_free_CMB1_vp4( 0,2) * d_rj(i_n1,is_fld  )   &
     &           + coef_fdm_free_CMB1_vp4( 1,2) * d_rj(inod,is_fld  )
        d2s_dr2 =  coef_fdm_free_CMB1_vp4(-2,3) * d_rj(i_n3,is_fld  )   &
     &           + coef_fdm_free_CMB1_vp4(-1,3) * d_rj(i_n2,is_fld  )   &
     &           + coef_fdm_free_CMB1_vp4( 0,3) * d_rj(i_n1,is_fld  )   &
     &           + coef_fdm_free_CMB1_vp4( 1,3) * d_rj(inod,is_fld  )
...
!
     ...
     &                  * ar_1d_rj(kr_out,2)*d_rj(i_n1,is_fld  ) )
      end do
!$omp end parallel do
!
...
!
     ...
     &    - g_sph_rj(j,3)*ar_1d_rj(kr_out,2)*d_rj(i_n1,is_fld  ) )
        d_rj(i_n1,is_diffuse+2) =  coef_d * (d2t_dr2                    &
     &    - g_sph_rj(j,3)*ar_1d_rj(kr_out,2)*d_rj(i_n1,is_fld+2) )
      end do
!$omp end parallel do
!
