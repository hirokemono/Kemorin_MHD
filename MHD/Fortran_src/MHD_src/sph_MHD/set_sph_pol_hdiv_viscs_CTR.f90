...
!!        real(kind = kreal), intent(in) :: mat2_viscous_CTR1(0:1,jmax)
...
!!        real(kind = kreal), intent(in) :: mat4_viscous_CTR1(0:2,jmax)
!!        real(kind = kreal), intent(in) :: d_vpol(nnod_rj)
!!        real(kind = kreal), intent(in) :: press_e(nnod_rj)
!!        real(kind = kreal), intent(inout) :: d_viscous_p(nnod_rj)
...
!!        real(kind = kreal), intent(in) :: mat1_grad_p_CTR1(0:1)
!!        real(kind = kreal), intent(in) :: mat2_viscous_CTR1(-1:1,jmax)
...
!!        real(kind = kreal), intent(in) :: mat4_viscous_CTR1(-2:2,jmax)
...
!     &                        + mat2_viscous_CTR1(-1,j) * d_vpol(i_n1) &
     &                        - mat1_grad_p_CTR1( 0) *  press_e(inod)   &
     &                        + mat2_viscous_CTR1( 0,j) * d_vpol(inod)  &
     &                        - mat1_grad_p_CTR1( 1) *  press_e(i_p1)   &
     &                        + mat2_viscous_CTR1( 1,j) * d_vpol(i_p1)
      end do
!$omp end parallel do
!
...
!
        d_viscous_p(inod) = d_viscous_p(inod)                           &
!     &                        + mat4_viscous_CTR1(-2,j) * d_vpol(i_n2) &
!     &                        - mat3_grad_p_CTR1(-1) *  press_e(i_n1)  &
!     &                        + mat4_viscous_CTR1(-1,j) * d_vpol(i_n1) &
     &                        - mat3_grad_p_CTR1( 0) *  press_e(inod)   &
     &                        + mat4_viscous_CTR1( 0,j) * d_vpol(inod)  &
     &                        - mat3_grad_p_CTR1( 1) *  press_e(i_p1)   &
     &                        + mat4_viscous_CTR1( 1,j) * d_vpol(i_p1)  &
     &                        - mat3_grad_p_CTR1( 2) *  press_e(i_p2)   &
     &                        + mat4_viscous_CTR1( 2,j) * d_vpol(i_p2)
      end do
!$omp end parallel do
!
...
!        mat7(6, 0,j) = mat7(6, 0,j) - mat2_viscous_CTR1(-1,j)
        mat7(5, 1,j) = mat7(5, 1,j) + mat1_grad_p_CTR1(0)
!
        mat7(4, 2,j) = mat7(4, 2,j) - mat2_viscous_CTR1( 0,j)
!
        mat7(3, 3,j) = mat7(3, 3,j) + mat1_grad_p_CTR1(1)
        mat7(2, 4,j) = mat7(2, 4,j) - mat2_viscous_CTR1( 1,j)
...
!
      do j = 1, jmax
!        mat9(9,-2,j) = mat9(9,-2,j) - mat4_viscous_CTR1(-2,j)
!        mat9(8,-1,j) = mat9(8,-1,j) + mat3_grad_p_CTR1(-1)
!        mat9(7, 0,j) = mat9(7, 0,j) - mat4_viscous_CTR1(-1,j)
        mat9(6, 1,j) = mat9(6, 1,j) + mat3_grad_p_CTR1( 0)
!
        mat9(5, 2,j) = mat9(5, 2,j) - mat4_viscous_CTR1( 0,j)
!
        mat9(4, 3,j) = mat9(4, 3,j) + mat3_grad_p_CTR1( 1)
        mat9(3, 4,j) = mat9(3, 4,j) - mat4_viscous_CTR1( 1,j)
        mat9(2, 5,j) = mat9(2, 5,j) + mat3_grad_p_CTR1( 2)
        mat9(1, 6,j) = mat9(1, 6,j) - mat4_viscous_CTR1( 2,j)
      end do
!
