!>@file   local_field_4_dynamobench.f90
!!@brief  module local_field_4_dynamobench
!!
!!@author H. Matsui
...
!!
!!@n      subroutine deallocate_mid_eq_field
!!@n      subroutine set_mid_equator_point_global
!!@n      subroutine mid_eq_transfer_dynamobench
!
      module local_field_4_dynamobench
!
      use m_precision
!
      use m_constants
...
!
      integer(kind = kint) :: kr_gl_rmid_in, kr_gl_rmid_out
      real(kind = kreal) :: coef_gl_rmid_in, coef_gl_rmid_out
!
      real(kind = kreal), allocatable :: d_rj_mid_lc(:,:)
      real(kind = kreal), allocatable :: d_rj_mid_eq(:,:)
!
      integer(kind = kint), parameter :: imul_eq = 4
      integer(kind = kint) :: mphi_eq
...
!
        allocate( d_rtp_eq_mid(mphi_eq,7) )
!
        d_rj_mid_eq = 0.0d0
        d_rtp_eq_mid = 0.0d0
      end if
...
!
      call equator_transfer_scalar(nidx_global_rj(2), mphi_eq,          &
     &    d_rj_mid_eq(0,ibench_temp), d_rtp_eq_mid(1,ibench_temp))
      call equator_transfer_vector(nidx_global_rj(2), mphi_eq,          &
...
! ----------------------------------------------------------------------
!
      end module local_field_4_dynamobench
