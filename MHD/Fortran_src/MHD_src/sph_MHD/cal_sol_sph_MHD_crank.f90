...
!  Input: ipol%i_temp,  Solution: ipol%i_temp
     ...
     &     MHD_prop%ht_prop, sph_MHD_bc%sph_bc_T, sph_MHD_bc%bc_Tspec,  &
...
!  Input: ipol%i_light,  Solution: ipol%i_light
      ...
      if(MHD_prop%cp_prop%iflag_scheme .gt. id_no_evolution) then
        call cal_sol_scalar_sph_crank(dt, sph_rj,                       &
     &     MHD_prop%cp_prop, sph_MHD_bc%sph_bc_C, sph_MHD_bc%bc_Cspec,  &
...
!
     ...
     &          sph_bc_T, bc_Tspec, fdm2_center, leg, ipol, rj_fld)
!
      use t_physical_property
!
      type(sph_rj_grid), intent(in) ::  sph_rj
      ...
      type(sph_scalar_BC_coef), intent(in) :: bc_Tspec
...
!         Input: ipol%i_temp,  Solution: ipol%i_grad_t
     ...
     &     (sph_rj, r_2nd, sph_bc_T, bc_Tspec, fdm2_center,             &
...
!
     ...
     &          sph_bc_C, bc_Cspec, fdm2_center, leg, ipol, rj_fld)
!
      use t_physical_property
!
      type(sph_rj_grid), intent(in) ::  sph_rj
      ...
      type(sph_scalar_BC_coef), intent(in) :: bc_Cspec
      type(fdm2_center_mat), intent(in) :: fdm2_center
      type(legendre_4_sph_trans), intent(in) :: leg
      type(phys_address), intent(in) :: ipol
      type(phys_data), intent(inout) :: rj_fld
...
!         Input: ipol%i_light,  Solution: ipol%i_grad_composit
      if(ipol%i_grad_composit .gt. 0) then
        call const_radial_grad_scalar                                   &
     &     (sph_rj, r_2nd, sph_bc_C, bc_Cspec, fdm2_center,             &
