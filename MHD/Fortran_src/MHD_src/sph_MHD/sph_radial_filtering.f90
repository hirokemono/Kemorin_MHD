...
!!     &          i_field, i_filter, nnod_rj, ntot_comp, d_rj)
!!
!!      subroutine vector_sph_horiz_filter                              &
!!     &         (ltr_filter, weight, nidx_rj, idx_gl_1d_rj_j,          &
!!     &          i_field, i_filter, nnod_rj, ntot_comp, d_rj)
!!      subroutine scalar_sph_horiz_filter                              &
!!     &         (ltr_filter, weight, nidx_rj, idx_gl_1d_rj_j,          &
!!     &          i_field, i_filter, nnod_rj, ntot_comp, d_rj)
!!      subroutine sym_tensor_sph_horiz_filter                          &
!!     &         (ltr_filter, weight, nidx_rj, idx_gl_1d_rj_j,          &
!!     &          i_field, i_filter, nnod_rj, ntot_comp, d_rj)
!!@endverbatim
!!
...
!$omp parallel do private(k,j,l,inod)
      do k = 1, nidx_rj(1)
        do j = 1, nidx_rj(2)
          ...
          d_rj(inod,i_filter  ) = d_rj(inod,i_field  ) * weight(l)
          d_rj(inod,i_filter+1) = d_rj(inod,i_field+1) * weight(l)
          d_rj(inod,i_filter+2) = d_rj(inod,i_field+2) * weight(l)
        end do
      end do
!$omp end parallel do
!
      end subroutine vector_sph_horiz_filter
!
! ----------------------------------------------------------------------
!
      subroutine scalar_sph_horiz_filter                                &
...
!$omp end parallel do
!
      end subroutine scalar_sph_horiz_filter
!
! ----------------------------------------------------------------------
!
      subroutine sym_tensor_sph_horiz_filter                            &
...
!$omp parallel do private(k,j,l,inod)
      do k = 1, nidx_rj(1)
        do j = 1, nidx_rj(2)
          ...
          d_rj(inod,i_filter+1) = d_rj(inod,i_field+1) * weight(l)
          d_rj(inod,i_filter+2) = d_rj(inod,i_field+2) * weight(l)
          d_rj(inod,i_filter+3) = d_rj(inod,i_field+3) * weight(l)
          d_rj(inod,i_filter+4) = d_rj(inod,i_field+4) * weight(l)
          d_rj(inod,i_filter+5) = d_rj(inod,i_field+5) * weight(l)
        end do
      end do
!$omp end parallel do
!
      end subroutine sym_tensor_sph_horiz_filter
!
! ----------------------------------------------------------------------
!
