...
!
     ...
     &      fem_int%jcs, fem_int%rhs_tbl, fem_wk, f_l,                  &
...
!
     ...
     &          (n_int, node, ele, fluid, conduct, insulate, jacs,      &
     &           rhs_tbl, fem_wk, f_l, m_lump, mk_MHD)
!
      integer(kind = kint), intent(in) :: n_int
!
...
!
     ...
     &  write(*,*) 'int_lump_mass_matrix_linear'
      call int_lump_mass_matrix_linear(node, ele,                       &
     &    jacs%g_FEM, jacs%jac_3d, rhs_tbl, n_int, fem_wk, f_l, m_lump)
!
      if (iflag_debug.eq.1)                                             &
...
!
     ...
     &  write(*,*) 'int_mass_matrix_diag insulator'
      call int_mass_matrix_diag(node, ele, jacs%g_FEM, jacs%jac_3d,     &
     &    rhs_tbl,insulate%istack_ele_fld_smp, n_int, fem_wk, f_l,      &
     &    mk_MHD%mlump_ins)
!
