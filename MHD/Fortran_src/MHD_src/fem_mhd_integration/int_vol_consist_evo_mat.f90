...
!
     ...
     &      .and. coef_velo.gt.0.0d0 ) then
          call add_skv1_to_crs_matrix33(ele, rhs_tbl, mat_tbl_fl_q,     &
     &        k2, fem_wk%sk6, Vmat_DJDS%num_non0, Vmat_DJDS%aiccg)
        end if
!
        if ( iflag_t_evo_4_temp .eq. id_Crank_nicolson_cmass            &
     &      .and. coef_temp.gt.0.0d0 ) then
          call add_skv1_to_crs_matrix11(ele, rhs_tbl, mat_tbl_fl_q,     &
     &        k2, fem_wk%sk6, Tmat_DJDS%num_non0, Tmat_DJDS%aiccg)
        end if
!
        if ( iflag_t_evo_4_composit .eq. id_Crank_nicolson_cmass        &
     &      .and. coef_light .gt. 0.0d0) then
          call add_skv1_to_crs_matrix11(ele, rhs_tbl, mat_tbl_fl_q,     &
     &        k2, fem_wk%sk6, Cmat_DJDS%num_non0, Cmat_DJDS%aiccg)
        end if
!
        if ( iflag_t_evo_4_magne .eq. id_Crank_nicolson_cmass           &
     &      .and. coef_magne.gt.0.0d0) then
          call add_skv1_to_crs_matrix33                                 &
     &       (ele, rhs_tbl, mat_tbl_full_cd_q,                          &
