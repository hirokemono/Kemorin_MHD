...
!     &          nod_fld, iele_fsmp_stack, n_int, bxe, fem_wk, f_nl)
...
! -------- loop for shape function for the phsical values
     ...
     &      iphys%i_velo,  fem_wk%velo_1)
        call vector_phys_2_each_ele_type(mesh, nod_fld, k2,             &
     &      iphys%i_magne, fem_wk%vector_1)
!
        call fem_skv_induction_type(iele_fsmp_stack, n_int, k2,         &
     &      coef_induct, fem_wk%velo_1, fem_wk%vxe, mesh%ele,           &
     &      jac_3d, fem_wk)
      end do
!
...
!
        call fem_skv_induction_type(iele_fsmp_stack, n_int, k2,         &
     &      coef_induct, fem_wk%velo_1, fem_wk%vxe, bxe, mesh%ele,      &
     &      jac_3d, fem_wk)
      end do
!
      call add3_skv_to_ff_v_smp_type(mesh, rhs_tbl, fem_wk, f_nl)
