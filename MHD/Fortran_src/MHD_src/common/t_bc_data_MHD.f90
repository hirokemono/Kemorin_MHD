...
!
      if (iflag_t_evo_4_velo .gt. id_no_evolution) then
        ...
        call set_bc_velo_id(mesh%node, mesh%ele, MHD_mesh%fluid,        &
     &      group%nod_grp, nodal_bc%Vnod_bcs)
        if ( iflag_debug .eq.1) write(*,*)  'set boundary id 4 P'
        call set_bc_press_id(mesh%node, mesh%ele, MHD_mesh%fluid,       &
     &      group%nod_grp, nodal_bc%Vnod_bcs)
      end if
!
      if (iflag_t_evo_4_temp .gt. id_no_evolution) then
        call set_bc_temp_id(mesh%node, mesh%ele, MHD_mesh%fluid,        &
     &      group%nod_grp, nodal_bc%Tnod_bcs)
      end if
!
!
      if (iflag_t_evo_4_composit .gt. id_no_evolution) then
        call set_bc_temp_id(mesh%node, mesh%ele, MHD_mesh%fluid,        &
     &      group%nod_grp, nodal_bc%Cnod_bcs)
      end if
!
!
     ...
     &     (mesh%node, mesh%ele, group%nod_grp, nodal_bc%Bnod_bcs)
     ...
     &     (mesh%node, mesh%ele, MHD_mesh%conduct, MHD_mesh%insulate,   &
