...
!        type(DJDS_MATRIX), intent(inout) :: mat33
!
!      subroutine set_bc_4_scalar_mat_type(iele, k1, k2, mesh,          &
!     &          djds_tbl, mat11)
...
!
      ...
      type(DJDS_MATRIX), intent(inout) :: mat33
!
      integer(kind = kint) :: nd, iele, k0, k1, k2
!
!
      do nd = 1, n_vector
...
!
          ...
          do k2 = 1, mesh%ele%nnod_4_ele
            call set_bc_4_vector_mat_type(nd, nd, iele, k1, k2,         &
     &            mesh, djds_tbl, mat33)
          end do
!
...
!
          ...
          do k2 = 1, mesh%ele%nnod_4_ele
            call set_bc_4_vector_mat_type(ione, n_vector, iele,         &
     &          k1, k2, mesh, djds_tbl, mat33)
          end do
!
          k2 = rot_bc%nod_bc_id(k0)
          do k1 = 1, mesh%ele%nnod_4_ele
            call set_bc_4_vector_mat_type(ione, n_vector, iele, k1, k2, &
     &            mesh, djds_tbl, mat33)
          end do
!
        end do
...
!
          ...
          do k2 = 1, nnod
            call set_bc_4_scalar_mat_type(iele, k1, k2, mesh,           &
     &          djds_tbl, mat11)
          end do
!
...
! -----------------------------------------------------------------------
!
      subroutine set_bc_4_vector_mat_type(nst, ned, iele, k1, k2,       &
     &          mesh, djds_tbl, mat33)
!
...
!
      do nd2 = nst, ned
        call correct_matrix33_4_boundary(mesh%node%numnod,              &
...
!
      ...
      nod2 = mesh%ele%ie(iele,k2)
!
      call set_off_diag_type(mesh%node, djds_tbl, nod1, nod2, mat_num)
      call correct_matrix11_4_boundary                                  &
     &    (mesh%node%numnod, djds_tbl%itotal_u, djds_tbl%itotal_l,      &
     &     mat_num, mat11%aiccg)
!
