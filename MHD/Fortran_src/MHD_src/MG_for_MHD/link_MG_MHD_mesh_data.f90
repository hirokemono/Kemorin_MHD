...
!
!!      subroutine s_link_MG_MHD_mesh_data(ele_1st)
...
!
     ...
     &    MHD1_matrices%MG_comm_fluid(1))
!
      call init_interpolate_mat_type                                    &
     &    (ele_1st, MHD1_matrices%MG_interpolate(1)%f2c)
      call init_interpolate_mat_type                                    &
     &   (MG_mesh(1)%mesh%ele, MHD1_matrices%MG_interpolate(1)%c2f)
!
     ...
     &      MHD1_matrices%MG_comm_fluid(i_level))
!
        call init_interpolate_mat_type(MG_mesh(i_level-1)%mesh%ele,     &
     &      MHD1_matrices%MG_interpolate(i_level)%f2c)
        call init_interpolate_mat_type(MG_mesh(i_level)%mesh%ele,       &
     &      MHD1_matrices%MG_interpolate(i_level)%c2f)
      end do
!
