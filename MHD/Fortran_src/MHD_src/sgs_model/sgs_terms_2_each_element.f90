!sgs_terms_2_each_element.f90
!      module sgs_terms_2_each_element
!
!      Written by H. Matsui on July, 2005
!
!      subroutine SGS_vector_2_each_element(numnod, numele, nnod_4_ele, &
!     &          ie, np_smp, iele_smp_stack, k2, i_vect, i_scalar,      &
!     &          i_sgs, ntot_comp, d_nod, sgs_e, flux_e)
!      subroutine SGS_tensor_2_each_element(numnod, numele, nnod_4_ele, &
!     &          ie, np_smp, iele_smp_stack, k2, i_vect, i_sgs,         &
!     &          ntot_comp, d_nod, sgs_e, flux_e)
!      subroutine SGS_induct_to_each_element(numnod, numele, nnod_4_ele,&
!     &          ie, np_smp, iele_smp_stack, k2, i_b, i_v, i_sgs,       &
!     &          ntot_comp, d_nod, sgs_e, flux_e)
!
!      subroutine SGS_flux_2_each_element_vec(numnod, numele,           &
!     &          nnod_4_ele, ie, np_smp, iele_smp_stack, k2, nd,        &
!     &          i_vect, i_field, i_sgs, ntot_comp, d_nod,              &
!     &          sgs_e, flux_e)
!      subroutine SGS_induct_vec_2_each_element(numnod, numele,         &
!     &          nnod_4_ele, ie, np_smp, iele_smp_stack, k2, nd,        &
!     &          i_b, i_v, i_sgs, ntot_comp, d_nod, sgs_e, flux_e)
!
      module sgs_terms_2_each_element
!
      use m_precision
...
!cdir nodep
         do iele = ist, ied
           inod = ie(iele,k2)
           sgs_e(iele,1) = d_nod(inod,i_sgs  )
           sgs_e(iele,2) = d_nod(inod,i_sgs+1)
           sgs_e(iele,3) = d_nod(inod,i_sgs+2)
           flux_e(iele,1) =  d_nod(inod,i_sgs  )                        &
     &                     + d_nod(inod,i_scalar)*d_nod(inod,i_vect  )
           flux_e(iele,2) =  d_nod(inod,i_sgs+1)                        &
     &                     + d_nod(inod,i_scalar)*d_nod(inod,i_vect+1)
           flux_e(iele,3) =  d_nod(inod,i_sgs+2)                        &
     &                     + d_nod(inod,i_scalar)*d_nod(inod,i_vect+2)
         end do
       end do
!$omp end parallel do
!
      end subroutine SGS_vector_2_each_element
!
!  ---------------------------------------------------------------------
!
      subroutine SGS_tensor_2_each_element(numnod, numele, nnod_4_ele,  &
...
!cdir nodep
         do iele = ist, ied
           ...
           sgs_e(iele,2) = d_nod(inod,i_sgs+1)
           sgs_e(iele,3) = d_nod(inod,i_sgs+2)
           sgs_e(iele,4) = d_nod(inod,i_sgs+3)
           sgs_e(iele,5) = d_nod(inod,i_sgs+4)
           sgs_e(iele,6) = d_nod(inod,i_sgs+5)
           flux_e(iele,1) =   d_nod(inod,i_sgs  )                       &
     ...
     &                     +  d_nod(inod,i_vect  )*d_nod(inod,i_vect+2)
           flux_e(iele,4) =   d_nod(inod,i_sgs+3)                       &
     &                     +  d_nod(inod,i_vect+1)*d_nod(inod,i_vect+1)
           flux_e(iele,5) =   d_nod(inod,i_sgs+4)                       &
     &                     +  d_nod(inod,i_vect+1)*d_nod(inod,i_vect+2)
           flux_e(iele,6) =   d_nod(inod,i_sgs+5)                       &
     &                     +  d_nod(inod,i_vect+2)*d_nod(inod,i_vect+2)
         end do
       end do
!$omp end parallel do
!
      end subroutine SGS_tensor_2_each_element
!
!  ---------------------------------------------------------------------
!
      subroutine SGS_induct_to_each_element(numnod, numele, nnod_4_ele, &
...
!
!
      end subroutine SGS_induct_to_each_element
!
!  ---------------------------------------------------------------------
!  ---------------------------------------------------------------------
!
...
!cdir nodep
         do iele = ist, ied
           inod = ie(iele,k2)
           sgs_e(iele,1) = d_nod(inod,n1)
           sgs_e(iele,2) = d_nod(inod,n2)
           sgs_e(iele,3) = d_nod(inod,n3)
           flux_e(iele,1) =  d_nod(inod,n1)                             &
     &                     + d_nod(inod,i_vect  )*d_nod(inod,nf)
           flux_e(iele,2) =  d_nod(inod,n2)                             &
     &                     + d_nod(inod,i_vect+1)*d_nod(inod,nf)
           flux_e(iele,3) =  d_nod(inod,n3)                             &
     &                     + d_nod(inod,i_vect+2)*d_nod(inod,nf)
         end do
       end do
!$omp end parallel do
!
      end subroutine SGS_flux_2_each_element_vec
!
!  ---------------------------------------------------------------------
!
      subroutine SGS_induct_vec_2_each_element(numnod, numele,          &
...
!cdir nodep
         do iele = ist, ied
           inod = ie(iele,k2)
           sgs_e(iele,1) = dble(l_asim_t(1,nd,2)) * d_nod(inod,n1)
           sgs_e(iele,2) = dble(l_asim_t(2,nd,2)) * d_nod(inod,n2)
           sgs_e(iele,3) = dble(l_asim_t(3,nd,2)) * d_nod(inod,n3)
           flux_e(iele,1) =  dble(l_asim_t(1,nd,2)) * d_nod(inod,n1)    &
     &                     + d_nod(inod,i_b  )*d_nod(inod,i_v+nd-1)     &
     &                     - d_nod(inod,i_v  )*d_nod(inod,i_b+nd-1)
           flux_e(iele,2) =  dble(l_asim_t(2,nd,2)) * d_nod(inod,n2)    &
     &                     + d_nod(inod,i_b+1)*d_nod(inod,i_v+nd-1)     &
     &                     - d_nod(inod,i_v+1)*d_nod(inod,i_b+nd-1)
           flux_e(iele,3) = dble(l_asim_t(3,nd,2)) * d_nod(inod,n3)     &
...
!
!
      end subroutine SGS_induct_vec_2_each_element
!
!  ---------------------------------------------------------------------
!
      end module sgs_terms_2_each_element
