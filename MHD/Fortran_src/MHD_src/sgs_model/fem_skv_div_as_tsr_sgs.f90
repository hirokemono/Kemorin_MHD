!fem_skv_div_as_tsr_sgs.f90
!     module fem_skv_div_as_tsr_sgs
!
!        programmed by H.Matsui on July, 2005
!        modified by H. Matsui on Aug., 2007
!
!      subroutine fem_skv_div_ast_sgs_pg(numele, nnod_4_e1, nnod_4_e2,  &
...
!     &          xmom_order2, ak_diff, as_tsr_1, sk_v)
!
      module fem_skv_div_as_tsr_sgs
!
      use m_precision
...
!
     ...
     &             + ak_diff(iele) * div_y ) * as_tsr_1(iele,1)         &
     &              - (an1(k1,ix) * dnx2(iele,k2,ix,3)                  &
     &             + ak_diff(iele) * div_z ) * as_tsr_1(iele,3))        &
     &           * xjac(iele,ix)*owe3d(ix)
!
              sk_v(iele,2,k1) = sk_v(iele,2,k1)                         &
     &            + (-(an1(k1,ix) * dnx2(iele,k2,ix,1)                  &
     &             + ak_diff(iele) * div_x ) * as_tsr_1(iele,1)         &
     &              + (an1(k1,ix) * dnx2(iele,k2,ix,3)                  &
     &             + ak_diff(iele) * div_z ) * as_tsr_1(iele,2))        &
     &           * xjac(iele,ix)*owe3d(ix)
!
              sk_v(iele,3,k1) = sk_v(iele,3,k1)                         &
     &            + ( (an1(k1,ix) * dnx2(iele,k2,ix,1)                  &
     &             + ak_diff(iele) * div_x ) * as_tsr_1(iele,3)         &
     &              - (an1(k1,ix) * dnx2(iele,k2,ix,2)                  &
     &             + ak_diff(iele) * div_y ) * as_tsr_1(iele,2))        &
     &           * xjac(iele,ix)*owe3d(ix)
            end do
          end do
!
...
!$omp end parallel do
!
      end subroutine fem_skv_div_ast_sgs_pg
!
!-----------------------------------------------------------------------
!
      end module fem_skv_div_as_tsr_sgs
