...
!
     ...
     &                     iphys%i_sgs_grad, iphys%i_magne
      call cal_rotation_whole(iphys%i_sgs_grad, iphys%i_magne)
      if (iflag_debug.gt.0)                                             &
     &   write(*,*) 'cal_gradent_in_fluid', i_sgs_grad_p, iphys%i_mag_p
      call cal_gradent_in_fluid(i_sgs_grad_p, iphys%i_mag_p)
!      call cal_divergence_whole(iphys%i_sgs_grad+6, iphys%i_magne)
!
...
!
      call cal_commute_error_f_magne(n_quad, iphys%i_sgs_grad_f)
      call cal_commute_error_f_magne_p(n_quad, i_sgs_grad_fp)
!
...
!
      call cal_commute_error_magne(n_second, iphys%i_sgs_grad)
      call cal_commute_error_magne_p(n_second, i_sgs_grad_p)
!
