...
!
     ...
     &    sgs_coefs%num_field, sgs_coefs%ntot_comp, wk_sgs1)
      call alloc_sgs_coefs_layer(layer_tbl%e_grp%num_grp,               &
     &    diff_coefs%num_field, ntot_diff_comp, wk_diff1)
!
       ...
       if (iflag_t_evo_4_temp .gt. id_no_evolution) then
         if (iflag_SGS_heat .ne. id_SGS_none) then
           ...
           if (iflag_commute_heat .eq. id_SGS_commute_ON) then
             icomp_diff_hf = id
             iak_diff_hf = jd
             wk_diff1%name(jd) = fhd_SGS_h_flux
       ...
       if (iflag_t_evo_4_velo .gt. id_no_evolution) then
         if (iflag_SGS_inertia .ne. id_SGS_none) then
           ...
           if (iflag_commute_inertia .eq. id_SGS_commute_ON) then
             icomp_diff_mf = id
             iak_diff_mf = jd
             wk_diff1%name(jd) = fhd_SGS_m_flux
...
!
         if (iflag_SGS_lorentz .ne. id_SGS_none) then
           icomp_sgs_lor = i
           iak_sgs_lor =   j
           wk_sgs1%name(j) = fhd_SGS_maxwell_t
           sgs_coefs%num_comps(j) = 6
           i = i + sgs_coefs%num_comps(j)
           j = j + 1
           if (iflag_commute_lorentz .eq. id_SGS_commute_ON) then
             icomp_diff_lor = id
             iak_diff_lor = jd
             wk_diff1%name(jd) = fhd_SGS_Lorentz
             diff_coefs%num_comps(jd) = 9
             id = id + diff_coefs%num_comps(jd)
             jd = jd + 1
           end if
...
!
        if (iflag_SGS_gravity .ne. id_SGS_none) then
          if(iflag_4_gravity .gt. 0) then
            icomp_sgs_tbuo = i
            iak_sgs_tbuo =   j
            wk_sgs1%name(j) = fhd_SGS_buoyancy
            sgs_coefs%num_comps(j) = 6
            i = i + sgs_coefs%num_comps(j)
            j = j + 1
          end if
          if(iflag_4_composit_buo .gt. id_turn_OFF) then
            icomp_sgs_cbuo = i
            iak_sgs_cbuo =   j
            wk_sgs1%name(j) = fhd_SGS_comp_buo
            sgs_coefs%num_comps(j) = 6
            i = i + sgs_coefs%num_comps(j)
            j = j + 1
          end if
...
!
       if (iflag_t_evo_4_vect_p .gt. id_no_evolution) then
         if (iflag_SGS_induction .ne. id_SGS_none) then
           icomp_sgs_uxb = i
           iak_sgs_uxb =   j
           wk_sgs1%name(j) = fhd_SGS_induction
           sgs_coefs%num_comps(j) = 3
           i = i + sgs_coefs%num_comps(j)
           j = j + 1
         end if
       ...
       if (iflag_t_evo_4_magne .gt. id_no_evolution) then
         if (iflag_SGS_induction .ne. id_SGS_none) then
           ...
           if (iflag_commute_induction .eq. id_SGS_commute_ON) then
             icomp_diff_uxb = id
             iak_diff_uxb = jd
             wk_diff1%name(jd) = fhd_SGS_induction
             diff_coefs%num_comps(jd) = 9
             id = id + diff_coefs%num_comps(jd)
             jd = jd + 1
           end if
       ...
       if (iflag_t_evo_4_composit .gt. id_no_evolution) then
         if (iflag_SGS_comp_flux .ne. id_SGS_none) then
           ...
           if (iflag_commute_c_flux .eq. id_SGS_commute_ON) then
             icomp_diff_cf = id
             iak_diff_cf = jd
             wk_diff1%name(jd) = fhd_SGS_c_flux
             diff_coefs%num_comps(jd) = 3
             id = id + diff_coefs%num_comps(jd)
             jd = jd + 1
           end if
