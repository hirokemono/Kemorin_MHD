!fem_skv_div_tensor_sgs_upw.f90
!     module fem_skv_div_tensor_sgs_upw
!
!        programmed by H.Matsui on July, 2005
!        modified by H. Matsui on Aug., 2007
!
!      subroutine fem_skv_div_tsr_sgs_upw(numele, nnod_4_e1, nnod_4_e2, &
...
!
!$omp parallel do                                                       &
!$omp&  private(k1,ii,ix,iele,istart,iend,div_x,div_y,div_z,tau)
      do iproc = 1, np_smp
...
!
     ...
     &             + ak_diff(iele) * div_x ) * tensor_1(iele,1)         &
     &              + (tau * dnx2(iele,k2,ix,2)                         &
     &             + ak_diff(iele) * div_y ) * tensor_1(iele,2)         &
     &              + (tau * dnx2(iele,k2,ix,3)                         &
     &             + ak_diff(iele) * div_z ) * tensor_1(iele,3))        &
     &           * xjac(iele,ix)*owe3d(ix)
!
              sk_v(iele,2,k1) = sk_v(iele,2,k1)                         &
     &            + ( (tau * dnx2(iele,k2,ix,1)                         &
     &             + ak_diff(iele) * div_x ) * tensor_1(iele,2)         &
     &              + (tau * dnx2(iele,k2,ix,2)                         &
     &             + ak_diff(iele) * div_y ) * tensor_1(iele,4)         &
     &              + (tau * dnx2(iele,k2,ix,3)                         &
     &             + ak_diff(iele) * div_z ) * tensor_1(iele,5))        &
     &           * xjac(iele,ix)*owe3d(ix)
!
              sk_v(iele,3,k1) = sk_v(iele,3,k1)                         &
     &            + ( (tau * dnx2(iele,k2,ix,1)                         &
     &             + ak_diff(iele) * div_x ) * tensor_1(iele,3)         &
     &              + (tau * dnx2(iele,k2,ix,2)                         &
     &             + ak_diff(iele) * div_y ) * tensor_1(iele,5)         &
     &              + (tau * dnx2(iele,k2,ix,3)                         &
     &             + ak_diff(iele) * div_z ) * tensor_1(iele,6))        &
     &           * xjac(iele,ix)*owe3d(ix)
            end do
          end do
!
...
!$omp end parallel do
!
      end subroutine fem_skv_div_tsr_sgs_upw
!
!-----------------------------------------------------------------------
!
      end module fem_skv_div_tensor_sgs_upw
