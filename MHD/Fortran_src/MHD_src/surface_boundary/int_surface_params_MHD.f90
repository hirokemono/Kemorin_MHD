...
!
!!      subroutine int_surface_parameters(mesh, group, surf_wk)
...
!
      ...
      call position_2_each_surface(mesh%node, mesh%ele, mesh%surf,      &
     &    group%surf_grp, surf_wk%xe_sf)
!
      if (iflag_debug.eq.1)  write(*,*) 'delta_x_2_each_surface'
      call delta_x_2_each_surface(mesh%node, mesh%ele, mesh%surf,       &
     &    group%surf_grp, surf_wk%dxe_sf)
!
