...
!
     ...
     &        rhs_tbl, FEM_elens, diff_coefs%Cdiff_SGS_mf, Vsf_bcs%sgs, &
     &        num_int, SGS_param%ifilter_final, iphys_SGS%i_SGS_m_flux, &
...
!
      if (SGS_param%iflag_SGS_lorentz .ne. id_SGS_none) then
        if (cmt_param%iflag_c_lorentz .eq. id_SGS_commute_ON) then
          call int_sf_skv_sgs_div_t_flux                                &
     &      (node, ele, surf, sf_grp, nod_fld, g_FEM, jac_sf_grp,       &
     &       rhs_tbl, FEM_elens, diff_coefs%Cdiff_SGS_lor, Bsf_bcs%sgs, &
     &       num_int, SGS_param%ifilter_final, iphys_SGS%i_SGS_maxwell, &
