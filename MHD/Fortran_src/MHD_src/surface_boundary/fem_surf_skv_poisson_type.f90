...
!!     &         (ele, surf, sf_grp, g_FEM, jac_sf_grp_l, igrp,         &
!!     &          k2, n_int, vect_sf, sk_v)
!!      subroutine fem_surf_skv_poisson_sph_out                         &
!!     &         (ele, surf, sf_grp, g_FEM, jac_sf_grp_l, igrp,         &
!!     &          k2, n_int, xe_sf, vect_sf, sk_v)
!!
!!      subroutine fem_surf_skv_current_by_vecp                         &
!!     &          (ele, surf, sf_grp, g_FEM, jac_sf_grp, igrp,          &
!!     &           k2, nd, n_int, dxe_sf, scalar_sf, sk_v)
!!      subroutine fem_surf_skv_diffuse_galerkin                        &
!!     &         (ele, surf, sf_grp, g_FEM, jac_sf_grp, igrp,           &
!!     &          k2, nd, n_int, dxe_sf, scalar_sf, ak_d, sk_v)
!!
!!      subroutine fem_surf_skv_trq_sph_out                             &
!!     &         (ele, surf, sf_grp, g_FEM, jac_sf_grp, igrp,           &
!!     &          k2, n_int, ak_d_velo, xe_sf, vect_sf, sk_v)
!!
!!      subroutine fem_surf_skv_norm_grad_galerkin                      &
!!     &         (ele, surf, sf_grp, g_FEM, jac_sf_grp, grad_sf,        &
!!     &          n_int, nd, ak_d, sk_v)
!!      subroutine fem_surf_skv_norm_poisson_pg(ele, surf, sf_grp,      &
!!     &          g_FEM, jac_sf_grp, grad_sf, n_int, sk_v)
...
!
     ...
     &   g_FEM%owe2d, igrp, k2, n_int, jac_sf_grp_l%ntot_int,           &
     &   jac_sf_grp_l%xsf_sf, jac_sf_grp_l%an_sf, jac_sf_grp_l%an_sf,   &
     &   vect_sf, sk_v)
!
...
!
     ...
     &   g_FEM%max_int_point, g_FEM%maxtot_int_2d, g_FEM%int_start2,    &
     &   g_FEM%owe2d, igrp, k2, n_int, jac_sf_grp_l%ntot_int,           &
     &   jac_sf_grp_l%xj_sf, jac_sf_grp_l%an_sf, jac_sf_grp_l%an_sf,    &
     &   xe_sf, vect_sf, sk_v)
!
...
!
     ...
     &    igrp, k2, nd, n_int, g_FEM%max_int_point,                     &
     &    g_FEM%maxtot_int_2d, g_FEM%int_start2, g_FEM%owe2d,           &
     &    jac_sf_grp%ntot_int, jac_sf_grp%xj_sf,                        &
...
!
     ...
     &    jac_sf_grp%an_sf, ak_d, dxe_sf, scalar_sf, sk_v)
!
...
!
     ...
     &    sf_grp%num_grp_smp, sf_grp%istack_grp_smp,                    &
     &    g_FEM%max_int_point, g_FEM%maxtot_int_2d, g_FEM%int_start2,   &
     &    g_FEM%owe2d, igrp, k2, n_int, jac_sf_grp%ntot_int,            &
...
!
     ...
     &   g_FEM%max_int_point, g_FEM%maxtot_int_2d, g_FEM%int_start2,    &
     &   g_FEM%owe2d, n_int, nd, jac_sf_grp%ntot_int,                   &
...
!
     ...
     &   grad_sf%ist_ele_sf_fix_fx, grad_sf%sf_apt_fix_fx,              &
     &   g_FEM%max_int_point, g_FEM%maxtot_int_2d, g_FEM%int_start2,    &
     &   g_FEM%owe2d, n_int, jac_sf_grp%ntot_int, jac_sf_grp%xj_sf,     &
     &   jac_sf_grp%an_sf, sk_v)
!
