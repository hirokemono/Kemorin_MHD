...
!
!!      subroutine cal_correlate_sgs_dynamic(layer_tbl,                 &
!!     &          node, ele, iphys, nod_fld, jac_3d_q, jac_3d_l,        &
!!     &          n_tensor, icomp_f, n_int)
!!      subroutine cal_correlate_diff_dynamic(layer_tbl,                &
!!     &          node, ele, iphys, nod_fld, jac_3d_q, jac_3d_l,        &
!!     &          n_tensor, icomp_f, n_int)
...
!  Volume integration:                      int_vol_layer_correlate
      call int_vol_layer_correlate(layer_tbl,                           &
     &    node, ele, iphys, nod_fld, jac_3d_q, jac_3d_l,                &
     &    n_tensor, n_int,   ave_diff_simi(1,icomp_f),                  &
     &    ave_diff_grad(1,icomp_f) )
!
      call sum_layerd_correlation(layer_tbl%e_grp%num_grp)
      call cal_layered_correlation                                      &
     &   (layer_tbl%e_grp%num_grp, n_tensor, layer_tbl%a_vol_layer,     &
...
!  Volume integration:                      int_vol_diff_correlate
     ...
     &    node, ele, iphys, nod_fld, jac_3d_q, jac_3d_l,                &
     &    n_tensor, n_int, ave_diff_grad_w(icomp_f),                    &
     &    ave_diff_grad_w(icomp_f) )
!
      call sum_whole_correlation
      call cal_all_layer_correlation                                    &
     &   (n_tensor, layer_tbl%vol_total_layer(1),                       &
