...
!>       FEM order of each field
        integer (kind=kint), pointer :: iorder_ele(:)
!
!>       field name
...
!
      ...
      allocate( fld%iorder_ele(fld%num_phys) )
...
!
!
      deallocate( fld%phys_name, fld%iorder_ele, fld%iflag_monitor)
...
!
      ...
      new_fld%iorder_ele =>       org_fld%iorder_ele
...
!
!
       nullify( fld%phys_name, fld%iorder_ele, fld%iflag_monitor)
