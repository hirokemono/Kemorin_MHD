...
!!      subroutine set_element_4_comm_test(internal_node, numele, ie,   &
...
!
      ...
      real(kind = kreal), intent(inout) ::      xx_test(numnod,3)
!
...
!$omp parallel do
      do inod = 1, internal_node
        ...
        xx_test(inod,1) =   xx(inod,1)
        xx_test(inod,2) =   xx(inod,2)
        xx_test(inod,3) =   xx(inod,3)
      end do
!$omp end parallel do
!
!$omp parallel do
      do inod =  internal_node+1, numnod
        i_gl_test(inod) = 0
        xx_test(inod,1) =   0.0d0
        xx_test(inod,2) =   0.0d0
        xx_test(inod,3) =   0.0d0
      end do
!$omp end parallel do
!
!
...
!
      ...
      real(kind = kreal), intent(inout) ::      xx_test(numele,3)
!
!
...
!$omp parallel do
      do iele = 1, numele
        if(ie(iele,1) .gt. internal_node) then
          i_gl_test(iele) = 0
          xx_test(iele,1) = 0.0d0
          xx_test(iele,2) = 0.0d0
          xx_test(iele,3) = 0.0d0
        else
          i_gl_test(iele) = iele_gl(iele)
          xx_test(iele,1) = x_ele(iele,1)
          xx_test(iele,2) = x_ele(iele,2)
          xx_test(iele,3) = x_ele(iele,3)
        end if
      end do
!$omp end parallel do
!
...
!
     ...
     &        + abs(x_comm(3*inod  ) - x_org(inod,3))
        if(inod_gl(inod) .ne. icomm_gl(inod) .or.                       &
     &      diff .gt. TINY) num_diff_l = num_diff_l + 1
      end do
...
!
     ...
     &        + abs(x_comm(inod,2) - x_org(inod,2))                   &
     &        + abs(x_comm(inod,3) - x_org(inod,3))
        if(inod_gl(inod) .ne. icomm_gl(inod) .or.                       &
     &      diff .gt. TINY)  then
          icou = icou + 1
          id_diff(icou) =        inod
          ...
          x_diff(6*icou-2) =      x_comm(inod,1)
          x_diff(6*icou-1) =      x_comm(inod,2)
          x_diff(6*icou  ) =      x_comm(inod,3)
        end if
      end do
!
