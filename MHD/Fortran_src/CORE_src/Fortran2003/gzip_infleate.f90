...
!
!  -----------------
        subroutine gzip_infleat_tako                                    &
     &           ()      &
     &            BIND(C, name = 'gzip_infleat_tako')
!
        use ISO_C_BINDING
!
...
!
        end subroutine gzip_infleat_tako
!  ---------------------------------------------------------------------
!
      end interface
!
...
!
      ...
      integer(kind = 4), parameter :: i_YEAR = 2021
      character(len = 1), parameter :: MONTH =  char(7)
      character(len = 1), parameter :: DAY =    char(29)
      character(len = 1), parameter :: HOUR =   char(4)
      character(len = 1), parameter :: MINUTE = char(42)
      character(len = 1), parameter :: SECOND = char(37)
!
      integer(kind = 4), parameter :: len_text = 22
      character(len=4), parameter :: TEXT_HEADER = 'tEXt'
      character(len = 22), parameter                                    &
     &    :: SOFTWARE = "Software"//char(0)//"Kemo's viewer"
!
...
!
      integer(kind = kint) :: iflag_filter
!
...
!
      ...
      do iy = 1, size(PNG_HEADER)
        ...
        if(cmp_char .ne. PNG_HEADER(iy)) write(*,*) 'Fail at ', iy
      end do
!
      i_crc = 0
...
!
      ...
      write(id_png) num2bit4_big(ixpixel)
      call crc32_4_png(len(num2bit4_big(ixpixel)), num2bit4_big(ixpixel), i_crc)
      write(id_png) num2bit4_big(iypixel)
      call crc32_4_png(len(num2bit4_big(iypixel)), num2bit4_big(iypixel), i_crc)
...
!
      ...
      write(id_png) num2bit2_big(i_YEAR)
      call crc32_4_png(len(num2bit2_big(i_YEAR)), num2bit2_big(i_YEAR), i_crc)
      write(id_png) MONTH
      call crc32_4_png(len(MONTH), MONTH, i_crc)
      write(id_png) DAY
      call crc32_4_png(len(DAY), DAY, i_crc)
      write(id_png) HOUR
      call crc32_4_png(len(HOUR), HOUR, i_crc)
      write(id_png) MINUTE
      call crc32_4_png(len(MINUTE), MINUTE, i_crc)
      write(id_png) SECOND
      call crc32_4_png(len(SECOND), SECOND, i_crc)
      write(id_png) num2bit4_big(i_crc)
!
      i_crc = 0
...
!------------------------------------------------------------------------
!
      subroutine sub_filter_png_rgba_line(npix_x, i_rgb_line)
!
      implicit none
!
      integer(kind = 4), intent(in) :: npix_x
...
!$omp end parallel do
!
        call  sub_filter_png_rgba_line(npix_x, i_rgb_line)
!
        call  up_filter_png_rgba_line(npix_x, i_rgb_up, i_rgb_line)
!
        call  ave_filter_png_rgba_line(1, j,                            &
     &                               npix_x, i_rgb_up, i_rgb_line)
!
        call  paeth_filter_png_rgba_line(npix_x, i_rgb_up, i_rgb_line)
!
        iflag_filter = iachar(png_rgb(1+(j-1)*(3*npix_x+1)))
!        write(*,*) j, iflag_filter
!
!$omp parallel do private(i,icou)
        do i = 1, npix_x
          icou = 1 + 3*(i-1) + (j-1)*(3*npix_x+1)
          png_rgb(icou+1:icou+3) = char(i_rgb_line(1:3,i))
        end do
!$omp end parallel do
!
        call  unfilter_png_rgba_line(iflag_filter, i, j,                &
     &                               npix_x, i_rgb_up, i_rgb_line)
!
      end do
      deallocate(i_rgb_line, i_rgb_up)
