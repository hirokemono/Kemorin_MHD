...
!!      subroutine read_gz_itp_table_file_b(file_name, id_rank,         &
...
!
      ...
      if(zbuf_itp%ierr_zlib .gt. 0) go to 99
!
      call write_gz_itp_table_org_b(id_rank, IO_itp_org, zbuf_itp)
      if(zbuf_itp%ierr_zlib .gt. 0) go to 99
      call write_gz_itp_coefs_org_b(IO_itp_org, zbuf_itp)
      if(zbuf_itp%ierr_zlib .gt. 0) go to 99
!
  99  continue
...
!
      ...
      if(zbuf_itp%ierr_zlib .ne. 0) goto 99
!
      call read_gz_itp_domain_org_b(zbuf_itp, n_rank_file, IO_itp_org)
      if(zbuf_itp%ierr_zlib .gt. 0) goto 99
!
      call read_gz_itp_table_org_b(zbuf_itp, IO_itp_org)
      if(zbuf_itp%ierr_zlib .ne. 0) goto 99
!
