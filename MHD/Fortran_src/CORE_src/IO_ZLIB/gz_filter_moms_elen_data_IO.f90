...
!
!!      subroutine read_ref_filter_param_gz(FPz_f, filter_conf, zbuf)
...
!!     &         (FPz_f, nele, el_e, el_ediff, el_edif2, zbuf)
!!        character, pointer, intent(in) :: FPz_f
...
!!        type(buffer_4_gzip), intent(inout) :: zbuf
!!      subroutine write_elens_nod_gz(FPz_f, nnod,                      &
...
!!     &         (FPz_f, nele, moms, m_diff, m_diff2, zbuf)
!!        character, pointer, intent(in) :: FPz_f
...
!
      call skip_gz_comment_real(FPz_f, filter_conf%f_width(1), zbuf)
      read(zbuf%fixbuf(1),*) filter_conf%f_width(1:zbuf%num_word)
      if(filter_conf%nf_type .gt. zbuf%num_word) then
        call read_gz_multi_real(FPz_f,                                  &
     &                          (filter_conf%nf_type - zbuf%num_word),  &
     &                          filter_conf%f_width(zbuf%num_word+1),   &
     &                          zbuf)
      end if
!
      do ifil = 1, filter_conf%nf_type
...
!
     ...
     &   (FPz_f, nele, el_e%f_x2, el_e%f_y2, el_e%f_z2, zbuf)
      call read_elength_gz                                              &
     &   (FPz_f, nele, el_e%f_xy, el_e%f_yz, el_e%f_zx, zbuf)
!
      call read_mom_coefs_dx_gz(FPz_f, nele,                            &
     &    el_ediff%df_x2, el_ediff%df_y2, el_ediff%df_z2, zbuf)
      call read_mom_coefs_dx_gz(FPz_f, nele,                            &
     &    el_ediff%df_xy, el_ediff%df_yz, el_ediff%df_zx, zbuf)
!
      call read_mom_coefs_dx_gz(FPz_f, nele,                            &
     &    el_edif2%df_x2, el_edif2%df_y2, el_edif2%df_z2, zbuf)
      call read_mom_coefs_dx_gz(FPz_f, nele,                            &
     &    el_edif2%df_xy, el_edif2%df_yz, el_edif2%df_zx, zbuf)
!
...
!
     ...
     &      char(10), char(0)
      call gz_write_textbuf_no_lf(FPz_f, zbuf)
!
      call write_mom_coefs_dx_gz(FPz_f, nele,                           &
     &    el_ediff%df_x2, el_ediff%df_y2, el_ediff%df_z2, zbuf)
!
      write(zbuf%fixbuf(1),'(a,2a1)')                                   &
...
!
      call write_elength_gz                                             &
     &   (FPz_f, nnod, e_x2_nod, e_y2_nod, e_z2_nod, zbuf)
!
...
!
      call write_elength_gz                                             &
     &   (FPz_f, nnod, e_xy_nod, e_yz_nod, e_zx_nod, zbuf)
!
!
      write(zbuf%fixbuf(1),'(a,2a1)')                                   &
...
!
     ...
     &   (FPz_f, nnod, e_x2_nod_dx, e_y2_nod_dx, e_z2_nod_dx, zbuf)
!
      write(zbuf%fixbuf(1),'(a,2a1)')                                   &
...
!
      call write_mom_coefs_dx_gz                                        &
     &   (FPz_f, nnod, e_xy_nod_dx, e_yz_nod_dx, e_zx_nod_dx, zbuf)
!
!
...
!
!
      call read_elength_gz(FPz_f, nele,                                 &
     &    moms%f_x2, moms%f_y2, moms%f_z2, zbuf)
      call read_elength_gz(FPz_f, nele,                                 &
     &    moms%f_xy, moms%f_yz, moms%f_zx, zbuf)
      call read_elength_gz(FPz_f, nele,                                 &
     &    moms%f_x,  moms%f_y,  moms%f_z,  zbuf)
!
!
      call read_mom_coefs_dx_gz                                         &
     &   (FPz_f, nele, m_diff%df_x2, m_diff%df_y2, m_diff%df_z2, zbuf)
      call read_mom_coefs_dx_gz                                         &
     &   (FPz_f, nele, m_diff%df_xy, m_diff%df_yz, m_diff%df_zx, zbuf)
      call read_mom_coefs_dx_gz                                         &
     &   (FPz_f, nele, m_diff%df_x,  m_diff%df_y,  m_diff%df_z,  zbuf)
!
!
      call read_mom_coefs_dx_gz(FPz_f, nele,                            &
     &    m_diff2%df_x2, m_diff2%df_y2, m_diff2%df_z2, zbuf)
      call read_mom_coefs_dx_gz(FPz_f, nele,                            &
     &    m_diff2%df_xy, m_diff2%df_yz, m_diff2%df_zx, zbuf)
      call read_mom_coefs_dx_gz(FPz_f, nele,                            &
     &    m_diff2%df_x,  m_diff2%df_y,  m_diff2%df_z,  zbuf)
!
...
!
      call write_elength_gz                                             &
     &   (FPz_f, nele, moms%f_x2, moms%f_y2, moms%f_z2, zbuf)
!
      write(zbuf%fixbuf(1),'(a,2a1)')                                   &
...
!
      call write_elength_gz                                             &
     &   (FPz_f, nele, moms%f_xy, moms%f_yz, moms%f_zx, zbuf)
!
      write(zbuf%fixbuf(1),'(a,2a1)')                                   &
...
!
      call write_elength_gz                                             &
     &   (FPz_f, nele, moms%f_x, moms%f_y, moms%f_z, zbuf)
!
!
      write(zbuf%fixbuf(1),'(a,2a1)')                                   &
...
!
      call write_mom_coefs_dx_gz                                        &
     &   (FPz_f, nele, m_diff2%df_x, m_diff2%df_y, m_diff2%df_z, zbuf)
!
