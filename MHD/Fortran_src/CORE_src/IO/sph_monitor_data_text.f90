...
!!
!!      function picked_mode_data_text(i_step, time, kr_sph, r_sph,     &
!!     &        l_degree, m_order, ntot_sph_spec, spectr_IO)
!!        integer(kind = kint), intent(in) :: i_step, kr_sph, 
!!        integer(kind = kint), intent(in) :: l_degree, m_order
!!        real(kind = kreal), intent(in) :: r_sph, time
!!        integer(kind = kint), intent(in) :: ntot_sph_spec
!!        real(kind = kreal), intent(in) :: spectr_IO(ntot_sph_spec)
!!        character(len=(4*16 + (ntot_sph_spec+2)*25 + 1))              &
!!     &                                      :: picked_mode_data_text
!!@endverbatim
!
...
!
     ...
     &              ntot_sph_spec, '(1pE25.15e3),a1)'
      write(picked_mode_data_text,fmt_txt) i_step, time, kr_sph, r_sph, &
     &        l_degree, m_order, spectr_IO(1:ntot_sph_spec), char(10)
!
      end function picked_mode_data_text
!
!   --------------------------------------------------------------------
!
