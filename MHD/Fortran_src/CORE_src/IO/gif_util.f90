!
      module gif_util
!
! This module can write a GIF file in GIF89 format.  Originally written by Jos Bergervoet, this version by [[Clive Page]] makes use of Fortran [[Stream Input Output|stream I/O]].
...
!  Check all declarations
      private
!
!  bin_io is used private, no transfer to main program
!
      public  :: writegif
!
!      subroutine writegif (FileName, Pixel, ColorMap, Transparent)
!!
!! Codes pixel-map with palette into GIF format.
...
! output buffer vars
      ...
      integer, parameter, private  ::    nocode = maxcode+1
! definitions for LZW
...
! I/O unit to use
        integer  :: bl   
!   number of bytes to write (to be determined)
      
        if (ibuf > 255) then
          bl = 255       
!   we will write buf[1..255]
        else if (skip /= 0) then
          bl = ibuf      
!   buf[ibuf] is partially used, write buf[1..ibuf]
        else if (ibuf > 1) then
          bl = ibuf-1    
!   write buf[1..ibuf-1], there is no partial byte
        else
          return         
!   nothing to write
        end if
        ...
        buf(1:ibuf-bl) = buf(bl+1:ibuf)
! shift down remaining data
        ibuf = ibuf - bl
        return
...
!------------------------------------------------------------------------
!
      subroutine giflzw(F_unit, Pixel)         
! routine for LZW coding
        ...
        nout=0                         
! for counting the codes going out
        if (blen<2) then
          blen=2                       
! pixel code-length, 2 is minimum for GIF
        end if
        ...
        do j=1, ubound(Pixel,2)
         do i=1, ubound(Pixel,1)
          K = modulo(Pixel(i,j), maxbase+1)   
! take next byte, prevent overflow
          if (i==1 .and. j==1) then
            P = K                      
! first raster byte has one-byte code P
            cycle                      
! for the first byte no further action
          end if
                                       
! Now see if code exists for sequence [.P.]K
          child = follow(P)            
! [.P.]K is "string coded by P" followed by K
          childloop: do
          ...
          if (child /= nocode) then    
! If code for [.P.]K was found, store it in P
            P = child
          else                         
! If not: output P and create code for [.P.]K
            call slicewrite(F_unit, P)
            if (ncod > maxcode) then   
! check if a new code can be added
              call slicewrite(F_unit, CC)      
! If not: tell listener to clear table
              call InitTable()         
! and clear our own table
            else
              if (ncod > curmaxcode) then
                slen = slen+1                    
! New codes will be one bit longer
                curmaxcode = curmaxcode * 2 + 1  
! and more codes are possible
              end if
              endbyte(ncod) = char(K)  
! ncod is the new code for [.P.]K
              follow(ncod) = nocode
              next(ncod) = follow(P)   
! include ncod in the hashing list
              follow(P) = ncod         
!     of codes with same start-sequence
              ncod = ncod+1
            end if
            P = K
          end if
        ...
        call slicewrite(F_unit, P)             
! send the last code to buffer
        call slicewrite(F_unit, EOI)           
! send 'end of image' to buffer
        call flushbuffer(F_unit)       
...
!
      subroutine InitTable()
        ...
        do i=0,maxbase                 
! Start with defining the codes 0..maxbase
          endbyte(i) = char(i)         
! for one-pixel sequences (code=pixelvalue)
        end do                         
! Initially no multi-pixel codes exist
        follow(0:maxbase) = nocode
        next(0:maxbase) = nocode
        CC = maxbase+1                 
! `clear code-tabel', a control code
        EOI = maxbase+2                
! `end of image', another control code
        ncod = CC + 2                  
! ncod = number of currently defined codes
        slen = blen + 1                
! current number of bits to write one code
        curmaxcode = 2**slen - 1       
! currently the highest, until slen increases
        return
...
!------------------------------------------------------------------------
!
      subroutine slicewrite(F_unit, code)      
...
! initiate output buffer
        ...
        accum = accum + code * 2**skip 
! add bits at correct position in accum
        skip = skip + slen             
! slen is current slice length, in bits
        ...
        shiftout: do
          ...
          ibuf = ibuf+1                
! last written buffer-byte is now permanent
          accum = accum / 256          
! remove that byte from accum
          skip = skip-8                
! skip points to next bit to write in accum
        end do shiftout
      
        if (ibuf>255) then
          call flushbuffer(F_unit)           
! won't write unfinished byte in buf[ibuf]
        end if
        return                         
...
!!  print *,'image size', nx, ny, ' colours', maxincol
        do i=1,8                          
! find the bitsize, blen, for pixels
          blen = i
          maxgifcol = 2**blen - 1         
! Number of colors has to be power of 2 
          if (maxgifcol >= maxincol) then
            exit                          
! now blen and maxgifcol are correct
          end if                          
! only op to 256 colors can be 
        end do
         write(F_unit) "GIF89a" 
...
!
        if (present(Transparent)) then
          ...
          s = "!" // char(249) // char(4) // char(1) // char(0)         &
     &                  // char(0) // char(Transparent) // char(0)
          write(F_unit) s(1:8)
! GIF transparent extension
        end if
!
         write(F_unit) ","
! Announce image
!    Now create and write image descriptor
        HasMap = 0
        InfoByte = HasMap * 128 + blen-1
! add 64, if interlaced
!    x_margin, y_margin (not used), image dimensions
        write(F_unit) char2(0), char2(0), char2(nx),                    &
     &      char2(ny), CHAR(InfoByte) 
        call giflzw (F_unit, Pixel)
! now the raster data
!        write(F_unit) CHAR(0), ';'
        write(F_unit) CHAR(0)
        write(F_unit) ';'
! Terminating 0-block ; for GIF
        close(unit=F_unit)
        return
!
