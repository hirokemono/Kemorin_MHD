...
!
      if (itype_ucd_data_file .eq. iflag_bin) then
        call read_alloc_ucd_2_fld_header_b(my_rank, istep_udt)
!
#ifdef ZLIB_IO
      else if(itype_ucd_data_file .eq. iflag_udt_gz) then
        call read_and_allocate_udt_head_gz(my_rank, istep_udt)
      else if(itype_ucd_data_file .eq. iflag_fld_gz) then
        call read_alloc_ucd_2_gz_fld_file(my_rank, istep_udt)
#endif
!
      else if(itype_ucd_data_file .eq. iflag_udt) then
        call read_and_alloc_udt_params(my_rank, istep_udt)
      else
        call read_alloc_ucd_2_fld_file(my_rank, istep_udt)
      end if
!
...
!
      if (itype_ucd_data_file .eq. iflag_bin) then
        call read_alloc_ucd_2_fld_file_b(my_rank, istep_udt)
!
#ifdef ZLIB_IO
      else if(itype_ucd_data_file .eq. iflag_udt_gz) then
          call read_and_allocate_udt_file_gz(my_rank, istep_udt)
      else if(itype_ucd_data_file .eq. iflag_fld_gz) then
          call read_alloc_ucd_2_gz_fld_file(my_rank, istep_udt)
#endif
!
      else if(itype_ucd_data_file .eq. iflag_udt) then
        call read_and_alloc_udt_file(my_rank, istep_udt)
      else
        call read_alloc_ucd_2_fld_file(my_rank, istep_udt)
      end if
!
...
!
      if (itype_ucd_data_file .eq. iflag_bin) then
        call read_ucd_2_fld_file_b(my_rank, istep_udt)
!
#ifdef ZLIB_IO
      else if(itype_ucd_data_file .eq. iflag_udt_gz) then
        call read_udt_file_gz(my_rank, istep_udt)
      else if(itype_ucd_data_file .eq. iflag_fld_gz) then
        call read_ucd_2_gz_fld_file(my_rank, istep_udt)
#endif
!
      else if(itype_ucd_data_file .eq. iflag_udt) then
        call read_udt_file(my_rank, istep_udt)
      else
        call read_ucd_2_fld_file(my_rank, istep_udt)
      end if
!
