!>@file   single_pin_ISPACK1_smp.f90
!!@brief  module single_pin_ISPACK1_smp
!!
!!@author H. Matsui
...
!!
!!@verbatim
!!      subroutine single_pin_FTTRUF_smp(Nsmp, Nstacksmp, M, Nfft, X,   &
!!     &          IT_ispack, T_ispack, WORK_ispack,                     &
...
!!
!! ------------------------------------------------------------------
!!      subroutine single_pin_FTTRUB_smp(Nsmp, Nstacksmp, M, Nfft, X,   &
!!     &          IT_ispack, T_ispack, WORK_ispack,                     &
!!     &          elapsed_fft, elapsed_cpy)
!!        integer(kind = kint), intent(in) ::  Nsmp
!!        integer(kind = kint_gl), intent(in) :: Nstacksmp(0:Nsmp)
...
!
      end subroutine single_pin_FTTRUF_smp
!
! ------------------------------------------------------------------
!
      subroutine single_pin_FTTRUB_smp(Nsmp, Nstacksmp, M, Nfft, X,     &
...
!
          st_c = OMP_GET_WTIME()
!          X(1:2,j) = X(1:2,j)
          X(3:Nfft-1:2,j) =  half * X(3:Nfft-1:2,j)
          X(4:Nfft:  2,j) = -half * X(4:Nfft:  2,j)
          ed_c = st_c + OMP_GET_WTIME() - st_c
!
          st_f = OMP_GET_WTIME()
          call FTTRUB(ione, Nfft, X(1,j), WORK_ispack(1,ismp),          &
     &                IT_ispack(1), T_ispack(1))
...
!
      ...
      elapsed_cpy = elapsed_cpy + ed_c / dble(Nsmp)
!
      end subroutine single_pin_FTTRUB_smp
!
! ------------------------------------------------------------------
!
      end module single_pin_ISPACK1_smp
