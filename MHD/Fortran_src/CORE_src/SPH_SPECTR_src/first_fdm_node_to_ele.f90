...
!
     ...
     &   (sph_rj%nidx_rj(1), mat_fdm, fdm_1st_ele%fdm(1))
      deallocate(mat_fdm)
!
...
! -----------------------------------------------------------------------
!
      subroutine copy_first_fdm_node_to_ele(nri, mat_fdm, fdm1)
!
      integer(kind = kint), intent(in) :: nri
...
!$omp parallel do private (k)
      do k = 1, nri-1
        fdm1%dmat(k,-1) = mat_fdm(2,1,k)
        fdm1%dmat(k, 0) = mat_fdm(2,2,k)
      end do
!$omp end parallel do
!
