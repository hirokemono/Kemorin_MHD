...
!      subroutine nod_r_2nd_fdm_coef_equi(dr_1, mat_fdm)
!*
!***********************************************************************
!
...
!
     ...
     &      radius_1d_rj_r(1), mat_fdm_2(1,1,1))
        ...
        call nod_r_2nd_fdm_coef_equi                                    &
     &      (ione, dr_1d_rj(1,0), mat_fdm_2(1,1,1))
      end if
!
      do kr = 2, nidx_rj(1)
        call nod_r_2nd_fdm_coef_equi                                    &
     &      (kr, dr_1d_rj(kr,0), mat_fdm_2(1,1,kr))
      end do
!
...
!
      ...
      mat_taylor_3(2,3) = half * dr_p1*dr_p1
!
...
! -----------------------------------------------------------------------
!
      subroutine nod_r_2nd_fdm_coef_equi(kr, dr_1, mat_fdm)
!
      use cal_inverse_small_matrix
!
      integer(kind = kint), intent(in) :: kr
      real(kind = kreal), intent(in) :: dr_1
      real(kind = kreal), intent(inout) :: mat_fdm(3,3)
!
      real(kind = kreal) :: mat_taylor_3(3,3)
      integer(kind = kint) :: ierr
...
!
      mat_taylor_3(2,1) = one
      mat_taylor_3(2,2) = dr_1
      mat_taylor_3(2,3) = half * dr_1*dr_1
!
      mat_taylor_3(3,1) = one
      mat_taylor_3(3,2) =-dr_1
      mat_taylor_3(3,3) = half * dr_1*dr_1
!
      call cal_inverse_33_matrix(mat_taylor_3, mat_fdm, ierr)
!
      if(ierr .eq. 1) then
        write(*,*) 'singular matrix at nod_r_2nd_fdm_coef_equi', kr
      end if
!
      end subroutine nod_r_2nd_fdm_coef_equi
!
! -----------------------------------------------------------------------
!
