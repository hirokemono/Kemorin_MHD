...
!
     ...
     &   (id_rank, bbuf1, ele_mesh_IO%comm)
      if(bbuf1%ierr_bin .gt. 0) go to 99
!      call read_element_geometry_b                                     &
!     &   (bbuf1, ele_mesh_IO%node, ele_mesh_IO%sfed)
!
  99  continue
...
!
      call open_read_binary_file(file_name, id_rank, bbuf1)
      if(bbuf1%ierr_bin .ne. 0) goto 99
      call read_surface_connection_b(id_rank, bbuf1,                    &
     &    surf_mesh_IO%comm, surf_mesh_IO%ele, surf_mesh_IO%sfed)
      if(bbuf1%ierr_bin .gt. 0) go to 99
!      call read_surface_geometry_b                                     &
!     &   (bbuf1, surf_mesh_IO%node, surf_mesh_IO%sfed)
!
  99  continue
      call close_binary_file
      ierr = bbuf1%ierr_bin
...
!
      call open_read_binary_file(file_name, id_rank, bbuf1)
      if(bbuf1%ierr_bin .ne. 0) goto 99
      call read_edge_connection_b(id_rank, bbuf1,                       &
     &    edge_mesh_IO%comm, edge_mesh_IO%ele, edge_mesh_IO%sfed)
      if(bbuf1%ierr_bin .gt. 0) go to 99
!      call read_edge_geometry_b                                        &
!     &    (bbuf1, edge_mesh_IO%node, edge_mesh_IO%sfed)
!
  99  continue
      call close_binary_file
      ierr = bbuf1%ierr_bin
...
!
     ...
     &   (id_rank, ele_mesh_IO%comm, bbuf1)
      if(bbuf1%ierr_bin .gt. 0) go to 99
!      call write_element_geometry_b                                    &
!     &   (ele_mesh_IO%node, ele_mesh_IO%sfed, bbuf1)
!
  99  continue
      call close_binary_file
      ierr = bbuf1%ierr_bin
...
!
     ...
     &    surf_mesh_IO%ele, surf_mesh_IO%sfed, bbuf1)
      if(bbuf1%ierr_bin .ne. 0) go to 99
!      call write_surface_geometry_b                                    &
!     &   (surf_mesh_IO%node, surf_mesh_IO%sfed, bbuf1)
  99  continue
      call close_binary_file
      ierr = bbuf1%ierr_bin
!
...
!
     ...
     &    edge_mesh_IO%ele, edge_mesh_IO%sfed, bbuf1)
      if(bbuf1%ierr_bin .gt. 0) go to 99
!      call write_edge_geometry_b                                       &
!     &   (edge_mesh_IO%node, edge_mesh_IO%sfed, bbuf1)
  99  continue
      call close_binary_file
      ierr = bbuf1%ierr_bin
!
