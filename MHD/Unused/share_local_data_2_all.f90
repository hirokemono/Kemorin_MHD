...
!
      call calypso_mpi_allgather_real(real_send(1), int(nmax),          &
     &                                real_recv(1), int(nmax))
!
      do ip = 1, nprocs
        do i = 1, num_each(ip)
          inod = i + istack(ip-1)
...
!
      call calypso_mpi_allgather_real(real_send(1), int(isix*nmax),     &
     &                                real_recv(1), int(isix*nmax))
!
      do ip = 1, nprocs
        do i = 1, num_each(ip)
          inod = i + istack(ip-1)
