...
!
      write(zbuf1%fixbuf(1),'(a,2a1)') '# ----- rotate.dat -----',      &
     &                        char(10), char(0)
      call gz_write_textbuf_no_lf(zbuf1)
      write(zbuf1%fixbuf(1),'(a,2a1)') '#  truncation',                 &
     &                                char(10), char(0)
      call gz_write_textbuf_no_lf(zbuf1)
      write(zbuf1%fixbuf(1),'(i16,2a1)')  ltr_cor_IO, char(10), char(0)
      call gz_write_textbuf_no_lf(zbuf1)
!
      j1 = 2
      write(zbuf1%fixbuf(1),'(a,2a1)') '# j1, l2_gl, j3_gl, Ki/pi',     &
     &                        char(10), char(0)
      call gz_write_textbuf_no_lf(zbuf1)
      do j3 = 1 ,jmax_cor_IO
        do j2 = 1, 2
          write(zbuf1%fixbuf(1),'(3i16,1pE25.15e3,2a1)')                &
     &        j1, jgl_kcor_IO(j3,j2,j1), j3, gk_cor_IO(j3,j2,j1),       &
     &        char(10), char(0)
          call gz_write_textbuf_no_lf(zbuf1)
        end do
      end do
      write(zbuf1%fixbuf(1),'(a,2a1)') '# j1, l2_gl, j3_gl, Li/pi',     &
     &                                char(10), char(0)
      call gz_write_textbuf_no_lf(zbuf1)
      do j3 = 1 ,jmax_cor_IO
        write(zbuf1%fixbuf(1),'(3i16,1pE25.15e3,2a1)')                  &
...
!
      ...
      read(zbuf1%fixbuf(1),*) itmp, jgl_kcor_IO(1,1,j1), itmp,          &
     &                gk_cor_IO(1,1,j1)
      call get_one_line_from_gz_f(zbuf1)
      read(zbuf1%fixbuf(1),*) itmp, jgl_kcor_IO(1,2,j1), itmp,          &
     &                gk_cor_IO(1,2,j1)
      do j3 = 2 ,jmax_cor_IO
        do j2 = 1, 2
          call get_one_line_from_gz_f(zbuf1)
          read(zbuf1%fixbuf(1),*) itmp, jgl_kcor_IO(j3,j2,j1), itmp,    &
     &                gk_cor_IO(j3,j2,j1)
        end do
      end do
!
      call skip_gz_comment_int(itmp, zbuf1)
      read(zbuf1%fixbuf(1),*) itmp, jgl_lcor_IO(1,1,j1), itmp,          &
     &                el_cor_IO(1,1,j1)
      do j3 = 2 ,jmax_cor_IO
        call get_one_line_from_gz_f(zbuf1)
        read(zbuf1%fixbuf(1),*) itmp, jgl_lcor_IO(j3,1,j1), itmp,       &
     &                el_cor_IO(j3,1,j1)
      end do
!*
!
     ...
     &                          gk_cor_IO(1,1,j1)
        do j2 = 2, 4
          call get_one_line_from_gz_f(zbuf1)
          read(zbuf1%fixbuf(1),*) itmp, jgl_kcor_IO(1,j2,j1), itmp,     &
     &                gk_cor_IO(1,j2,j1)
        end do
        do j3 = 2, jmax_cor_IO
          do j2 = 1, 4
...
!*
     ...
     &                 el_cor_IO(1,1,j1)
        call get_one_line_from_gz_f(zbuf1)
        read(zbuf1%fixbuf(1),*) itmp, jgl_lcor_IO(1,2,j1), itmp,        &
     &                el_cor_IO(1,2,j1)
        do j3 = 2, jmax_cor_IO
          do j2 = 1, 2
            call get_one_line_from_gz_f(zbuf1)
            read(zbuf1%fixbuf(1),*) itmp, jgl_lcor_IO(j3,j2,j1), itmp,  &
     &                el_cor_IO(j3,j2,j1)
          end do
        end do
      end do
