...
!!     &         (FPz_f, num_pe, view_nod_grps, zbuf)
!!        character, pointer, intent(in) :: FPz_f
...
!!     &         (FPz_f, num_pe, view_ele_grps, zbuf)
!!        character, pointer, intent(in) :: FPz_f
...
!!     &         (FPz_f, num_pe, view_sf_grps, zbuf)
!!        character, pointer, intent(in) :: FPz_f
...
!
     ...
     &   (FPz_f, ngrp_pe, domain_grps%node_grp%istack_sf(1), zbuf)
      do ip = 1, num_pe
        ist = domain_grps%node_grp%istack_sf(ip-1)
        num = domain_grps%node_grp%istack_sf(ip) - ist
        if(num .gt. 0) then
          call write_gz_multi_int_8i16                                  &
     &       (FPz_f, num, domain_grps%node_grp%item_sf(ist+1), zbuf)
        end if
      end do
!
...
!
      call write_gz_multi_int_8i16                                      &
     &   (FPz_f, ngrp_pe, domain_grps%surf_grp%istack_sf(1), zbuf)
      do ip = 1, num_pe
        ist = domain_grps%surf_grp%istack_sf(ip-1)
        num = domain_grps%surf_grp%istack_sf(ip) - ist
        if(num .gt. 0) then
          call write_gz_multi_int_8i16                                  &
     &       (FPz_f, num, domain_grps%surf_grp%item_sf(ist+1), zbuf)
        end if
      end do
!
...
!
      call write_gz_multi_int_8i16                                      &
     &   (FPz_f, ngrp_pe, domain_grps%edge_grp%istack_sf(1), zbuf)
      do ip = 1, num_pe
        ist = domain_grps%edge_grp%istack_sf(ip-1)
        num = domain_grps%edge_grp%istack_sf(ip) - ist
        if(num .gt. 0) then
          call write_gz_multi_int_8i16                                  &
     &       (FPz_f, num, domain_grps%edge_grp%item_sf(ist+1), zbuf)
      end do
!
...
!
     ...
     &     char(10), char(0)
      call gz_write_textbuf_no_lf(FPz_f, zbuf)
!
      call write_viewer_group_data_gz                                   &
     &   (FPz_f, num_pe, view_nod_grps%num_grp,                         &
...
!
!
      call skip_gz_comment_int(FPz_f, view_nod_grps%num_grp, zbuf)
!
      call alloc_merged_node_grps_stack(num_pe, view_nod_grps)
!
      call read_gz_multi_int(FPz_f, (num_pe*view_nod_grps%num_grp),     &
...
!
      call read_viewer_group_item_gz                                    &
     &   (FPz_f, num_pe, view_nod_grps%num_grp,                         &
     &    view_nod_grps%grp_name, view_nod_grps%node_grp, zbuf)
!
...
!
      call write_viewer_group_data_gz                                   &
     &   (FPz_f, num_pe, view_ele_grps%num_grp,                         &
...
!
      call skip_gz_comment_int(FPz_f, view_ele_grps%num_grp, zbuf)
...
!
      ...
      call read_gz_multi_int(FPz_f, (num_pe*view_ele_grps%num_grp),     &
...
!
      call write_viewer_group_data_gz                                   &
     &   (FPz_f, num_pe, view_sf_grps%num_grp,                          &
...
!
      call skip_gz_comment_int(FPz_f, view_sf_grps%num_grp, zbuf)
...
!
      ...
      call read_gz_multi_int(FPz_f, (num_pe*view_sf_grps%num_grp),      &
...
!
     ...
     &         (FPz_f, num_pe, ngrp, name, group, zbuf)
!
      use gzip_file_access
      use gz_data_IO
!
...
!
      ...
      do i = 1, ngrp
        ist = (i-1) * num_pe
        call write_gz_multi_int_8i16                                    &
     &     (FPz_f, ngrp_pe, group%istack_sf(ist+1), zbuf)
      end do
!
      if (ngrp .gt. 0) then
        do i = 1, ngrp
...
!
          if(num .le. 0) then
            ...
            do ip = 1, num_pe
              ...
              if(num .gt. 0) then
                call write_gz_multi_int_8i16                            &
     &             (FPz_f, num, group%item_sf(ist+1), zbuf)
              end if
            end do
          end if
        end do
...
!
      do i = 1, ngrp
        ...
        num = group%istack_sf(num_pe*i)                                 &
     &       - group%istack_sf(num_pe*(i-1))
        call skip_gz_comment_chara(FPz_f, name(i), zbuf)
        call read_gz_multi_int(FPz_f, num, group%item_sf(ist), zbuf)
      end do
!
