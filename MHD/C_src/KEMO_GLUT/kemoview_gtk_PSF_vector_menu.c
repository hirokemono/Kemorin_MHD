...
void add_gtk_psf_vector_menu(struct colormap_view *color_vws, GtkWidget *box){
	...
	index = append_ci_item_to_tree(index, "Full", FULL_COMPONENT, child_model);
	index = append_ci_item_to_tree(index, "Tangential", TANGENTIAL_COMPONENT, child_model);
	
	combobox_vecmode = gtk_combo_box_new_with_model(child_model);
	...
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(combobox_vecmode), child_model, TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combobox_vecmode), child_model,
	...
	gtk_box_pack_start(GTK_BOX(vbox_iso), switch_1, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox_iso), combobox_vecmode, FALSE, FALSE, 0);
