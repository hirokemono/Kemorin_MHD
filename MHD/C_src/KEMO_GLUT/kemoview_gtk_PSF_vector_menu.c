...
static void set_vector_mode_cb(GtkComboBox *combobox_vecmode, gpointer user_data)
{
	...
	kemoview_set_PSF_tangential_vec_mode(index_mode, kemo_gl->kemoview_data);
    draw_full_gl(kemo_gl);
    return;
}
...
static void set_ref_vector_CB(GtkWidget *entry, gpointer user_data)
{
	...
	kemoview_get_each_PSF_color_w_exp(kemo_gl->kemoview_data, ISET_PSF_REFVECT,
...
static void set_vect_increment_CB(GtkWidget *entry, gpointer user_data)
{
	...
	int i_digit;
	double current_value;
	kemoview_get_each_PSF_color_w_exp(kemo_gl->kemoview_data, ISET_VECTOR_INC,
                                      &current_value, &i_digit);
...
static void set_vector_width_CB(GtkWidget *entry, gpointer user_data)
{
	...
	int i_digit;
	double current_value;
	kemoview_get_each_PSF_color_w_exp(kemo_gl->kemoview_data, ISET_PSF_V_THICK,
                                      &current_value, &i_digit);
...
void set_gtk_psf_vector_menu(struct kemoviewer_gl_type *kemo_gl,
	...
	iflag = kemoview_get_PSF_draw_flags(kemo_gl->kemoview_data, PSFTANVEC_TOGGLE);
	...
	iflag = kemoview_get_PSF_color_param(kemo_gl->kemoview_data, ISET_VECTOR_COLOR);
...
GtkWidget * make_gtk_psf_vector_menu(struct kemoviewer_gl_type *kemo_gl, GtkWidget *window,
	...
	g_signal_connect(G_OBJECT(psf_vector_menu->combobox_vecmode), "changed", 
				G_CALLBACK(set_vector_mode_cb), (gpointer) kemo_gl);
	...
	g_signal_connect(G_OBJECT(psf_vector_menu->combobox_veccolor), "changed", 
				G_CALLBACK(set_vector_color_cb), (gpointer) kemo_gl);
	...
	g_signal_connect(psf_vector_menu->spin_vect_width, "value-changed",
					 G_CALLBACK(set_vector_width_CB), (gpointer) kemo_gl);
	...
	g_signal_connect(psf_vector_menu->spin_width_digit, "value-changed",
					 G_CALLBACK(set_width_digit_CB), (gpointer) kemo_gl);
