...
void sel_lc_write_rotate_views(GLFWwindow *glfw_win,
    ...
    if(iflag_img == SAVE_QT_MOVIE){
        if(kemoview_get_quilt_nums(kemo_sgl, ISET_QUILT_MODE) > 0){
            write_rotate_quilt_movie(glfw_win, kemo_sgl, kemo_gl,
                                     ied_deg, inc_deg,
                                     image_prefix, i_fps, 
                                     npix_x, npix_y, image);
        }else{
            write_rotate_movie(glfw_win, kemo_sgl, kemo_gl,
                               ied_deg, inc_deg,
                               image_prefix, i_fps, 
                               npix_x, npix_y, image);
        };
    ...
    if(kemoview_get_quilt_nums(kemo_sgl, ISET_QUILT_MODE) > 0){
        write_rotate_quilt_views(glfw_win, kemo_sgl, kemo_gl,
                                 ied_deg, inc_deg,
                                 iflag_img, image_prefix,
                                 npix_x, npix_y, image);
    }else{
        write_rotate_views(glfw_win, kemo_sgl, kemo_gl,
                           ied_deg, inc_deg,
                           iflag_img, image_prefix,
                           npix_x, npix_y, image);
	}
...
void sel_lc_write_evolution_views(GLFWwindow *glfw_win,
    ...
    if(iflag_img == SAVE_QT_MOVIE){
        if(kemoview_get_quilt_nums(kemo_sgl, ISET_QUILT_MODE) != 0){
            write_evolution_quilt_movie(glfw_win, kemo_sgl, kemo_gl,
                                        ist_udt, ied_udt, inc_udt,
                                        image_prefix, i_fps,
                                        npix_x, npix_y, image);
        }else{
            write_evolution_movie(glfw_win, kemo_sgl, kemo_gl,
                                  ist_udt, ied_udt, inc_udt,
                                  image_prefix, i_fps,
                                  npix_x, npix_y, image);
        }
	...
	if(kemoview_get_quilt_nums(kemo_sgl, ISET_QUILT_MODE) != 0){
        write_evolution_quilt_views(glfw_win, kemo_sgl, kemo_gl,
                                    ist_udt, ied_udt, inc_udt,
                                    iflag_img, image_prefix,
                                    npix_x, npix_y, image);
	}else{
        write_evolution_views(glfw_win, kemo_sgl, kemo_gl,
                              ist_udt, ied_udt, inc_udt,
                              iflag_img, image_prefix,
                              npix_x, npix_y, image);
	};
