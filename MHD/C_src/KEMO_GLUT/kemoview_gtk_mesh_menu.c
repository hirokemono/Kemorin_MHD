...
static void subdoain_distance_CB(GtkWidget *entry, gpointer data)
{
	...
	if(dist >= 0.0) kemoview_set_domain_distance(dist, kemo_gl->kemoview_data);
	kemoview_draw_with_modified_domain_distance(kemo_gl->kemoview_data);
...
static void node_size_CB(GtkWidget *entry, gpointer data)
{
	...
	kemoview_get_node_diamater(kemo_gl->kemoview_data, &current_size, &i_digit);
	kemoview_set_node_diamater(floatvalue, i_digit, kemo_gl->kemoview_data);
...
static void node_digit_CB(GtkWidget *entry, gpointer data)
{
	...
	kemoview_get_node_diamater(kemo_gl->kemoview_data,
                               &current_size, &i_digit);
...
static void num_color_loop_CB(GtkWidget *entry, gpointer data)
{
	...
	kemoview_set_num_of_color_loop(nloop, kemo_gl->kemoview_data);
...
static void set_mesh_color_mode_CB(GtkComboBox *combobox_sfcolor, gpointer data)
{
	...
	kemoview_set_mesh_color_mode(index_mode, kemo_gl->kemoview_data);
	
    draw_full_gl(kemo_gl);
	return;
};

void set_gtk_mesh_menu(struct kemoviewer_gl_type *kemo_gl,
                       struct kemoview_mesh_view *mesh_vws){
	double current_size;
	int i_digit;

	int iflag_mode =          kemoview_get_mesh_color_mode(kemo_gl->kemoview_data);
	int current_num_loop =    kemoview_get_num_of_color_loop(kemo_gl->kemoview_data);
	double current_distance = kemoview_get_domain_distance(kemo_gl->kemoview_data);
	kemoview_get_node_diamater(kemo_gl->kemoview_data, &current_size, &i_digit);
...
static void init_gtk_mesh_menu(struct kemoviewer_gl_type *kemo_gl,
                               struct kemoview_mesh_view *mesh_vws,
                               GtkWidget *window){
    ...
    g_signal_connect(mesh_vws->spin_dist, "value-changed",
                     G_CALLBACK(subdoain_distance_CB), (gpointer) kemo_gl);
    ...
    g_signal_connect(mesh_vws->spin_node_size, "value-changed",
                     G_CALLBACK(node_size_CB), (gpointer) kemo_gl);
    ...
    g_signal_connect(mesh_vws->spin_node_digit, "value-changed",
                     G_CALLBACK(node_digit_CB), (gpointer) kemo_gl);
    ...
    g_signal_connect(mesh_vws->spin_num_loop, "value-changed",
                     G_CALLBACK(num_color_loop_CB), (gpointer) kemo_gl);
    ...
    g_signal_connect(G_OBJECT(mesh_vws->combobox_color_mode), "changed",
                     G_CALLBACK(set_mesh_color_mode_CB), (gpointer) kemo_gl);
    
    set_gtk_mesh_menu(kemo_gl, mesh_vws);
    
    init_domain_draw_expander(kemo_gl, window, mesh_vws->domain_group_gmenu);
    init_nod_group_draw_expander(kemo_gl,  window, mesh_vws->node_group_gmenu);
    init_ele_group_draw_box(kemo_gl,  window, mesh_vws->ele_group_gmenu);
    init_surf_group_draw_box(kemo_gl, window, mesh_vws->surf_group_gmenu);
    
    set_domain_draw_box(kemo_gl, mesh_vws->domain_group_gmenu);
    set_nod_group_draw_box(kemo_gl, mesh_vws->node_group_gmenu);
    set_ele_group_draw_box(kemo_gl, mesh_vws->ele_group_gmenu);
    set_surf_group_draw_box(kemo_gl, mesh_vws->surf_group_gmenu);
...
static void set_mesh_menu_box(struct kemoviewer_gl_type *kemo_gl,
                              struct kemoview_mesh_view *mesh_vws,
                              GtkWidget *meshWin){
    init_mesh_views_4_viewer(kemo_gl->kemoview_data, mesh_vws);
    
    /*  Set buttons */
    g_object_set_data(G_OBJECT(meshWin), "mesh_menu",   (gpointer) mesh_vws);
    g_object_set_data(G_OBJECT(meshWin), "kemoview_gl", (gpointer) kemo_gl);
    ...
    init_gtk_mesh_menu(kemo_gl, mesh_vws, meshWin);
    return;
}
...
void init_mesh_window(struct kemoviewer_gl_type *kemo_gl,
    ...
    set_mesh_menu_box(kemo_gl, mesh_vws, meshWin);
