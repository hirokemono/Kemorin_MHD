...
static void write_rotate_quilt_views(struct kemoviewer_type *kemo_sgl,
	...
	for (i = 0; i< ied_deg; i++) {
        ...
        kemoview_write_window_to_movie(iflag_img, i, image_prefix,
                                       (nimg_column * npix_x),
                                       (nimg_raw * npix_y),
                                       quilt_image, kemo_encode);
	};
    if(iflag_img == SAVE_QT_MOVIE){finalize_FFMPEG_encoder(kemo_encode);}
    free(quilt_image);
    free(image);
	return;
}

static double draw_rotate_views(struct kemoviewer_type *kemo_sgl,
...
double draw_rotate_gl_views(struct kemoviewer_type *kemo_sgl,
    ...
    return draw_rotate_views(kemo_sgl, i_axis, inc_deg, num_rotation);
}
