...
static void toggle_draw_surf_grp_patch_CB(GtkTreeViewColumn *renderer, 
    ...
    kemoview_set_draw_mesh_item(SURF_GRP_FLAG, SURFSOLID_TOGGLE, index_grp, index1_for_toggle);
...
static void toggle_draw_surf_grp_grid_CB(GtkTreeViewColumn *renderer, 
    ...
    kemoview_set_draw_mesh_item(SURF_GRP_FLAG, SURFGRID_TOGGLE, index_grp, index2_for_toggle);
...
static void toggle_draw_surf_grp_node_CB(GtkTreeViewColumn *renderer, gchar *path_str, gpointer user_data){
    ...
    kemoview_set_draw_mesh_item(SURF_GRP_FLAG, SURFNOD_TOGGLE, index_grp, index3_for_toggle);
...
static void set_single_surf_grp_patch_color_CB(GtkButton *button, gpointer user_data)
{
	...
	if(iflag_set > 0) {kemoview_set_mesh_color_code(SURF_GRP_FLAG, SURFSOLID_TOGGLE, colorcode4);};
...
static void set_single_surf_grp_grids_color_CB(GtkButton *button, gpointer user_data)
{
	...
	int iflag_set = kemoview_gtk_colorsel_CB(parent, colorcode4);
	if(iflag_set > 0) {kemoview_set_mesh_color_code(SURF_GRP_FLAG, SURFGRID_TOGGLE, colorcode4);};
...
static void set_single_surf_grp_nodes_color_CB(GtkButton *button, gpointer user_data)
{
	...
	int iflag_set = kemoview_gtk_colorsel_CB(parent, colorcode4);
	if(iflag_set > 0) {kemoview_set_mesh_color_code(SURF_GRP_FLAG, SURFNOD_TOGGLE, colorcode4);};
...
void set_surf_group_draw_box(struct gtk_surf_grp_menu *gtk_surf_group){
	...
	iflag_color = kemoview_get_mesh_color_flag(SURF_GRP_FLAG, SURFSOLID_TOGGLE);
	if(iflag_color == GROUP_COLOR){
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_patch_color), 3);
	} else 	if(iflag_color == DOMAIN_COLOR){
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_patch_color), 2);
	} else 	if(iflag_color == SINGLE_COLOR){
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_patch_color), 1);
	} else {
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_patch_color), 0);
	};
	
	iflag_color = kemoview_get_mesh_color_flag(SURF_GRP_FLAG, SURFGRID_TOGGLE);
	if(iflag_color == GROUP_COLOR){
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_grid_color), 3);
	} else 	if(iflag_color == DOMAIN_COLOR){
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_grid_color), 2);
	} else 	if(iflag_color == SINGLE_COLOR){
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_grid_color), 1);
	} else {
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_grid_color), 0);
	};
	
	iflag_color = kemoview_get_mesh_color_flag(SURF_GRP_FLAG, SURFNOD_TOGGLE);
	if(iflag_color == GROUP_COLOR){
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_node_color), 3);
	} else 	if(iflag_color == DOMAIN_COLOR){
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_node_color), 2);
	} else 	if(iflag_color == SINGLE_COLOR){
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_node_color), 1);
	} else {
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_node_color), 0);
	};
	
	kemoview_get_mesh_color_code(SURF_GRP_FLAG, SURFSOLID_TOGGLE, color4);
	set_color_to_GTK(color4, &gtk_surf_group->gcolor);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(gtk_surf_group->spin_opacity), (double) color4[3]);
	gtk_color_chooser_set_rgba(GTK_COLOR_CHOOSER(gtk_surf_group->button_patch_color),
							   &gtk_surf_group->gcolor);
	
	
	kemoview_get_mesh_color_code(SURF_GRP_FLAG, SURFGRID_TOGGLE, color4);
	set_color_to_GTK(color4, &gtk_surf_group->gcolor);
	gtk_color_chooser_set_rgba(GTK_COLOR_CHOOSER(gtk_surf_group->button_grid_color),
							   &gtk_surf_group->gcolor);
	
	kemoview_get_mesh_color_code(SURF_GRP_FLAG, SURFNOD_TOGGLE, color4);
	set_color_to_GTK(color4, &gtk_surf_group->gcolor);
	gtk_color_chooser_set_rgba(GTK_COLOR_CHOOSER(gtk_surf_group->button_node_color),
							   &gtk_surf_group->gcolor);
	return;
};
...
void add_surf_group_draw_box(struct ci3_clist_view *surf_grp_vws, GtkWidget *window_mesh,
	...
	gtk_surf_group->combobox_patch_color = gtk_combo_box_new_with_model(child_model_patch_color);
	renderer_patch_color = gtk_cell_renderer_text_new();
	gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_patch_color), 2);
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(gtk_surf_group->combobox_patch_color),
							   renderer_patch_color, TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(gtk_surf_group->combobox_patch_color),
								   renderer_patch_color, "text", COLUMN_FIELD_NAME, NULL);
	g_signal_connect(G_OBJECT(gtk_surf_group->combobox_patch_color), "changed", 
	...
	gtk_surf_group->combobox_grid_color = gtk_combo_box_new_with_model(child_model_grid_color);
	renderer_grid_color = gtk_cell_renderer_text_new();
	gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_grid_color), 2);
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(gtk_surf_group->combobox_grid_color), 
							   renderer_grid_color, TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(gtk_surf_group->combobox_grid_color),
								   renderer_grid_color, "text", COLUMN_FIELD_NAME, NULL);
	g_signal_connect(G_OBJECT(gtk_surf_group->combobox_grid_color), "changed", 
	...
	gtk_surf_group->combobox_node_color = gtk_combo_box_new_with_model(child_model_node_color);
	renderer_node_color = gtk_cell_renderer_text_new();
	gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_surf_group->combobox_node_color), 2);
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(gtk_surf_group->combobox_node_color), 
							   renderer_node_color, TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(gtk_surf_group->combobox_node_color),
								   renderer_node_color, "text", COLUMN_FIELD_NAME, NULL);
	g_signal_connect(G_OBJECT(gtk_surf_group->combobox_node_color), "changed", 
				G_CALLBACK(surf_grp_node_colormode_CB), (gpointer) window_mesh);
	
	
	set_color_to_GTK(color4, &gtk_surf_group->gcolor);
	adj_opacity = gtk_adjustment_new(color4[3], 0.0, 1.0, 0.01, 0.01, 0.0);
	gtk_surf_group->spin_opacity = gtk_spin_button_new(GTK_ADJUSTMENT(adj_opacity), 0, 2);
	g_signal_connect(gtk_surf_group->spin_opacity, "value-changed", 
				G_CALLBACK(set_surf_grp_opacity_CB), NULL);
	
	gtk_surf_group->button_patch_color = gtk_color_button_new_with_rgba(&gtk_surf_group->gcolor);
    g_signal_connect(G_OBJECT(gtk_surf_group->button_patch_color), "clicked", 
                     G_CALLBACK(set_single_surf_grp_patch_color_CB), (gpointer) window_mesh);
	
	gtk_surf_group->button_grid_color = gtk_color_button_new_with_rgba(&gtk_surf_group->gcolor);
    g_signal_connect(G_OBJECT(gtk_surf_group->button_grid_color), "clicked", 
				G_CALLBACK(set_single_surf_grp_grids_color_CB), (gpointer) window_mesh);
	
	gtk_surf_group->button_node_color = gtk_color_button_new_with_rgba(&gtk_surf_group->gcolor);
    g_signal_connect(G_OBJECT(gtk_surf_group->button_node_color), "clicked", 
				G_CALLBACK(set_single_surf_grp_nodes_color_CB), (gpointer) window_mesh);
	
	set_surf_group_draw_box(gtk_surf_group);
	...
	gtk_box_pack_start(GTK_BOX(hbox_one_opacity), gtk_surf_group->spin_opacity, TRUE, TRUE, 0);
	...
	gtk_box_pack_start(GTK_BOX(hbox_patch_color), gtk_surf_group->combobox_patch_color, TRUE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(hbox_patch_color), gtk_surf_group->button_patch_color, TRUE, FALSE, 0);
	...
	gtk_box_pack_start(GTK_BOX(hbox_grid_color), gtk_surf_group->combobox_grid_color, TRUE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(hbox_grid_color), gtk_surf_group->button_grid_color, TRUE, FALSE, 0);
	...
	gtk_box_pack_start(GTK_BOX(hbox_node_color), gtk_surf_group->combobox_node_color, TRUE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(hbox_node_color), gtk_surf_group->button_node_color, TRUE, FALSE, 0);
