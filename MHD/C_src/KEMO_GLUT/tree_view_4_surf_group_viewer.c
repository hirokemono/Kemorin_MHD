...
void add_surf_group_draw_box(struct ci3_clist_view *surf_grp_vws, GtkWidget *vbox)
{
	...
	gtk_container_add(GTK_CONTAINER(scrolled_window), surf_grp_vws->tree_view);
	...
	add_sorting_signal_w_label(GTK_TREE_VIEW(surf_grp_vws->tree_view), vbox_iso);
	...
	expander_iso = gtk_expander_new_with_mnemonic("Surface group");
