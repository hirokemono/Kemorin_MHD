...
void Const_VAO_Index_Phong(struct VAO_ids *psf_solid_index_VAO,
                           struct gl_strided_buffer *strided_buf,
                           struct gl_index_buffer *index_buf){
    psf_solid_index_VAO->npoint_draw = index_buf->ntot_vertex;
    if(psf_solid_index_VAO->npoint_draw <= 0) return;

    GLenum ErrorCheckValue = glGetError();
    
//    Const_VAO_4_Phong(psf_solid_index_VAO, strided_buf);

    glDeleteBuffers(1, &psf_solid_index_VAO->id_vertex);

    glGenBuffers(1, &psf_solid_index_VAO->id_vertex);
    glBindBuffer(GL_ARRAY_BUFFER, psf_solid_index_VAO->id_vertex);
    ...
    glBindVertexArray(psf_solid_index_VAO->id_VAO);
    glDeleteBuffers(1, &psf_solid_index_VAO->id_index);
    /* Create index buffer on GPU, and then copy from CPU */
    glGenBuffers(1, &psf_solid_index_VAO->id_index);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, psf_solid_index_VAO->id_index);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, (psf_solid_index_VAO->npoint_draw * sizeof(unsigned int)),
...
void set_PSF_solid_objects_VAO(struct gl_strided_buffer *PSF_solid_buf, struct gl_strided_buffer *PSF_stxur_buf,
                               ...
                               struct gl_strided_buffer *PSF_node_buf, struct gl_index_buffer *PSF_solid_index_buf,
    ...
    Const_VAO_Index_Phong(psf_solid_index_VAO[0],
                          PSF_node_buf, PSF_solid_index_buf);
...
void drawgl_patch_index_phong(struct transfer_matrices *matrices,
    ...
    glBindVertexArray(psf_solid_index_VAO->id_VAO);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, psf_solid_index_VAO->id_index);
    glDrawElements(GL_TRIANGLES, psf_solid_index_VAO->npoint_draw , GL_UNSIGNED_INT, 0);
    return;
}
...
void draw_PSF_solid_objects_VAO(struct transfer_matrices *matrices,
    ...
    drawgl_patch_index_phong(matrices, lights, psf_solid_index_VAO[0], kemo_shaders);
...
void draw_PSF_trans_objects_VAO(struct transfer_matrices *matrices,
    ...
    drawgl_patch_index_phong(matrices, lights, psf_trans_index_VAO[0], kemo_shaders);
