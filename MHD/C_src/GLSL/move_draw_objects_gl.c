...
struct kemoview_VAOs * init_kemoview_VAOs(void){
	...
	kemo_VAOs->grid_VAO = (struct VAO_ids *) malloc(sizeof(struct VAO_ids));
...
void dealloc_kemoview_VAOs(struct kemoview_VAOs *kemo_VAOs){
	...
	for(i=0;i<3;i++){free(kemo_VAOs->map_VAO[i]);};
...
void assign_kemoview_VAOs(struct kemoview_VAOs *kemo_VAOs){
    ...
    glGenVertexArrays(1, &(kemo_VAOs->grid_VAO->id_VAO));
    ...
    for(i=0;i<3;i++){glGenVertexArrays(1, &(kemo_VAOs->map_VAO[i]->id_VAO));};
...
void clear_kemoview_VAOs(struct kemoview_VAOs *kemo_VAOs){
    ...
    Destroy_VAO(kemo_VAOs->grid_VAO);
    ...
    for(i=0;i<3;i++){Destroy_VAO(kemo_VAOs->map_VAO[i]);};
...
/*  Draw mesh data */
		...
		drawgl_lines(view_matrices, kemo_VAOs->grid_VAO, kemo_shaders);
...
static void set_draw_objects_to_VAO(struct kemoview_psf *kemo_psf,
    ...
    if(view_s->iflag_view_type == VIEW_MAP){
        ...
        Const_VAO_4_Phong(kemo_VAOs->axis_VAO, kemo_buffers->axis_buf);
        ...
        Const_VAO_4_Simple(kemo_VAOs->grid_VAO, kemo_buffers->coast_buf);
    ...
    const_texture_VBO(kemo_buffers->cbar_min_buf->image,  kemo_buffers->cbar_min_buf->vertex,  kemo_VAOs->cbar_VAO[1]);
    const_texture_VBO(kemo_buffers->cbar_max_buf->image,  kemo_buffers->cbar_max_buf->vertex,  kemo_VAOs->cbar_VAO[2]);
    const_texture_VBO(kemo_buffers->cbar_zero_buf->image, kemo_buffers->cbar_zero_buf->vertex, kemo_VAOs->cbar_VAO[3]);
    const_texture_VBO(kemo_buffers->timelabel_buf->image, kemo_buffers->timelabel_buf->vertex, kemo_VAOs->time_VAO);
