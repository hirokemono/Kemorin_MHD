...
static void read_group_stack_gz_viewer(void *FP_gzip, int ngrp, int *stack_sf){
	read_listed_item_gz_viewer(FP_gzip, ngrp, &stack_sf[1]);
	return;
};

static void read_group_item_gz_viewer(void *FP_gzip, int npe, int ngrp, 
	...
	for (j = 0; j < ngrp; j++) {
		...
		read_group_stack_gz_viewer(FP_gzip, npe, istack_pe);
		for (i = 0; i < npe; i++){
...
int read_viewer_mesh_gz_c(const char *file_name, struct viewer_mesh *mesh_s){
	...
	/*get_one_line_from_gz_c(FP_gzip1, lbuf, num_word, nchara, buf);*/
	...
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->inod_sf_stack);
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, istack_pe);
...
/*	printf("mesh_s->xx_view %e %e %e\n",
	...
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->isurf_sf_stack);
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, istack_pe);
	
	alloc_sf_type_viewer_s(mesh_s);
	
	read_listed_item_gz_viewer(FP_gzip1, mesh_s->nsurf_viewer, 
							   mesh_s->surftyp_viewer);
...
/*	printf("mesh_s->ie_sf_viewer %d %d %d %d\n",
	...
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->iedge_sf_stack);
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, istack_pe);
...
/*	printf("mesh_s->nnod_domain_sf %d\n", mesh_s->nnod_domain_sf);*/
	...
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->num_pe_sf,
	...
	read_listed_item_gz_viewer(FP_gzip1, mesh_s->nnod_domain_sf,
	...
	read_listed_item_gz_viewer(FP_gzip1, mesh_s->nedge_domain_sf,
	...
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->ngrp_nod_sf, istack_grp);
	mesh_s->nnod_nod_sf = istack_grp[mesh_s->ngrp_nod_sf];
	
	alloc_nod_grp_item_viewer_s(mesh_s);
	read_group_item_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->ngrp_nod_sf,
							  mesh_s->nod_stack_sf, istack_pe, 
	...
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->ngrp_ele_sf, istack_grp);
	mesh_s->nele_ele_sf = istack_grp[mesh_s->ngrp_ele_sf];
	alloc_ele_grp_item_viewer_s(mesh_s);
	
	read_group_item_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->ngrp_ele_sf,
							  mesh_s->ele_stack_sf, istack_pe, 
	...
	read_group_item_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->ngrp_ele_sf,
							  mesh_s->ele_nod_stack_sf, istack_pe, 
	...
	read_group_item_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->ngrp_ele_sf,
							  mesh_s->ele_edge_stack_sf, istack_pe, 
	...
	read_group_stack_gz_viewer(FP_gzip1, mesh_s->ngrp_surf_sf, istack_grp);
	mesh_s->nsurf_surf_sf = istack_grp[mesh_s->ngrp_surf_sf];
	alloc_surf_grp_item_viewer_s(mesh_s);
	
	read_group_item_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->ngrp_surf_sf,
							  mesh_s->surf_stack_sf, istack_pe, 
	...
	read_group_item_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->ngrp_surf_sf, 
							  mesh_s->surf_nod_stack_sf, istack_pe, 
	...
	read_group_item_gz_viewer(FP_gzip1, mesh_s->num_pe_sf, mesh_s->ngrp_surf_sf,
                              mesh_s->surf_edge_stack_sf, istack_pe,
    ...
    close_gzfile_c(FP_gzip1);                           /* close file */
