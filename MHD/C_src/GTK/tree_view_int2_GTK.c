...
static int append_i2_item_to_tree(const int index, const int i1_data, const int i2_data, 
...
int append_i2_list_from_ctl(int index, struct int2_ctl_list *head, 
			GtkTreeView *i2_tree_view){
    GtkTreeModel *model = gtk_tree_view_get_model (i2_tree_view);  
    GtkTreeModel *child_model = gtk_tree_model_sort_get_model(GTK_TREE_MODEL_SORT(model));
    head = head->_next;
	while (head != NULL){
		index = append_i2_item_to_tree(index, head->i2_item->i_data[0], 
					head->i2_item->i_data[1], child_model);
        head = head->_next;
    };
    return index;
}
...
void i2_tree_value1_edited(gchar *path_str, gchar *new_text,
	...
	gtk_tree_model_get(child_model, &iter, COLUMN_FIELD_NAME, &old_int2, -1);
	...
	update_int2_clist_by_c_tbl(old_int1, old_int2, 
							   new_int, old_int2, i2_clist_gtk);
};
...
void i2_tree_value2_edited(gchar *path_str, gchar *new_text,
    ...
    int old_int1, old_int2, new_int2;
    
	sscanf(new_text, "%d", &new_int2);
    gtk_tree_model_get_iter(child_model, &iter, child_path);  
	gtk_tree_model_get(child_model, &iter, COLUMN_FIELD_INDEX, &old_int1, -1);
	gtk_tree_model_get(child_model, &iter, COLUMN_FIELD_NAME, &old_int2, -1);
    ...
    gtk_list_store_set(GTK_LIST_STORE(child_model), &iter,
                       COLUMN_FIELD_NAME, new_int2, -1);
    gtk_tree_path_free(child_path);
    gtk_tree_path_free(path);
	
	
	update_int2_clist_by_c_tbl(old_int1, old_int2, 
							   old_int1, new_int2, i2_clist_gtk);
}

static void column_clicked(GtkTreeViewColumn *column, gpointer user_data){
    GtkTreeView *i2_tree_view = GTK_TREE_VIEW(user_data);
    GtkTreeModel *model;
    gint column_id;
    gint cur_id;
    GtkSortType order;
    ...
    if (gtk_widget_is_focus(GTK_WIDGET(i2_tree_view)) == FALSE) {
        gtk_widget_grab_focus(GTK_WIDGET(i2_tree_view));
    }
    
    column_id = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(column), "column_id"));
    model = gtk_tree_view_get_model(i2_tree_view);
    ...
    if (gtk_tree_sortable_get_sort_column_id(GTK_TREE_SORTABLE(model), &cur_id, &order) == TRUE) {
        ...
        cur_column = gtk_tree_view_get_column(i2_tree_view, cur_id);
...
int add_i2_list_items(GtkTreeView *i2_tree_view, struct int2_clist *i2_clist_gtk){
    ...
    selection = gtk_tree_view_get_selection(i2_tree_view);
...
void delete_i2_list_items(GtkTreeView *i2_tree_view, struct int2_clist *i2_clist_gtk){
    ...
    /* Get path of selected raw */
    /* The path is for tree_model_sort */
    model_to_del = gtk_tree_view_get_model(i2_tree_view);
    ...
    for (cur = g_list_first(reference_list); cur != NULL; cur = g_list_next(cur)) {
        ...
        gtk_tree_model_get(child_model_to_del, &iter, COLUMN_FIELD_NAME, &i2value, -1);
        ...
        /* Update control data */
        del_int2_clist_by_c_tbl(i1value, i2value, i2_clist_gtk);
    }
    g_list_free(reference_list);
}
...
void create_i2_tree_view(GtkTreeView *i2_tree_view, struct int2_clist *i2_clist_gtk, 
    ...
    child_model = gtk_list_store_new(2, G_TYPE_INT, G_TYPE_INT);
    g_object_set_data(G_OBJECT(child_model), "selection_list", NULL);
    
    /* Construct model for sorting and set to tree view */
    model = gtk_tree_model_sort_new_with_model(GTK_TREE_MODEL(child_model));
    gtk_tree_view_set_model(GTK_TREE_VIEW(i2_tree_view), model);
    
    /* First raw */
    column = gtk_tree_view_column_new();
    gtk_tree_view_append_column(i2_tree_view, column);
    gtk_tree_view_column_set_title(column, i2_clist_gtk->i1_name);
    ...
    g_signal_connect(G_OBJECT(column), "clicked", 
                     G_CALLBACK(column_clicked), (gpointer) i2_tree_view);
    */
    /* Second row */
    column = gtk_tree_view_column_new();
    gtk_tree_view_append_column(i2_tree_view, column);
    ...
    */
    ...
    /* sort */
    column = gtk_tree_view_get_column(i2_tree_view, COLUMN_FIELD_INDEX);
...
GtkWidget * i2_list_box_expander(char *array_name_c, GtkWidget *i2_tree_view, 
	...
	selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(i2_tree_view));
 	GtkWidget *label = gtk_label_new("Values");   
    g_object_set_data(G_OBJECT(selection), "label", label);
    
    model = gtk_tree_view_get_model(GTK_TREE_VIEW(i2_tree_view));
