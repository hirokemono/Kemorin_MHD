...
void create_direction_label_tree(GtkWidget *label_tree){
	...
	gtk_tree_view_column_set_title(column, "Direction");
	renderer = gtk_cell_renderer_text_new();
	gtk_tree_view_column_pack_start(column, renderer, TRUE);
	gtk_tree_view_column_set_attributes(column, renderer, "text", COLUMN_FIELD_INDEX, NULL);
/*	g_object_set(renderer, "width", (gint)60, NULL);
...
static void append_scalar_componnet_label(GtkWidget *label_tree){
...
static void append_vector_componnet_label(GtkWidget *label_tree){
    GtkTreeModel *model = gtk_tree_view_get_model (label_tree);  
	GtkTreeModel *child_model = gtk_tree_model_sort_get_model(GTK_TREE_MODEL_SORT(model));
	int i;
	GtkTreeIter iter;
...
static void append_sym_tensor_componnet_label(GtkWidget *label_tree){
    GtkTreeModel *model = gtk_tree_view_get_model (label_tree);  
	GtkTreeModel *child_model = gtk_tree_model_sort_get_model(GTK_TREE_MODEL_SORT(model));
	int i;
	GtkTreeIter iter;
...
static void append_xyz_componnet_label(GtkWidget *label_tree){
    GtkTreeModel *model = gtk_tree_view_get_model (label_tree);  
	GtkTreeModel *child_model = gtk_tree_model_sort_get_model(GTK_TREE_MODEL_SORT(model));
	int i;
	GtkTreeIter iter;
...
static void append_surface_equation_label(GtkWidget *label_tree){
    GtkTreeModel *model = gtk_tree_view_get_model (label_tree);  
	GtkTreeModel *child_model = gtk_tree_model_sort_get_model(GTK_TREE_MODEL_SORT(model));
	int i;
	GtkTreeIter iter;
...
{
	create_direction_label_tree(fields_vws->scalar_label_view);
    append_scalar_componnet_label(fields_vws->scalar_label_view);
	
	create_direction_label_tree(fields_vws->vector_label_view);
	append_vector_componnet_label(fields_vws->vector_label_view);
	
	create_direction_label_tree(fields_vws->sym_tensor_label_view);
	append_vector_componnet_label(fields_vws->sym_tensor_label_view);
	
	create_direction_label_tree(fields_vws->xyz_dir_label_view);
	append_vector_componnet_label(fields_vws->xyz_dir_label_view);
	
	create_direction_label_tree(fields_vws->surface_eq_view);
