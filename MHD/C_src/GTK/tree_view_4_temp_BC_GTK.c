...
void init_temp_bc_views_GTK(struct mhd_model_control_c *model_ctl,
    ...
    bc_vws->bc_ctl_gtk = model_ctl->nbc_ctl;
    return;
}

static void thermal_bc_name_edited_cb(GtkCellRendererText *cell, gchar *path_str, 
			gchar *new_text, gpointer user_data)
{
    struct boundary_condition_view *bc_vws = (struct boundary_condition_view *) user_data;
	
	c2r_tree_type_edited(path_str, new_text, bc_vws->bc_tree_view, 
				&bc_vws->bc_ctl_gtk->bc_T_ctl);
	write_c2r_ctl_list(stdout, 0, "buoyancy changed", 
                              &bc_vws->bc_ctl_gtk->bc_T_ctl);
			...
			gchar *new_text, gpointer user_data)
{
	...
	c2r_tree_name_edited(path_str, new_text, bc_vws->bc_tree_view, 
				&bc_vws->bc_ctl_gtk->bc_T_ctl);
	write_c2r_ctl_list(stdout, 0, "BC type changed", 
                              &bc_vws->bc_ctl_gtk->bc_T_ctl);
			...
			gchar *new_text, gpointer user_data)
{
	...
	c2r_tree_value_edited(path_str, new_text, bc_vws->bc_tree_view,
                    &bc_vws->bc_ctl_gtk->bc_T_ctl);
    write_c2r_ctl_list(stdout, 0, "buoyancy changed", 
                              &bc_vws->bc_ctl_gtk->bc_T_ctl);
...
static void cb_delete_thermal_buo_coef_new(GtkButton *button, gpointer user_data)
{
    ...
    delete_c2r_list_items_GTK(bc_vws->bc_tree_view, 
                             &bc_vws->bc_ctl_gtk->bc_T_ctl);
    write_c2r_ctl_list(stdout, 0, "buoyancy coeffient deleted", 
                              &bc_vws->bc_ctl_gtk->bc_T_ctl);
...
static void cb_add_thermal_buo_coef(GtkComboBox *combobox_add, gpointer user_data)
{
    ...
    bc_vws->index_bc = add_c2r_list_from_combobox_GTK_w_one(bc_vws->index_bc, 
                                                           path, model_comp, bc_vws->bc_tree_view,
                                                           &bc_vws->bc_ctl_gtk->bc_T_ctl);
    write_c2r_ctl_list(stdout, 0, "buoyancy coeffient added", 
                              &bc_vws->bc_ctl_gtk->bc_T_ctl);
    return;
}
...
void init_bc_temp_tree_view(struct boundary_condition_view *bc_vws){
    ...
    g_signal_connect(G_OBJECT(renderer_text), "edited", 
                     G_CALLBACK(thermal_bc_name_edited_cb), bc_vws);
    ...
    bc_vws->index_bc = append_c2r_list_from_ctl(bc_vws->index_bc, 
                                                &bc_vws->bc_ctl_gtk->bc_T_ctl,
                                                bc_vws->bc_tree_view);
...
void add_bc_temp_selection_box(struct boundary_condition_view *bc_vws, GtkWidget *vbox)
{
    ...
    g_signal_connect(G_OBJECT(button_add), "clicked", 
                     G_CALLBACK(cb_add_thermal_buo_coef_new), bc_vws);
    g_signal_connect(G_OBJECT(combobox_add), "changed", 
                     G_CALLBACK(cb_add_thermal_buo_coef), bc_vws);
