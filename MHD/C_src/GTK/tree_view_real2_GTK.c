			...
			gchar *new_text, gpointer user_data)
{
	...
	GtkTreeModel *model = gtk_tree_view_get_model (r2_vws->tree_view);  
    GtkTreeModel *child_model = gtk_tree_model_sort_get_model(GTK_TREE_MODEL_SORT(model));
	...
	update_real2_clist_by_c_tbl(old_value1, old_value2, 
				new_value, old_value2, r2_vws->r2_clist_gtk);
...
static void r2_tree_value2_edited_cb(GtkCellRendererText *cell, gchar *path_str,
    ...
    update_real2_clist_by_c_tbl(old_value1, old_value2, old_value1, new_value, r2_vws->r2_clist_gtk);
...
static void add_r2_list_items_cb(GtkButton *button, gpointer user_data)
{
    ...
    /* 選択されている行のパスを取得する */
    /* パスはツリーモデルソートのもの */
    model_to_add = gtk_tree_view_get_model(r2_vws->tree_view);
    child_model_to_add = gtk_tree_model_sort_get_model(GTK_TREE_MODEL_SORT(model_to_add));
        
    selection = gtk_tree_view_get_selection(r2_vws->tree_view);
    ...
    for (cur = g_list_first(reference_list); cur != NULL; cur = g_list_next(cur)) {
        ...
        /* Add */
		add_real2_clist_before_c_tbl(value1, value2, r2_vws->r2_clist_gtk);
    ...
    write_real2_clist(stdout, 0, "colormap added", r2_vws->r2_clist_gtk);
	return;
}
...
static void delete_r2_list_items_cb(GtkButton *button, gpointer user_data)
{
    ...
    /* 選択されている行のパスを取得する */
    /* パスはツリーモデルソートのもの */
    model_to_del = gtk_tree_view_get_model(r2_vws->tree_view);
    ...
    for (cur = g_list_first(reference_list); cur != NULL; cur = g_list_next(cur)) {
        ...
        /* Update control data */
        del_real2_clist_by_c_tbl(value1, value2, r2_vws->r2_clist_gtk);
    }
    g_list_free(reference_list);
...
void create_real2_tree_view(struct r2_clist_view *r2_vws)
{
    ...
    gtk_tree_view_set_model(GTK_TREE_VIEW(r2_vws->tree_view), model);
    
    /* First raw */
    column = gtk_tree_view_column_new();
    gtk_tree_view_append_column(r2_vws->tree_view, column);
    ...
    g_signal_connect(G_OBJECT(column), "clicked", 
                     G_CALLBACK(column_clicked), r2_vws->tree_view);
...
//                                100, 21474836);
    ...
    /* 1行毎に背景色を変更 */
    gtk_tree_view_set_rules_hint(r2_vws->tree_view, TRUE);
    
    /* ソート */
    column = gtk_tree_view_get_column(r2_vws->tree_view, COLUMN_FIELD_INDEX);
...
void add_real2_list_box_w_addbottun(struct r2_clist_view *r2_vws, GtkWidget *vbox){
    ...
    model = gtk_tree_view_get_model(GTK_TREE_VIEW(r2_vws->tree_view));
