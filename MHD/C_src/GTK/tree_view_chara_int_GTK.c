			...
			GtkCellRenderer *renderer_text, GtkCellRenderer *renderer_spin)
{
    ...
    /* 1行毎に背景色を変更 */
    gtk_tree_view_set_rules_hint(ci_tree_view, TRUE);
			...
			GtkWidget *vbox)
{
    ...
    gtk_container_add(GTK_CONTAINER(scrolled_window), ci_tree_view);
    gtk_box_pack_start(GTK_BOX(vbox), scrolled_window, TRUE, TRUE, 0);
    
    add_sorting_shgnal_w_label(ci_tree_view, hbox);
};
