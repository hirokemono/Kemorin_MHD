...
static void add_ir_list_items_cb(GtkButton *button, gpointer user_data){
	...
	ir_vws->index_bc = add_ir_list_items(GTK_TREE_VIEW(ir_vws->ir_tree_view), ir_clist_gtk);
...
static void delete_ir_list_items_cb(GtkButton *button, gpointer user_data){
    struct ir_clist_view *ir_vws = (struct ir_clist_view *) user_data;
	struct int_real_clist *ir_clist_gtk = (struct int_real_clist *) g_object_get_data(G_OBJECT(button), "int_real_clist");
	delete_ir_list_items(GTK_TREE_VIEW(ir_vws->ir_tree_view), ir_clist_gtk);
	reflesh_f_ctl_ir_array_by_r_list(ir_clist_gtk);
};
...
static void ir_tree_value1_edited_cb(GtkCellRenderer *cell, gchar *path_str,
    ...
    ir_tree_value1_edited(path_str, new_text, GTK_TREE_VIEW(ir_vws->ir_tree_view), ir_clist_gtk);
			...
			gchar *new_text, gpointer user_data)
{
    struct ir_clist_view *ir_vws = (struct ir_clist_view *) user_data;
	struct int_real_clist *ir_clist_gtk = (struct int_real_clist *) g_object_get_data(G_OBJECT(cell), "int_real_clist");
    ir_tree_value2_edited(path_str, new_text, GTK_TREE_VIEW(ir_vws->ir_tree_view), ir_clist_gtk);
	update_f_ctl_ir_array_by_r_list(ir_clist_gtk);
};


GtkWidget *hbox_with_ir_array_checkbox(struct int_real_clist *ir_clist_gtk, struct ir_clist_view *ir_vws){
...
static void init_ir_tree_view(struct int_real_clist *ir_clist_gtk, struct ir_clist_view *ir_vws){
	...
	create_ir_tree_view(GTK_TREE_VIEW(ir_vws->ir_tree_view), ir_clist_gtk, 
                           renderer_value1, renderer_value2);
	
	ir_vws->index_bc = append_ir_list_from_ctl(ir_vws->index_bc,
				&ir_clist_gtk->ir_item_head, GTK_TREE_VIEW(ir_vws->ir_tree_view));
};

GtkWidget *  add_ir_list_box_w_addbottun(struct int_real_clist *ir_clist_gtk, struct ir_clist_view *ir_vws){
	GtkWidget *vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 5);
	init_ir_tree_view(ir_clist_gtk, ir_vws);
	
    GtkWidget *button_add = gtk_button_new_with_label("Add");
    GtkWidget *button_delete = gtk_button_new_with_label("Remove");
	
	GtkWidget * hbox1 = hbox_with_ir_array_checkbox(ir_clist_gtk, ir_vws);
	...
	char * ctmp = strngcopy_from_f(c_int_real_array_block_name(ir_clist_gtk->f_self));
