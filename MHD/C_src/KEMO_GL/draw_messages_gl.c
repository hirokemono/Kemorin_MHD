...
static void count_message_box_VAO(struct cbar_work *cbar_wk, struct VAO_ids *msg_VAO){
	int num_patch;
	
	num_patch = 4 * cbar_wk->num_quad;
	num_patch = num_patch + 2*(cbar_wk->iflag_zero + IFOUR);
	msg_VAO->npoint_draw = ITHREE * num_patch;
	return;
};
...
static void set_message_box_VAO(int iflag_retina, GLfloat text_color[4], GLfloat bg_color[4], 
			struct colormap_params *cmap_s, struct cbar_work *cbar_wk,
			struct VAO_ids *msg_VAO, struct gl_strided_buffer *cbar_buf){
	int inum_quad;
	set_buffer_address_4_patch(msg_VAO->npoint_draw, cbar_buf);
	...
	Const_VAO_4_Simple(msg_VAO, cbar_buf);
	return;
};

static void set_message_text_VAO(int iflag_retina, GLfloat text_color[4], GLfloat bg_color[4], 
			struct colormap_params *cmap_s, struct msg_work *msg_wk, struct cbar_work *cbar_wk,
...
void set_message_VAO(int iflag_retina, GLint nx_win, GLint ny_win,
	...
	for(i=0; i<psf_a->nmax_loaded; i++){
		if(psf_a->iflag_loaded[i] != 0 && psf_m[i]->draw_psf_cbar > 0) {
			...
			count_message_box_VAO(psf_a->cbar_wk, msg_VAO[0]);
			count_message_text_VAO(psf_a->cbar_wk, msg_VAO[1]);
			set_message_box_VAO(iflag_retina, text_color, bg_color, 
						psf_m[i]->cmap_psf_comp[icomp], psf_a->cbar_wk, msg_VAO[0], cbar_buf);
			set_message_text_VAO(iflag_retina, text_color, bg_color, 
						psf_m[i]->cmap_psf_comp[icomp], msg_wk, psf_a->cbar_wk, msg_VAO[1], cbar_buf);
		};
	};
	free(cbar_buf->v_buf);
	free(cbar_buf);
...
void draw_message_VAO(struct msg_work *msg_wk, struct cbar_work *cbar_wk,
	...
	if(msg_VAO[1]->npoint_draw <= 0) return;
	...
	glBindVertexArray(msg_VAO[0]->id_VAO);
	glDrawArrays(GL_TRIANGLES, IZERO, msg_VAO[0]->npoint_draw);
	
	
	glUseProgram(kemo_shaders->simple_texure->programId);
	map_matrix_to_shader(kemo_shaders->simple_texure, orthogonal);
	
	glBindVertexArray(msg_VAO[1]->id_VAO);
	glBindBuffer(GL_ARRAY_BUFFER, msg_VAO[1]->id_vertex);
	...
	glDrawArrays(GL_TRIANGLES, 0, msg_VAO[1]->npoint_draw);
