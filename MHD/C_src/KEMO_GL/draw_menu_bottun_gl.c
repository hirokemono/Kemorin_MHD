...
void draw_menubottun_gl(){
	...
	for(i = 0 ; i < MENU_HEIGHT ; i++) {
		...
		for(j = 0 ; j < MENU_WIDTH ; j++) {
			...
			menubottun_bits[index  ] = r;
			menubottun_bits[index+1] = (( j * 0xFF ) / MENU_WIDTH);
			menubottun_bits[index+2] = ~r;
		}
	}
	for(i = 0 ; i < MENU_HEIGHT ; i++) {
		for(j = 0 ; j < 3 ; j++) {
			...
			for(k=0;k<3;k++) {
				if(menubottun_bits[index+k] < 50*(3-j)){
					menubottun_bits[index+k] = 0;
				} else {
					menubottun_bits[index+k] = menubottun_bits[index+k] - 50*(3-j);
				}
			}
			index = 3*(j + i*MENU_WIDTH);
			for(k=0;k<3;k++) {
	...
	for(i = 0 ; i < 3 ; i++) {
		for(j = 0 ; j < MENU_WIDTH ; j++) {
			index = 3*(j + i*MENU_WIDTH);
			for(k=0;k<3;k++) {
				if(menubottun_bits[index+k] < 50*(3-i)){
					menubottun_bits[index+k] = 0;
				} else {
					menubottun_bits[index+k] = menubottun_bits[index+k] - 50*(3-i);
				}
			}
		...
		*/
		for(j=0;j<32;j++){
			if(bitmap32[j]!=0){
				...
				for(k=0;k<3;k++) menubottun_bits[index+k] = 0;
