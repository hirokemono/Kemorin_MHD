...
!!      subroutine SPH_analyze_linear_conv(MHD_files, iflag_finish,     &
!!     &         SPH_model, MHD_step, sph_fst_IO, SPH_MHD, SPH_WK, m_SR)
!!        type(MHD_file_IO_params), intent(in) :: MHD_files
!!        type(SPH_MHD_model_data), intent(inout) :: SPH_model
!!        type(MHD_step_param), intent(inout) :: MHD_step
!!        type(SPH_mesh_field_data), intent(inout) :: SPH_MHD
...
!
      ...
      call s_sph_initial_licv_control(MHD_files, MHD_step,              &
     &    SPH_MHD%sph, SPH_model%sph_MHD_bc, SPH_model%refs,            &
     &    SPH_MHD%ipol, SPH_MHD%fld, sph_fst_IO)
