      ...
      &           time_cost(particles(i)%group_id)%total_time + itr_num
        ...
        do i = 1, num_domain
          if(time_cost(i)%cnt .ne. 0) then
            time_cost(i)%ave_time                                       &
     &            = time_cost(i)%total_time / time_cost(i)%cnt
...
! -------------------------------------------------------------------
!
      subroutine get_ele_group(eles, ele_id, nod_d_grp, group_id)
!
...
!
        ...
        group_id = nod_d_grp%IGROUP(nod_1_id)
      
      end subroutine get_ele_group
!
! -------------------------------------------------------------------
!
      logical function is_ele_in_group(ele_idx, eles, nod_d_grp)
!
...
! if the two node is not in same group
          if (nod_d_grp%IGROUP(nodes_ele(i))                            &
     &      .ne. nod_d_grp%IGROUP(nodes_ele(i-1))) then
...
! -------------------------------------------------------------------
!
      subroutine next_ele_in_group(eles, eles_idx, nod_d_grp, eleidx)
!
        type(element_data), intent(in) :: eles
        type(domain_group_4_partition), intent(in)  :: nod_d_grp
...
! -------------------------------------------------------------------
!
      subroutine get_particle_ele                                       &
     &         (eles, field_vec, ele_id, nod_d_grp, particle)
!
        type(element_data), intent(in) :: eles
        type(vector_field), intent(in) :: field_vec
        type(domain_group_4_partition), intent(in)  :: nod_d_grp
...
!
        ...
        call get_ele_group(eles, ele_id, nod_d_grp, particle%group_id)
...
!        allocate(particles(num_ptcls))
     ...
     &     (num, irest, elements%numele, num_ptcls)
        ...
        do i = 1, irest
          ...
          ncou = (nl + nr)/2
          nl = nr + 1
          call next_ele_in_group(elements, eleidx, nod_d_grp, ncou)
          call get_particle_ele(elements, field_vec, eleidx(ncou),      &
     &        nod_d_grp, particles(i))
        end do
...
! -------------------------------------------------------------------
!
      subroutine cal_partition_tbl                                      &
     &         (time_cost, num_domain, partition_tbl)
!
...
! -------------------------------------------------------------------
!
      subroutine cal_part_dim_tbl                                       &
     &         (num_domain, ndivide_eb, partition_tbl, part_dim_tbl)
!
        integer(kind = kint), intent(in) :: num_domain
        integer(kind = kint), intent(in) :: ndivide_eb(3)
...
!
        ...
        do i = 1, 3
          ...
          do j = 1, num_domain
            ...
            part_dim_tbl%part_dim(i)%ratio(idx_tmp)                     &
     &          = part_dim_tbl%part_dim(i)%ratio(idx_tmp)               &
     &           + partition_tbl(j)
          end do
        end do
! debug output
        do i = 1, 3
          write(*,*) i, ':',                                            &
     &        part_dim_tbl%part_dim(i)%ratio(1:ndivide_eb(i))
        end do
