...
!
      call alloc_double_numbering(new_fem%mesh%ele%numele,              &
     &                            new_iele_dbl1)
      call double_numbering_4_element(new_fem%mesh%ele, new_ele_comm2,  &
...
!
!
      call repartition_node_group2                                       &
     &   (org_mesh%node, org_groups%nod_grp, part_tbl,                  &
     &    new_mesh%node, new_mesh%nod_comm, new_groups%nod_grp,         &
     &    SR_sig, SR_i)
      call repartition_element_group2                                    &
     &   (org_mesh%ele, org_groups%ele_grp, ele_comm, ele_tbl,          &
     &    new_ele_comm, new_mesh%ele, new_groups%ele_grp, SR_sig, SR_i)
      call repartition_surface_group2                                    &
     &   (org_mesh%ele, org_groups%surf_grp, ele_comm, ele_tbl,         &
     &    new_ele_comm, new_mesh%ele, new_groups%surf_grp, SR_sig, SR_i)
!
      end subroutine s_redistribute_groups2
!
! ----------------------------------------------------------------------
! ----------------------------------------------------------------------
!
      subroutine repartition_node_group2(node, nod_grp, part_tbl,        &
...
!
      end subroutine repartition_node_group2
!
! ----------------------------------------------------------------------
!
...
!
     ...
     &     (new_ele%numele, iflag_new(1), new_ele_grp, icou)
      ...
      call deallocate_group_flags
!
      end subroutine repartition_element_group2
!
! ----------------------------------------------------------------------
!
      subroutine repartition_surface_group2(ele, surf_grp, ele_comm,     &
     &          ele_tbl, new_ele_comm, new_ele, new_surf_grp, SR_sig, SR_i)
!
      use calypso_SR_type
      use solver_SR_type
      use select_copy_from_recv
...
!
     ...
     &       (k1, new_ele%numele, iflag_new(1), new_surf_grp, icou)
      ...
      call deallocate_group_flags
!
      end subroutine repartition_surface_group2
!
! ----------------------------------------------------------------------
!
