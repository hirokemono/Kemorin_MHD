...
static char image_head[LENGTHBUF];
static char matrix_file_name[LENGTHBUF];
...
static void input_matrix_file_panel(int val)
{
	glui_name = editText_filename->get_text();
	sprintf(matrix_file_name, "%s",glui_name.c_str());
	printf("Matrix file: %s\n",matrix_file_name);
	glui_fwin->sync_live();
	return;
}

static void input_colormap_file_panel(int val)
{
	glui_name = editText_filename->get_text();
	sprintf(colormap_file_name, "%s",glui_name.c_str());
	printf("Colormap file: %s\n",colormap_file_name);
	glui_fwin->sync_live();
	return;
}

static void save_colormap_handler(int sel){
	write_current_PSF_colormap_control_file(colormap_file_name);
...
static void load_matrix_handler(int sel){
	load_modelview_file_glut(matrix_file_name);
	GLUI_Master.close_all();
	return;
};
...
static void save_evolution_handler(int sel){
	write_evolution_views_glut(image_fmt, image_head, 
...
static void save_rotation_views_handler(int rot_dir){
	write_rotate_views_glut(image_fmt, image_head, rot_dir);
	GLUI_Master.close_all();
};
...
void set_saveimage_menu_glui(int winid){
	...
	int_box = 0;
	istep = send_current_psf_full_path_header(image_head, &iflag);
...
void write_PSF_colormap_file_glui(int winid){
	...
	glui_fwin = GLUI_Master.create_glui("Save colormap file", 0, 100, 100);
	currentDir = new GLUI_TextBox(glui_fwin, text_current, false, -2);
...
void set_viewmatrix_glui(int winid){
	char current[LENGTHBUF];
	getcwd(current, sizeof(current));
	text_current = current;
	int iflag;
	...
	currentDir = new GLUI_TextBox(glui_fwin, text_current, false, -2);
	editText_filename = new GLUI_EditText( glui_fwin, "File name:	", GLUI_EDITTEXT_TEXT, 
								 matrix_file_name, -1, input_matrix_file_panel );
	file_brouser = new GLUI_FileBrowser(glui_fwin, "File list", GLUI_PANEL_RAISED,
										0, NoSetFilenameCB);
	glui_fwin->add_button("Save", 0, save_matrix_handler);
	editText_filename->set_w(300);
	currentDir->set_w(300);
	currentDir->set_h(20);
	currentDir->disable();
...
void load_viewmatrix_glui(int winid){
	char current[LENGTHBUF];
	getcwd(current, sizeof(current));
	text_current = current;
	int iflag;
	...
	file_brouser = new GLUI_FileBrowser(glui_fwin, "File list", GLUI_PANEL_RAISED,
										0, NoSetFilenameCB);
	glui_fwin->add_button("Load", 0, load_matrix_handler);
	editText_filename->set_w(300);
	currentDir->set_w(300);
	currentDir->set_h(20);
	currentDir->disable();
