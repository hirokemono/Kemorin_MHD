!>@file   test_sgl_cplx_rocFFT_prt.f90
!!@brief  program test_sgl_cplx_rocFFT_prt
!!
!!@author H. Matsui
!!@date Programmed in March, 2026
!
!> @brief Test program of single complex rocFFT with inner series array
!!
!!@verbatim
!! ----------------------------------------------------------------------
...
!! ----------------------------------------------------------------------
!!  begin FFT_test_ctl
!!    output_file_name    'sgl_prt_rocFFT_test.dat'
!!
!!    FFT_length_ctl         128
!!    num_series_ctl          24
...
!! ----------------------------------------------------------------------
!!@endverbatim
      program test_sgl_cplx_rocFFT_prt
!
      use iso_c_binding
      use omp_lib
!
...
!
      ...
      use single_pin_complex_rocFFT
!
      implicit none
!
      character(len = kchara), parameter                                &
     &             :: test_name = 'prt_single_complex_rocFFT'
      character(len = kchara), parameter                                &
     &             :: def_fname = 'sgl_prt_rocFFT_test.dat'
!
...
!
!
      write(*,'(a)') '-----  Test single prt rocFFT  -----'
!
...
!
!   Forward transform
        call single_pin_fwd_rocFFT_r2c(WK_fft, ft1%nfld, ft1%s_k,       &
     &                                 elapsed(2), elapsed(3))
!
        start = OMP_GET_WTIME()
!$omp parallel workshare
...
!
!   Backword transform
        call single_pin_bwd_rocFFT_c2r(WK_fft, ft1%nfld, ft1%f_x,       &
     &                                 elapsed(2), elapsed(3))
...
!
      stop 'finish'
      end program test_sgl_cplx_rocFFT_prt
