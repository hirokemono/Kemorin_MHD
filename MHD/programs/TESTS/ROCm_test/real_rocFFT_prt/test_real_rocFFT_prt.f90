!!>@file   test_real_rocFFT_prt.f90
!!@brief  program test_real_rocFFT_prt
!!
!!@author H. Matsui
!!@date Programmed in March, 2026
!
!>@brief Test program of real rocFFT
...
!! ----------------------------------------------------------------------
!!  begin FFT_test_ctl
!!    output_file_name       'prt_real_rocFFT_test.dat'
!!
!!    FFT_length_ctl         128
!!    num_series_ctl          24
...
!
     ...
     &             :: test_name = 'prt_real_rocFFT'
      character(len = kchara), parameter                                &
     &             :: def_fname = 'prt_real_rocFFT_test.dat'
!
...
!
!   Forward transform
        call multi_pin_fwd_rocFFT_r2r(fwd, WK_fft, ft1%s_k(1,1),        &
     &                                elapsed(2), elapsed(3))
!
        start = OMP_GET_WTIME()
!$omp parallel workshare
...
!
!   Backword transform
        call multi_pin_bwd_rocFFT_r2r(bwd, WK_fft, ft1%f_x,             &
     &                                elapsed(2), elapsed(3))
...
!
      stop 'finish'
      end program test_real_rocFFT_prt
