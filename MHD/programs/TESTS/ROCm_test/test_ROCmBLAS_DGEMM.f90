...
!
! Initialize matrices
      call RANDOM_NUMBER(A_org)
      call RANDOM_NUMBER(B_org)
      call RANDOM_NUMBER(C_org)
!
      call norm_dgemm_matrix_omp_target(lda, k, A_org)
      call norm_dgemm_matrix_omp_target(ldb, n, B_org)
      call norm_dgemm_matrix_omp_target(ldc, n, C_org)
!
! ----  Original mat multi   --------------
...
!
        start = OMP_GET_WTIME()
        call calypso_OpenMP_target_DGEMM(m, n, k, alpha,                &
...
!
     ...
     &                                                      C_ref, C))
      ...
      do iloop = 1, nloop
        write(*,'(a, i3, a, 1pE16.6e3)')   "  Time of ", iloop,         &
     &       "-th OpenMP offloading: ", elapsed(iloop)
      end do
!
! ----  hipfort OpenMP kernel with initialization --------------
      do iloop = 1, nloop
...
!
     ...
     &                                                      C_ref, C))
      ...
      do iloop = 1, nloop
        write(*,'(a, i3, a, 1pE16.6e3)')   "  Time of ", iloop,         &
     &       "-th OpenMP DGEMM kernel: ", elapsed(iloop)
      end do
!
...
!
      do iloop = 1, nloop
        write(*,'(a, i3, a, 1pE16.6e3)')   "  Time of ", iloop,         &
     &       "-th OpenMP DGEMM kernel w/o init: ", elapsed(iloop)
      end do
!
! ----  hipfort kernel --------------
      do iloop = 1, nloop
...
!
     ...
     &                                                      C_ref, C))
      ...
      do iloop = 1, nloop
        write(*,'(a, i3, a, 1pE16.6e3)')   "  Time of ", iloop,         &
     &       "-th hipfort kernel: ", elapsed(iloop)
      end do
!
