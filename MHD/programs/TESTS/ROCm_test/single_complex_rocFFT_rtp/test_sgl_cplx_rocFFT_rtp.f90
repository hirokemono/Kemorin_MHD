!>@file   test_sgl_cplx_rocFFT_rtp.f90
!!@brief  program test_sgl_cplx_rocFFT_rtp
!!
!!@author H. Matsui
!!@date Programmed in March, 2026
!
!> @brief Test program of single complex rocFFT with outer series array
!!
!!@verbatim
!! ----------------------------------------------------------------------
...
!! ----------------------------------------------------------------------
!!  begin FFT_test_ctl
!!    output_file_name    'sgl_rtp_rocFFT_test.dat'
!!
!!    FFT_length_ctl         128
!!    num_series_ctl          24
...
!! ----------------------------------------------------------------------
!!@endverbatim
      program test_sgl_cplx_rocFFT_rtp
!
      use iso_c_binding
      use omp_lib
!
...
!
      ...
      use single_pout_complex_rocFFT
!
      implicit none
!
      character(len = kchara), parameter                                &
     &             :: test_name = 'rtp_single_complex_rocFFT'
      character(len = kchara), parameter                                &
     &             :: def_fname = 'sgl_rtp_rocFFT_test.dat'
!
...
!
!   Forward transform
        call single_pout_fwd_rocFFT_r2c(WK_fft, ft1%nfld, ft1%s_k,      &
     &                                  elapsed(2), elapsed(3))
!
        start = OMP_GET_WTIME()
!$omp parallel workshare
...
!
!   Backword transform
        call single_pout_bwd_rocFFT_c2r(WK_fft, ft1%nfld, ft1%f_x,      &
     &                                  elapsed(2), elapsed(3))
...
!
      stop 'finish'
      end program test_sgl_cplx_rocFFT_rtp
