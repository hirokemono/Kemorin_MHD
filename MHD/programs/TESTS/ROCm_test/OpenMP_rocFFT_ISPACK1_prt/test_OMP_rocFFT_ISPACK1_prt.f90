!>@file   test_OMP_rocFFT_ISPACK1_prt.f90
!!@brief  program test_OMP_rocFFT_ISPACK1_prt
!!
!!@author H. Matsui
...
!! ----------------------------------------------------------------------
!!  begin FFT_test_ctl
!!    output_file_name    'prt_OpenMP_rocFFT_ISPACK1_test.dat'
!!
!!    FFT_length_ctl         128
!!    num_series_ctl          24
...
!! ----------------------------------------------------------------------
!!@endverbatim
      program test_OMP_rocFFT_ISPACK1_prt
!
      use iso_c_binding
      use omp_lib
!
...
!
      ...
      use pin_OMP_rocFFT_ISPACK1
!
      implicit none
!
      character(len = kchara), parameter                                &
     &             :: test_name = 'prt_OpenMP_rocFFT_ISPACK_v097'
      character(len = kchara), parameter                                &
     &             :: def_fname = 'prt_OpenMP_rocFFT_ISPACK1_test.dat'
!
...
!
      write(*,'(a)')                                                    &
     &     '-----  Test prt OpenMP rocFFT and ISPACK v0.97 -----'
!
      call default_FFT_test_parameters(test_name, def_fname,            &
     &                                 fft_test_p1)
      fft_test_p1%CPU_FFT_name = 'ISPACK_V0.97'
!
...
!   Initialize Fourier transform
      start = OMP_GET_WTIME()
      call init_pin_OMP_rocFFT_ISPACK1                                  &
...
!
!   Forward transform
        call pin_fwd_OMP_rocFFT_ISPACK1(ft1%nfld, fft_test_p1%nfld_CPU, &
...
!
!   Backword transform
        call pin_bwd_OMP_rocFFT_ISPACK1(ft1%nfld, fft_test_p1%nfld_CPU, &
...
!   Finalize
      start = OMP_GET_WTIME()
      call finalize_OMP_rocFFT_ISPACK1(fwd, bwd, WK_rocFFT, WK_ISPACK1)
...
!
      stop 'finish'
!
      end program test_OMP_rocFFT_ISPACK1_prt
!
! mpif90 --offload-arch=gfx942 -mcmodel=medium -mcmodel=medium -O3 -g -fopenmp -fopenmp-target-fast  -I. -I/home/hrmatsui/src_kemo/work -I/opt/rocm-7.2.0/include/hipfort/amdgcn -DPNG_OUTPUT -DZLIB_IO -DFFTW3 -D_AMD_ROCM_ -o ./test_OMP_rocFFT_ISPACK1_prt test_OMP_rocFFT_ISPACK1_prt.f90 -L/home/hrmatsui/src_kemo/work -lkemo_core -lkemo_c -lfftpack.5d -L/home/hrmatsui/local/amd/lib -lpng -L/home/hrmatsui/local/amd/lib -lz -L/home/hrmatsui/local/amd/lib -lfftw3 -L/opt/rocm-7.2.0/lib -lrocfft -lrocblas -lhipfort-amdgcn -lamdhip64
