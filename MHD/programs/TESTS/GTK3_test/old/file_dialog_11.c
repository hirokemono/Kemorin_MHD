...
extern void * MHD_block_label_to_c();
extern void * platfortm_block_label_to_c();
...
static void cb_Open(GtkButton *button, gpointer data)
{
  ...
  gchar *folder;
	...
	if( response == GTK_RESPONSE_ACCEPT ){
		...
		c_read_control_sph_SGS_MHD((char *) read_file_name);
...
void draw_MHD_control_list(GtkWidget *window, GtkWidget *vbox0, struct iso_ctl_c *iso_c){
	...
	GtkWidget *expand_PLT = wrap_into_expanded_frame_gtk
			(duplicate_underscore(platfortm_block_label_to_c()), 
			360, 240, window, evo_box);
	
	GtkWidget *expand_MHD = wrap_into_expanded_frame_gtk
			(duplicate_underscore(MHD_block_label_to_c()), 
