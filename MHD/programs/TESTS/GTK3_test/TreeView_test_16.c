...
static void append_model_data(GtkButton *button, gpointer user_data)
{
	...
	gtk_list_store_append(GTK_LIST_STORE(child_model), &iter);
			gtk_list_store_set(GTK_LIST_STORE(child_model), &iter,
					   ...
					   COLUMN_FIELD_NAME, all_fld_list->fld_list->field_name[serial_num],
					   COLUMN_FIELD_MATH, all_fld_list->fld_list->field_math[serial_num],
					   COLUMN_VIZ_FLAG, (gboolean) all_fld_list->iflag_viz[serial_num],
					   COLUMN_MONITOR_FLAG, (gboolean) all_fld_list->iflag_monitor[serial_num],
					   COLUMN_NUM_COMP, all_fld_list->fld_list->num_comp[serial_num],
					   COLUMN_QUADRATURE, all_fld_list->iflag_quad[serial_num],
						-1);
}
...
{
	GtkTreeView *used_tree_view = GTK_TREE_VIEW(user_data);
	GtkTreeModel *model;
	GtkTreeModel *child_model;
	GtkTreeSelection *selection;
	GList *list;
	...
	/* 選択されている行のパスを取得する */
	/* パスはツリーモデルソートのもの */
	model = gtk_tree_view_get_model(used_tree_view);
	selection = gtk_tree_view_get_selection(used_tree_view);
	...
	for (cur = g_list_first(reference_list); cur != NULL; cur = g_list_next(cur)) {
        ...
        gtk_tree_model_get(child_model, &iter, COLUMN_FIELD_NAME, &field_name, -1);
        gtk_tree_model_get(child_model, &iter, COLUMN_FIELD_MATH, &field_math, -1);
        gtk_tree_model_get(child_model, &iter, COLUMN_VIZ_FLAG, &iflag_viz, -1);
        gtk_tree_model_get(child_model, &iter, COLUMN_MONITOR_FLAG, &iflag_monitor, -1);
        gtk_tree_model_get(child_model, &iter, COLUMN_NUM_COMP, &num_comp, -1);
        gtk_tree_model_get(child_model, &iter, COLUMN_QUADRATURE, &iflag_quad, -1);
...
static GtkWidget *create_window(struct field_views *fields_vws, GtkWidget *window)
{
	...
	g_signal_connect(G_OBJECT(button), "clicked", 
				G_CALLBACK(remove_model_data), fields_vws->used_tree_view);
...
//	gtk_widget_set_size_request(scrolled_window_1, 400, 300);
	...
	g_signal_connect(G_OBJECT(button), "clicked", 
				G_CALLBACK(append_model_data), fields_vws->used_tree_view);
...
static void toggle_viz_switch(GtkTreeViewColumn *renderer, 
	...
	update_field_flag_in_ctl(index_field, all_fld_list,
							 &mhd_ctl->model_ctl->fld_ctl->field_list);
}
static void toggle_monitor_switch(GtkTreeViewColumn *renderer, gchar *path_str, gpointer user_data){
    ...
    update_field_flag_in_ctl(index_field, all_fld_list, &mhd_ctl->model_ctl->fld_ctl->field_list);
}
	...
	for(i=0;i<all_fld_list->fld_list->ntot_fields;i++){
		if(all_fld_list->iflag_use[i] > 0){
			...
			gtk_list_store_set(GTK_LIST_STORE(child_model), &iter,
					   ...
					   COLUMN_FIELD_MATH, all_fld_list->fld_list->field_math[i],
					   COLUMN_VIZ_FLAG, (gboolean) all_fld_list->iflag_viz[i],
					   COLUMN_MONITOR_FLAG, (gboolean) all_fld_list->iflag_monitor[i],
					   COLUMN_NUM_COMP, all_fld_list->fld_list->num_comp[i],
					   COLUMN_QUADRATURE, all_fld_list->iflag_quad[i],
						-1);
