			...
			GtkTreeView *tree_view_to_del, GtkTreeView *tree_view_to_add)
{
	...
	for (cur = g_list_first(reference_list); cur != NULL; cur = g_list_next(cur)) {
		...
		if(iflag_if_add == 1){
			...
			add_field_to_ctl(index_field, all_fld_list, &mhd_ctl->model_ctl->fld_ctl->field_list);
		} else {
			printf("Delete field list \n");
			delete_field_in_ctl(index_field, all_fld_list, &mhd_ctl->model_ctl->fld_ctl->field_list);
		};
...
/*	GtkTreeModel *child_model = GTK_TREE_MODEL(user_data);*/
	...
	for(i=0;i<all_fld_list->fld_list->ntot_fields;i++){
		if(all_fld_list->iflag_use[i] > 0) {append_model_data(i, child_model);};
	}
