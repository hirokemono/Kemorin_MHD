			...
			GtkTreeView *tree_view_to_del, GtkTreeView *tree_view_to_add)
{
	...
	for (cur = g_list_first(reference_list); cur != NULL; cur = g_list_next(cur)) {
		...
		if(iflag_if_add == 1){
			...
			add_field_wqflag_to_ctl(index_field, all_fld_list, mhd_ctl->model_ctl->fld_ctl);
		} else {
			printf("Delete field list \n");
			delete_field_wqflag_in_ctl(index_field, all_fld_list, mhd_ctl->model_ctl->fld_ctl);
		};
...
static void create_tree_view_window(GtkButton *button, gpointer user_data)
{
	...
	vbox = create_field_selection_box(fields_vws, window);
