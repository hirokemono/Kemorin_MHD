...
static void remove_model_data(GtkButton *button, gpointer user_data)
{
	...
	/* 選択されている行のパスを取得する */
	/* パスはツリーモデルソートのもの */
	model_to_del = gtk_tree_view_get_model(fields_vws->used_tree_view);
	child_model_to_del = gtk_tree_model_sort_get_model(GTK_TREE_MODEL_SORT(model_to_del));
	
	model_to_add = gtk_tree_view_get_model(fields_vws->unused_field_tree_view);
	child_model_to_add = gtk_tree_model_sort_get_model(GTK_TREE_MODEL_SORT(model_to_add));
	
	selection = gtk_tree_view_get_selection(fields_vws->used_tree_view);
	...
	for (cur = g_list_first(reference_list); cur != NULL; cur = g_list_next(cur)) {
		...
		/* Add */
		gtk_list_store_append(GTK_LIST_STORE(child_model_to_add), &iter);
		gtk_list_store_set(GTK_LIST_STORE(child_model_to_add), &iter,
					   ...
					   COLUMN_VIZ_FLAG, (gboolean) all_fld_list->iflag_viz[index_field],
					   COLUMN_MONITOR_FLAG, (gboolean) all_fld_list->iflag_monitor[index_field],
...
static GtkWidget *create_window(struct field_views *fields_vws, GtkWidget *window)
{
	...
	g_signal_connect(G_OBJECT(button), "clicked", 
				G_CALLBACK(remove_model_data), fields_vws);
