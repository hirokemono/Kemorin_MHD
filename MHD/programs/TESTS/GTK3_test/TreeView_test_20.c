...
static void append_model_data(int index_field, struct all_field_ctl_c *all_fld_list, GtkTreeModel *child_model)
{
	GtkTreeIter iter;
			...
			GtkTreeView *tree_view_to_del, GtkTreeView *tree_view_to_add)
{
	...
	for (cur = g_list_first(reference_list); cur != NULL; cur = g_list_next(cur)) {
		...
		if(iflag_if_add == 1){
			...
			add_field_wqflag_to_ctl(index_field, all_fld_list,
									mhd_ctl->model_ctl->fld_ctl);
		} else {
			printf("Delete field list \n");
			delete_field_wqflag_in_ctl(index_field, all_fld_list,
									   mhd_ctl->model_ctl->fld_ctl);
		};
...
static void toggle_viz_switch(GtkTreeViewColumn *renderer, 
    ...
    update_field_flag_wqflag_in_ctl(index_field, all_fld_list, mhd_ctl->model_ctl->fld_ctl);
}
...
static void create_field_tree_view(struct all_field_ctl_c *all_fld_list, struct field_views *fields_vws)
{
/*	GtkTreeModel *child_model = GTK_TREE_MODEL(user_data);*/
	...
	for(i=0;i<all_fld_list->fld_list->ntot_fields;i++){
		if(all_fld_list->iflag_use[i] > 0) {append_model_data(i, all_fld_list, child_model);};
	}
	
}

static void create_unused_field_tree_view(struct all_field_ctl_c *all_fld_list, struct field_views *fields_vws)
{
/*	GtkTreeModel *child_model = GTK_TREE_MODEL(user_data);*/
	GtkTreeModel *model;
	GtkCellRenderer *renderer;
