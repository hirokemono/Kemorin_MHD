...
!!     &          nfield_edge, ncomp_edge, nnod_edge, nline_on_node)
!!      subroutine set_section_list_in_triangle(nnod_tri, nedge_tri,    &
!!     &          nnod_edge, nline_on_node, iedge_4_line)
...
!!     &          iedge_4_line, coef_on_edge, inod_gl, xx_edge,         &
...
!
      integer(kind = kint) :: icou, inod, iedge
!
!
      icou = 0
      do inod = 1, nnod_tri
        if(inod_on_node(inod) .gt. 0) then
          icou = icou + 1
          iedge_4_line(icou) = inod
        end if
      end do
!
      icou = nline_on_node
...
!
     ...
     &          nfield, ncomptot, xx_tri, iedge_4_tri,                  &
     &          ncomp_tri, tri_phys_name, d_tri, nnod_edge,             &
     &          nline_on_node, iedge_4_line, coef_on_edge, inod_gl,     &
...
!
      integer(kind = kint) :: i1, i2, icou, inod, iedge
!
!
...
!$omp parallel do private(icou,inod)
      do icou = 1, nline_on_node
        inod = iedge_4_line(icou)
        write(*,*) 'inod', icou, inod
        xx_edge(inod,1:3) = xx_tri(inod,1:3)
        d_edge(inod,1:ncomptot) = d_tri(inod,1:ncomptot)
      end do
!$omp end parallel do
!
!$omp parallel do private(icou,iedge,i1,i2)
      do icou = nline_on_node+1, nnod_edge
