...
!!     &         (append_file_name, target_file_name)
!!      subroutine append_sph_layer_spectr_file                         &
!!     &         (append_file_name, target_file_name)
...
!
      sph_IN1%nri_sph = 1
      sph_IN1%num_time_labels = 2
      call gz_read_sph_pwr_vol_head(FPz_f1, id_read_file, flag_gzip1,   &
     &                              sph_lbl_IN1, sph_IN1, zbuf1)
!
      call alloc_sph_espec_name(sph_IN1)
      call sel_read_sph_spectr_name(FPz_f1, id_read_file, flag_gzip1,   &
...
!
      sph_OUT1%nri_sph = 1
      sph_OUT1%num_time_labels = 2
      call gz_read_sph_pwr_vol_head(FPz_f1, id_write_file, flag_gzip1,  &
     &                              sph_lbl_OUT1, sph_OUT1, zbuf1)
!
      call alloc_sph_espec_name(sph_OUT1)
      call sel_read_sph_spectr_name(FPz_f1, id_write_file, flag_gzip1,  &
     &   sph_OUT1%nfield_sph_spec, sph_OUT1%num_labels,                 &
     &   sph_OUT1%ncomp_sph_spec, sph_OUT1%ene_sph_spec_name, zbuf1)
!
      call sel_close_read_gz_stream_file                                &
...
!
     ...
     &      id_write_file, ntot_pick, zbuf1)
        ...
        call pick_copy_sph_vmean_to_end                                 &
...
!
      end subroutine append_sph_volume_mean_file
!
! -----------------------------------------------------------------------
!
      subroutine append_sph_volume_spectr_file                          &
...
!
      ...
      call gz_read_sph_pwr_layer_head(FPz_f1, id_read_file, flag_gzip1, &
     &                                sph_lbl_IN1, sph_IN1, zbuf1)
!
      call alloc_sph_espec_name(sph_IN1)
      call sel_read_sph_spectr_name(FPz_f1, id_read_file, flag_gzip1,   &
...
!
     ...
     &   (FPz_f1, id_write_file, target_file_name, flag_gzip1, zbuf1)
!
      sph_OUT1%num_time_labels = 4
      call gz_read_sph_pwr_layer_head(FPz_f1, id_write_file,            &
...
!
      sph_IN1%num_time_labels = 4
      call gz_read_sph_pwr_layer_head(FPz_f1, id_read_file,             &
...
!
     ...
     &      id_write_file, ntot_pick, zbuf1)
        ...
        write(*,*) 'Read and select data'
        call pick_copy_sph_lspec_to_end                                 &
     &     (FPz_f1, id_read_file, id_write_file, flag_gzip1,            &
     &      comp_tbl1, sph_IN1, sph_OUT1, zbuf1)
        call dealloc_monitor_fld_pickup_tbl(comp_tbl1)
      end if
...
!
      ...
      call dealloc_sph_espec_name(sph_OUT1)
!
      end subroutine append_sph_layer_spectr_file
!
! -----------------------------------------------------------------------
! -----------------------------------------------------------------------
!
      subroutine pick_copy_sph_vmean_to_end                             &
...
!
      ...
      allocate(spectr_OUT(sph_OUT%ntot_sph_spec))
!
!$omp parallel workshare
      spectr_IN(1:sph_IN%ntot_sph_spec) = 0.0d0
!$omp end parallel workshare
!$omp parallel workshare
      spectr_OUT(1:sph_OUT%ntot_sph_spec) = 0.0d0
!$omp end parallel workshare
!
      do
        call gz_read_volume_pwr_sph(FPz_f, id_read_file, flag_gzip,     &
     &      sph_IN%ntot_sph_spec, sph_IN%i_step, sph_IN%time,           &
     &      spectr_IN(1), zbuf, ierr)
        if(ierr .gt. 0) exit
!
     ...
     &      ione, spectr_IN(1), spectr_OUT(1))
        call write_vol_sph_data(id_write_file, sph_OUT, spectr_OUT)
      end do
...
! -----------------------------------------------------------------------
!
      subroutine pick_copy_sph_lspec_to_end                             &
     &         (FPz_f, id_read_file, id_write_file, flag_gzip,          &
     &          comp_tbl, sph_IN, sph_OUT, zbuf)
!
      use sel_gz_input_sph_mtr_head
...
!$omp end parallel workshare
!$omp parallel workshare
      spectr_OUT(1:sph_OUT%ntot_sph_spec,                               &
     &           0:sph_OUT%ltr_sph,1:sph_OUT%nri_sph) = 0.0d0
!$omp end parallel workshare
!
      do
     ...
     &      sph_IN%i_mode, spectr_IN(1,0,1), zbuf, ierr)
        if(ierr .gt. 0) exit
!
     ...
     &               = sph_IN%r_sph(1:sph_IN%nri_sph)
        call pick_copy_monitor_data                                     &
     &     (comp_tbl, sph_IN%ntot_sph_spec, sph_OUT%ntot_sph_spec,      &
     &      n_mode, spectr_IN(1,0,1), spectr_OUT(1,0,1))
        call write_layer_spectr_data(id_write_file, sph_OUT,            &
     &                               spectr_OUT)
      end do
      deallocate(spectr_IN, spectr_OUT)
!
