...
!
      ...
      real(kind = kreal), allocatable :: sdev_spec_l(:,:,:)
      real(kind = kreal), allocatable :: spectr_pre_l(:,:,:)
!
      private :: id_file_rms
      private :: ave_spec_l, sdev_spec_l, spectr_pre_l
...
!
      ...
      call init_ene_spectr_2_pre                                        &
     &   (sph_IN%nri_sph, ltr, sph_IN%ntot_sph_spec,                    &
     &    ave_spec_l, rms_spec_l)
...
!
     ...
     &          sph_IN%nri_sph, ltr, sph_IN%ntot_sph_spec,              &
     &          sph_IN%spectr_IO, spectr_pre_l)
          else
            call sum_average_ene_spectr(sph_IN%time, pre_time,          &
     &          sph_IN%nri_sph, ltr, sph_IN%ntot_sph_spec,              &
     &          sph_IN%spectr_IO, ave_spec_l, rms_spec_l, spectr_pre_l)
...
!
!
      call divide_average_ene_spectr                                    &
     &   (sph_IN%time, time_ini, sph_IN%nri_sph, ltr,                   &
     &    sph_IN%ntot_sph_spec, ave_spec_l, rms_spec_l, sdev_spec_l)
!
!  Output average
      write(file_name, '(a6,a)') 't_ave_', trim(fname_org)
      call copy_average_ene_spectr(sph_IN%nri_sph, ltr,                 &
     &    sph_IN%ntot_sph_spec, ave_spec_l, sph_IN%spectr_IO)
!
...
!
!  Output average
      write(file_name, '(a6,a)') 't_sdev_', trim(fname_org)
      call copy_average_ene_spectr(sph_IN%nri_sph, ltr,                 &
     &    sph_IN%ntot_sph_spec, sdev_spec_l, sph_IN%spectr_IO)
!
      open(id_file_rms, file=file_name)
      call select_output_sph_pwr_head(id_file_rms, sph_IN)
      call select_output_sph_pwr_data(id_file_rms, sph_IN)
...
!
      ...
      allocate( rms_spec_l(ncomp,0:ltr,sph_IN%nri_sph) )
      allocate( sdev_spec_l(ncomp,0:ltr,sph_IN%nri_sph) )
...
!
      deallocate(ave_spec_l, rms_spec_l, sdev_spec_l, spectr_pre_l)
!
