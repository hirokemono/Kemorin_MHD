...
!!     &         (id_file, file_name, flag_gzip, zbuf)
!!      subroutine sel_close_sph_monitor_file(id_file, flag_gzip, zbuf)
!!        integer(kind = kint), intent(in) :: id_file
!!        character(len=kchara), intent(in) :: file_name
!!        logical, intent(inout) :: flag_gzip
!!        type(buffer_4_gzip), intent(inout) :: zbuf
!!      subroutine select_input_sph_series_head(id_file, flag_gzip,     &
!!     &          flag_old_fmt, flag_spectr, flag_vol_ave, sph_IN, zbuf)
!!      subroutine select_input_sph_series_data(id_file, flag_gzip,     &
...
!!      subroutine select_read_sph_spectr_time(id_file, flag_gzip,      &
!!     &          nitem_snap, i_step, time, zbuf, ierr)
!!        integer(kind = kint), intent(in) :: id_file
...
!
      ...
      if(flag_gzip) then
        ...
        call open_rd_gzfile_a(file_name, zbuf)
        return
      end if
#endif
!
...
!
     ...
     &   (id_file, flag_gzip, flag_vol_ave, sph_IN, zbuf)
!
      call alloc_sph_espec_name(sph_IN)
      call select_read_spectr_name                                      &
     &   (id_file, flag_gzip, sph_IN, zbuf)
!
...
#ifdef ZLIB_IO
      if(flag_gzip) then
        call gz_read_sph_spectr_time(nitem_snap, i_step, time,          &
     &                               zbuf, ierr)
        return
      end if
#endif
...
!   --------------------------------------------------------------------
!
      subroutine select_read_sph_monitor_head                           &
     &         (id_file, flag_gzip, flag_vol_ave, sph_IN, zbuf)
!
...
#ifdef ZLIB_IO
      if(flag_gzip) then
        call sel_gz_read_sph_monitor_head(flag_vol_ave, sph_IN, zbuf)
        return
      end if
#endif
!
...
!   --------------------------------------------------------------------
!
      subroutine select_read_spectr_name(id_file, flag_gzip,            &
     &                                   sph_IN, zbuf)
!
      use gz_spl_sph_spectr_head_IO
      use simple_sph_spectr_head_IO
...
!   --------------------------------------------------------------------
!
      subroutine select_copy_sph_monitor_data(id_read, id_write,        &
     &          flag_gzip, sph_IN, nchara_line, textbuf, zbuf, ierr)
!
      use gz_spl_sph_spectr_data_IO
      use count_monitor_time_series
!
      ...
      if(flag_gzip) then
        do kr = 1, sph_IN%nri_sph
          do lth = 0, sph_IN%ltr_sph
            call gz_copy_spectr_monitor_data(id_write, zbuf, ierr)
            if(ierr .gt. 0) return
          end do
        end do
        return
