...
!
      if(elsasser_ctl1%vol_degree_spectr_file_name%iflag .gt. 0) then
        els_dat1%vol_l_spectr_file_name                                 &
     &     = elsasser_ctl1%vol_degree_spectr_file_name%charavalue
      end if
      if(elsasser_ctl1%vol_order_spectr_file_name%iflag .gt. 0) then
        els_dat1%vol_m_spectr_file_name                                 &
     &     = elsasser_ctl1%vol_order_spectr_file_name%charavalue
      end if
      if(elsasser_ctl1%Elsasser_file_name_ctl%iflag .gt. 0) then
        els_dat1%elsasser_file_name                                     &
     &     = elsasser_ctl1%Elsasser_file_name_ctl%charavalue
      end if
!
      els_dat1%flag_old_spectr_data = .FALSE.
      if(elsasser_ctl1%old_format_ctl%iflag .gt. 0) then
        els_dat1%flag_old_spectr_data                                   &
     &     = yes_flag(elsasser_ctl1%old_format_ctl%charavalue)
      end if
!
      if(elsasser_ctl1%start_time_ctl%iflag .eq. 0) then
        write(*,*) 'Set start time'
        stop
      end if
      els_dat1%start_time = elsasser_ctl1%start_time_ctl%realvalue
!
      if(elsasser_ctl1%end_time_ctl%iflag .eq. 0) then
        write(*,*) 'Set end time'
        stop
      end if
      els_dat1%end_time = elsasser_ctl1%end_time_ctl%realvalue
!
      dimless_list1%num = 0
      if(elsasser_ctl1%dless_ctl%dimless%num .gt. 0) then
        dimless_list1%num = elsasser_ctl1%dless_ctl%dimless%num
      end if
      call copy_dimless_from_ctl                                        &
     &   (elsasser_ctl1%dless_ctl%dimless, dimless_list1)
      call check_dimless_numbers(dimless_list1)
!
      clist_ME_to_KE%num = 0
      if(elsasser_ctl1%ME_to_KE_ctl%num .gt. 0) then
        clist_ME_to_KE%num = elsasser_ctl1%ME_to_KE_ctl%num
      end if
      call copy_power_and_names_from_ctl                                &
     &   (elsasser_ctl1%ME_to_KE_ctl, clist_ME_to_KE)
      call check_power_and_name_list(clist_ME_to_KE)
!
      clist_elsasser%num = 0
      if(elsasser_ctl1%Elsasser_coefs%num .gt. 0) then
        clist_elsasser%num = elsasser_ctl1%Elsasser_coefs%num
      end if
      call copy_power_and_names_from_ctl                                &
     &   (elsasser_ctl1%Elsasser_coefs, clist_elsasser)
      call check_power_and_name_list(clist_elsasser)
!
      clist_mag_Reynolds%num = 0
      if(elsasser_ctl1%mag_Re_coefs%num .gt. 0) then
        clist_mag_Reynolds%num = elsasser_ctl1%mag_Re_coefs%num
      end if
      call copy_power_and_names_from_ctl                                &
     &   (elsasser_ctl1%mag_Re_coefs, clist_mag_Reynolds)
