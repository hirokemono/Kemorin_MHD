...
!
      ...
      integer(kind=kint) :: ist_true, ied_true, num_pe
...
!
      ...
      write(xz_stacked_code,'(2i10)')  ist_true, ied_true
      write(yz_stacked_code,'(2i10)')  ist_true, ied_true
      write(xyz_stacked_code,'(2i10)') ist_true, ied_true
!
...
!
      do iii = 0, nx_2+ny_2-2
        i1 = iii + 1
        write(xyz_stacked_code,'(i10,1p120e20.11)') iii,                &
     &            ene_xyz_tave(i1,1:num_fft)
      end do
!
      do ix = 0, nx_2-1
        i1 = ix+1
        write(yz_stacked_code,'(i10,1p120e20.11)') ix,                  &
     &            ene_yz_tave(i1,1:num_fft)
      end do
!
      do iy = 0, ny_2-1
        i1 = iy+1
        write(xz_stacked_code,'(i10,1p120e20.11)') iy,                  &
...
!
        do iz = 1, nz_all
          write(horiz_ave_code,'(i10,1p120e20.11)') iz, zz(iz),         &
     &            phys_cxcy_tave(iz,1:num_fft)
          write(horiz_rms_code,'(i10,1p120e20.11)') iz, zz(iz),         &
