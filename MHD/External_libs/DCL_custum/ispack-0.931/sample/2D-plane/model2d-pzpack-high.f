...
************************************************************************
***********************************************************************
*     model2d-pzpack-high.f:
*               pzpackを使って2次元非発散渦度方程式を解くプログラム
*               (高解像度版)
*                                                  2009/05/25 K.Ishioka
***********************************************************************
      IMPLICIT REAL*8(A-H,O-Z)

* パラメターの設定 

*      PARAMETER(LM=127,KM=127)    !切断波数の設定(aliasingをとっていない)
      PARAMETER(LM=85,KM=85)    !切断波数の設定(aliasingをとる場合)
...
*----------------------------------------------------------------------
      ...
      IMPLICIT REAL*8(A-H,O-Z)
      CHARACTER CTIME*5
      PARAMETER(JM=256,IM=256)    !格子点数の設定(JM: y方向, IM: x方向)
*      PARAMETER(LM=127,KM=127)    !切断波数の設定(aliasingをとっていない)
      PARAMETER(LM=85,KM=85)    !切断波数の設定(aliasingをとる場合)
      ...
      DIMENSION ITJ(4),TJ(JM*6),ITI(4),TI(IM*8) !PZPACKで使われる配列
...
* それをPZG2SAを使ってスペクトル係数に変換して, それをZの初期値と
* している
     ...
     &              +EXP(-((X-X2)**2+(Y-Y2)**2)/(2*SIGMA**2))
      ...
      CALL PZG2SA(LM,KM,JM,IM,WG,Z,WG(0,0,3),ITJ,TJ,ITI,TI)
...
*----------------------------------------------------------------------
      ...
      CALL PZAJBS(LM,KM,JM,IM,R,Z,DZ,WS,WG,ITJ,TJ,ITI,TI)
...
*----------------------------------------------------------------------
      ...
      CALL PZS2GA(LM,KM,JM,IM,Z,WG,WG(0,0,3),ITJ,TJ,ITI,TI)
