...
************************************************************************
***********************************************************************
*     浅水方程式をsppackで解くためのサンプルプログラム       2002/05/19
*     (角運動量を保存する散逸項を使う)
...
*      Y0=PI/4     !擾乱の中心位置のφ座標
      ...
      CALL SBOPEN(IU,CF,DNU,ALPHA,NDV,DT)
...
************************************************************************
      SUBROUTINE SBOPEN(IUD,CF,DNUD,ALPHAD,NDV,DT)
...
*      DIMENSION WW(JW*IW),WS(8*JW*IW) ! SPMWNV を使う場合
*      DIMENSION WW(JW*IW*8),WS(8*JW*IW) ! SPMWNW を使う場合
      DIMENSION WW(JW*IW*7),WS(7*JW*IW) ! SPMWNX を使う場合      
      DIMENSION W(LM,3)      
      SAVE
...
*/ 重力波成分を陽に分離して解く際に必要な配列の準備
      ...
      CALL SPSWLI(MM,BARPHI,DNU,ALPHA,DT/(2*NDV),CL)
...
*      CALL SPMWNV(MM,IM,ID,JC,JDD,OMEGA,BARPHI,DNU,ALPHA,
*      CALL SPMWNW(MM,IM,ID,JC,JDD,OMEGA,BARPHI,DNU,ALPHA,
      CALL SPMWNX(MM,IM,ID,JC,JDD,OMEGA,BARPHI,DNU,
...
*-----------------------------------------------------------------------
      ...
      CALL SPMWCX(MM,IM,ID,JC,JDD,OMEGA,
     &  VAR(1,1),VAR(1,2),VAR(1,3),AM1,AM2,AM3,AENE,AENS,
     &  RN,IT,T,Y,IP,P,R,IA,A,Q,WS,WW)
      
      IF(IPROC.EQ.0) THEN
        print *,'amom1= ',AM1,' amom2= ',AM2,' amom3= ',AM3,
     &    ' aene=',AENE,' aens=',AENS
      END IF
