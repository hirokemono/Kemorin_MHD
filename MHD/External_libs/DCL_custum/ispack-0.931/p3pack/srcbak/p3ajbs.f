...
************************************************************************
      SUBROUTINE P3AJBS(NM,MM,LM,KM,JM,IM,
...
* U --> W(*,2)
      ...
      DO L=-LM,-1
        DO M=-MM,MM
          DO N=-NM,NM
            WS(-N,-M,-L,1)=(M*Z(N,M,L,2)-N*Z(N,M,L,1))/(L*L+M*M+N*N)
          END DO
        END DO
      END DO
      DO L=1,LM
...
* V --> W(*,3)
      ...
      DO L=-LM,-1
        DO M=-MM,MM
          DO N=-NM,NM
            WS(-N,-M,-L,1)=-(N*M*Z(N,M,L,1)+(L*L+N*N)*Z(N,M,L,2))
     &        /((L*L+M*M+N*N)*L)
          END DO
        END DO
      END DO
     ...
     &        /((L*L+M*M+N*N)*L)
      ...
      DO M=-MM,-1
        DO N=-NM,NM
          WS(-N,-M,-L,1)=N*Z(N,M,L,1)/(M*M+N*N)
        END DO
      END DO
      DO M=1,MM
        DO N=-NM,NM
...
* W --> W(*,4)
      ...
      DO L=-LM,-1
        DO M=-MM,MM
          DO N=-NM,NM
            WS(-N,-M,-L,1)=((L*L+M*M)*Z(N,M,L,1)+N*M*Z(N,M,L,2))
     &        /((L*L+M*M+N*N)*L)
          END DO
        END DO
      END DO
     ...
     &        /((L*L+M*M+N*N)*L)
      ...
      DO M=-MM,-1
        DO N=-NM,NM
          WS(-N,-M,-L,1)=-M*Z(N,M,L,1)/(M*M+N*N)
        END DO
      END DO
      DO M=1,MM
        DO N=-NM,NM
...
*      AENE=0
*      DO IJK=1,IJKM
*        AENE=AENE+W(IJK,2)**2+W(IJK,3)**2+W(IJK,4)**2
*      END DO
*      WRITE(6,'(A,F20.15)') 'ENECHECK=',AENE/2/IJKM
