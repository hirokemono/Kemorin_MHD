...
************************************************************************
************************************************************************
*     TRANSFORM GRID TO SPECTRA (y-integral)                  2009/06/15
************************************************************************
      SUBROUTINE UJG2SY(LM,KM,JM,IM,G,S,W,ITJ,TJ,ITI,TI,Y,R)
...
!$omp parallel do
      DO I=0,IM-1
        W(I)=G(0+JM*I)
        DO L=1,LM
          W(I+IM*(2*L-1))= G(2*L+1+JM*I)/L
          W(I+IM*(2*L  ))=-G(2*L  +JM*I)/L
        END DO
      END DO
!$omp end parallel do
...
!$omp parallel do
      DO L=1,LM
        ...
        DO K=1,KM
          S( L, K)
     &      =F*( TJ(L,13)*W(2*K+IM*(2*L-1))+TJ(L,14)*W(2*K+IM*(2*L  )))
          S(-L, K)
     &      =F*(-TJ(L,13)*W(2*K+IM*(2*L  ))+TJ(L,14)*W(2*K+IM*(2*L-1)))
          S( L,-K)
     &      =F*( TJ(L,13)*W(2*K+1+IM*(2*L-1))
     &          +TJ(L,14)*W(2*K+1+IM*(2*L  )))
          S(-L,-K)
     &      =F*(-TJ(L,13)*W(2*K+1+IM*(2*L  ))
     &          +TJ(L,14)*W(2*K+1+IM*(2*L-1)))
        END DO
      END DO
!$omp end parallel do      
