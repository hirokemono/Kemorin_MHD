...
! 02110-1301 USA.
!***********************************************************************
SUBROUTINE SYTS2V(MM,NM,NN,IM,JM,JV,S1,S2,G1,G2,IT,T,P,R,JC,W,IPOW,ICOM)
  ...
  IF(MN.GE.1) THEN
     ...
     DO K=0,MN-1
        ...
        IF(.NOT.LP(ITH)) THEN
           CALL MXALLC(WKP(ITH),JV*11*JR)
           CALL C_F_POINTER(WKP(ITH),WK,[JV*11*JR])
           CALL MXALLC(WSP(ITH),4*(NM+1))
           CALL C_F_POINTER(WSP(ITH),WS,[4*(NM+1)])
           CALL MXALLC(WGP(ITH),JM*2*2)
           CALL C_F_POINTER(WGP(ITH),WG,[JM*2,2_8])
           LP(ITH)=.TRUE.
        END IF
        ...
        IF(M.EQ.0) THEN
           CALL LXSSZV(NM,NN,JM,JV,JR,S1,S2,WG(1,1),WG(1,2),P,R,WK,WS,IPOW,0_8)                   
        ELSE
           CALL LXSSWV(NM,NN,JM,JV,JR,M,S1(NS),S2(NS),WG(1,1),WG(1,2), &
        ...
        DO WHILE(JS.LE.2*JM) 
           ...
           DO J=JS,JE
              W(NBF*2+(IPDEST*(MM/NP+1)+K)*JI*2+J-JS+1)=WG(J,1)
              W(NBF*3+(IPDEST*(MM/NP+1)+K)*JI*2+J-JS+1)=WG(J,2)              
           END DO
           JS=JE+1
        END DO
     END DO
  ...
  CALL MPI_ALLTOALL(W(NBF*2+1),NB,MPI_REAL8,W,NB,MPI_REAL8,ICOM4,IERR)
  CALL MPI_ALLTOALL(W(NBF*3+1),NB,MPI_REAL8,W(NBF+1),NB,MPI_REAL8,ICOM4,IERR)  
  ...
  IF(JP1.NE.0) THEN  
     ...
     DO JD=1,(JP2-JP1+1)/JV
        !$ ITH=omp_get_thread_num()
        IF(.NOT.LP(ITH)) THEN
           CALL MXALLC(WKP(ITH),JV*IM*2)
           CALL C_F_POINTER(WKP(ITH),WK,[JV*IM*2])
           LP(ITH)=.TRUE.
        END IF
        DO M=0,MM
           K=M/NP
           ...
           DO IV=1,JV*2
              ...
              WK(JV*IM+IV+(JV*2)*M) &
                   &  =W(NBF+IV+2*JV*(JD-1)+JI*2*(K+(MM/NP+1)*IPSRC))
           END DO
        END DO
        DO M=MM+1,IM/2-1
           DO IV=1,JV*2
              WK(IV+(JV*2)*M)=0              
              WK(JV*IM+IV+(JV*2)*M)=0              
           END DO
        END DO
        CALL FXRTBA(JV,IM,WK,IT,T)
        CALL FXRTBA(JV,IM,WK(JV*IM+1),IT,T)                
        DO IV=1,JV
           DO I=1,IM
              G1(I+IM*(IV-1+JV*(JD-1)))=WK(IV+JV*(I-1))
              G2(I+IM*(IV-1+JV*(JD-1)))=WK(JV*IM+IV+JV*(I-1))              
           END DO
        END DO
     END DO
     !$omp end do
