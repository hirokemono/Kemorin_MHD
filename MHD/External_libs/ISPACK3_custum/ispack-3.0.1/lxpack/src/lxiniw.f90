...
SUBROUTINE LXINIW(NM,JM,M,P,PM,R,JC)
  ...
  REAL(8) :: P(JM/2,5)
  REAL(8) :: QBIG,QBIGR,QBIGL,QCHK1,QCHK2,QPME
...
!  REAL(16),DIMENSION(:),ALLOCATABLE :: QWP
  REAL(8),DIMENSION(:),ALLOCATABLE :: QWP
  ...
  QPME=0
  DO MD=1,M
     QPME=QPME+0.5Q0*LOG(QONE*(2*MD+1)/(2*MD))
  END DO
  DPME=QPME
  ...
  IF(NM.LE.M+1) THEN
     ...
     END DO
     DO J=1,JC(IJ+LD)
        QCHK1=QPME+LOG(ABS(QONE*PM(J,1)))+M*LOG(QONE*P(J,3))
        PM(J,1)=SIGN(EXP(QCHK1),QONE*PM(J,1))
     END DO
  ELSE
     L=0
     LD=1
     ...
     DO J=1,JC(IJ+LD)
        QCHK1=QPME+LOG(ABS(QONE*PM(J,1)))+M*LOG(QONE*P(J,3))
        QCHK2=QPME+LOG(ABS(QONE*PM(J,2)))+M*LOG(QONE*P(J,3))
        PM(J,1)=SIGN(EXP(QCHK1),QONE*PM(J,1))
        PM(J,2)=SIGN(EXP(QCHK2),QONE*PM(J,2))
     END DO

     DO J=JC(IJ+LD)+1,JH
        QWP(J)=QPME+M*LOG(QONE*P(J,3))
        DWP(J)=QWP(J)
     END DO
     ...
     DO N=M,NM-10,8
        ...
        DO J=JC(IJ+LD)+1,JC(IJ+LD+1)
           QCHK1=LOG(ABS(QONE*PM(J,1)))+QWP(J)
           QCHK2=LOG(ABS(QONE*PM(J,2)))+QWP(J)
           PM(J,1)=SIGN(EXP(QCHK1),QONE*PM(J,1))
           PM(J,2)=SIGN(EXP(QCHK2),QONE*PM(J,2))
        END DO
     END DO
