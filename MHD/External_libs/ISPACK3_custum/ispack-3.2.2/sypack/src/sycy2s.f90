...
!***********************************************************************
!***********************************************************************
!     REPACKING SPECTRA FOR y-div                            
!***********************************************************************
SUBROUTINE SYCY2S(MM,NT,SG,S,C,ICOM)

  IMPLICIT NONE    
  INCLUDE 'mpif.h'
  INTEGER(8) :: MM,M1,N1,MN,NS,NSG,M,K,NT,L,ICOM
  ...
  DO K=0,MN-1
     ...
     IF(M.EQ.0) THEN
        CALL LXCYZS(NT,SG,S,C)
     ELSE
        CALL LXCYWS(NT,M,SG(NSG),S(NS),C(NS))
     END IF
  END DO
  !$omp end do
  !$omp end parallel

END SUBROUTINE SYCY2S
