...
! 02110-1301 USA.
!***********************************************************************
SUBROUTINE SYTG2S(MM,NM,NN,IM,JM,JV,S,G,IT,T,P,R,JC,W,IPOW,ICOM)
  ...
  IF(JP1.NE.0) THEN  
     ...
     CALL MXALLC(WKP,JV*IM)
     CALL C_F_POINTER(WKP,WK,[JV*IM])
     !$omp do schedule(dynamic)
     DO JD=1,(JP2-JP1+1)/JV
        DO IV=1,JV
           DO I=1,IM
              WK(IV+JV*(I-1))=G(I+IM*(IV-1+JV*(JD-1)))
           END DO
        END DO
        CALL FXRTFA(JV,IM,WK,IT,T)
        DO M=0,MM
           K=M/NP
  ...
  CALL MPI_ALLTOALL(W,NB,MPI_REAL8,W(NBF+1),NB,MPI_REAL8,ICOM4,IERR)    
  ...
  IF(MN.GE.1) THEN
     ...
     CALL MXALLC(WKP,JV*7*JR)
     CALL C_F_POINTER(WKP,WK,[JV*7*JR])
     CALL MXALLC(WSP,2*(NM+1))
     CALL C_F_POINTER(WSP,WS,[2*(NM+1)])
     CALL MXALLC(WGP,JM*2)
     CALL C_F_POINTER(WGP,WG,[JM*2])
     !$omp do schedule(dynamic)
     DO K=0,MN-1
        JS=1
        IPDEST=-1
        DO WHILE(JS.LE.2*JM) 
           ...
           DO J=JS,JE
              WG(J)=W(NBF+(IPDEST*(MM/NP+1)+K)*JI*2+J-JS+1)
           END DO
           JS=JE+1
        END DO
        ...
        IF(M.EQ.0) THEN
           CALL LXSGZS(NM,NN,JM,JV,JR,S,WG,P,R,WK,WS,IPOW,0_8)        
        ELSE
           CALL LXSGWS(NM,NN,JM,JV,JR,M,S(NS),WG,P,P(1,6+K*2), &
