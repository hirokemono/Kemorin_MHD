...
!***********************************************************************
! ISPACK FORTRAN SUBROUTINE LIBRARY FOR SCIENTIFIC COMPUTING
! Copyright (C) 1998--2022 Keiichi Ishioka <ishioka@gfd-dennou.org>
!
! This library is free software; you can redistribute it and/or
! modify it under the terms of the GNU Lesser General Public
! License as published by the Free Software Foundation; either
...
! 02110-1301 USA.
!***********************************************************************
SUBROUTINE LXXVWS(JB,AC,SD,Q,IL,ILEV)

  IMPLICIT NONE
  INTEGER(8),PARAMETER :: JV=256
  INTEGER(8) :: J,JB,IR,ILEV,IL
  INTEGER(8) :: I,K  
  REAL(8) :: AC(*),SD(4,*),Q(JV,0:10,JB)
  REAL(8) :: SDV(JV,4,10)  
  ...
  IF(ILEV.EQ.0) THEN
     ...
     DO IR=1,JB
        DO J=1,JV
           ...
           SDV(J,3,2)=SDV(J,3,2)+Q(J,7,IR)*Q(J,1,IR)
           SDV(J,4,2)=SDV(J,4,2)+Q(J,9,IR)*Q(J,1,IR)
           SDV(J,3,1)=SDV(J,3,1)+Q(J,8,IR)*Q(J,1,IR)
           SDV(J,4,1)=SDV(J,4,1)+Q(J,10,IR)*Q(J,1,IR)
           ...
           SDV(J,3,4)=SDV(J,3,4)+Q(J,7,IR)*Q(J,2,IR)
           SDV(J,4,4)=SDV(J,4,4)+Q(J,9,IR)*Q(J,2,IR)
           SDV(J,3,3)=SDV(J,3,3)+Q(J,8,IR)*Q(J,2,IR)
           SDV(J,4,3)=SDV(J,4,3)+Q(J,10,IR)*Q(J,2,IR)
           ...
           SDV(J,3,6)=SDV(J,3,6)+Q(J,7,IR)*Q(J,1,IR)
           SDV(J,4,6)=SDV(J,4,6)+Q(J,9,IR)*Q(J,1,IR)
           SDV(J,3,5)=SDV(J,3,5)+Q(J,8,IR)*Q(J,1,IR)
           SDV(J,4,5)=SDV(J,4,5)+Q(J,10,IR)*Q(J,1,IR)
           ...
           SDV(J,3,8)=SDV(J,3,8)+Q(J,7,IR)*Q(J,2,IR)
           SDV(J,4,8)=SDV(J,4,8)+Q(J,9,IR)*Q(J,2,IR)
           SDV(J,3,7)=SDV(J,3,7)+Q(J,8,IR)*Q(J,2,IR)
           SDV(J,4,7)=SDV(J,4,7)+Q(J,10,IR)*Q(J,2,IR)
  ...
  ELSE IF(ILEV.EQ.9) THEN
     ...
     DO IR=1,JB
        DO J=1,JV
           ...
           SDV(J,3,10)=SDV(J,3,10)+Q(J,7,IR)*Q(J,1,IR)
           SDV(J,4,10)=SDV(J,4,10)+Q(J,9,IR)*Q(J,1,IR)
           SDV(J,3,9)=SDV(J,3,9)+Q(J,8,IR)*Q(J,1,IR)
           SDV(J,4,9)=SDV(J,4,9)+Q(J,10,IR)*Q(J,1,IR)
        END DO
     END DO
     DO K=1,10
        DO I=1,4
