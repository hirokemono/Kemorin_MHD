...
! 02110-1301 USA.
!***********************************************************************
SUBROUTINE SXTS2G(MM,NM,NN,IM,JM,S,G,IT,T,P,R,JC,W,IPOW)
...
!$omp parallel private(NS,WP,WSP,IJ,IE,WORK,WS) num_threads(NP)
  ...
  DO M=0,MM
     IF(M.EQ.0) THEN
        CALL LXSSZG(NM,NN,JM,JV,JR,S,W,P,R,WORK,WS,IPOW,0_8)
     ELSE
        NS=1+NN+1+(M-1)*(NN+NN+2-M)
        IE=(M*(2*NM-M)+1)/4*3+M*(2*NM-M+1)/2+M+1
        IJ=(M-1)*(2*NM-M)/16+M
        CALL LXSSWG(NM,NN,JM,JV,JR,M,S(NS),W(1+M*2*JM),P,P(1,4+M*2), &
...
!$omp parallel private(WP,WORK) num_threads(NP)
  ...
  DO JD=1,JM/JV
     IF(JV.EQ.4) THEN     
        CALL SXQTB1(JM/JV,MM,IM,W(1+2*JV*(JD-1)),WORK)
     ELSE IF(JV.EQ.8) THEN     
        CALL SXOTB1(JM/JV,MM,IM,W(1+2*JV*(JD-1)),WORK)
     ELSE
        CALL SXLTB1(JV,JM/JV,MM,IM,W(1+2*JV*(JD-1)),WORK)
     END IF
     CALL FXRTBA(JV,IM,WORK,IT,T)
     IF(JV.EQ.4.AND.MOD(IM,4).EQ.0) THEN     
        CALL SXQTB2(IM,WORK,G(1+JV*IM*(JD-1)))
     ELSE IF(JV.EQ.8.AND.MOD(IM,8).EQ.0) THEN     
        CALL SXOTB2(IM,WORK,G(1+JV*IM*(JD-1)))
     ELSE
        CALL SXLTB2(JV,IM,WORK,G(1+JV*IM*(JD-1)))
     END IF
  END DO
  !$omp end do
  CALL MXFREE(WP)  
